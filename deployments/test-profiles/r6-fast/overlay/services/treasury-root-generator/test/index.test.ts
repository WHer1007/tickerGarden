...
const SINGLE_HOLDER_VECTOR = "0x949b4f088cc70573aa7f25320b07dd963c425c7f10b7d619af57046cfbae10be";
...
test("rejects incomplete histories, non-1-hour windows, and observations beyond the source tip", () => {
  assert.throws(
  ...
  assert.throws(() => generateTreasuryRoot(wrongWindow), /exactly 1 hour/);
...
test("zero eligible TWAB requires explicit policy and returns a reviewed rollover commitment", () => {
  const source = input([observation(1n, START, ZERO, ALICE, 100n)], [ALICE]);
  assert.throws(() => generateTreasuryRoot(source), /verified empty-epoch policy/);
  const result = generateTreasuryRoot({ ...source, emptyEpochPolicy: "reviewed-rollover" });
  assert.equal(result.merkleRoot, EMPTY_EPOCH_ROOT);
  assert.equal(result.totalAllocated, 0n);
  assert.equal(result.totalTwab, 0n);
  assert.equal(result.leafCount, 0);
  assert.deepEqual(result.leaves, []);
  const changedFunding = generateTreasuryRoot({ ...source, quoteAmount: 2000n, emptyEpochPolicy: "reviewed-rollover" });
  assert.notEqual(changedFunding.datasetHash, result.datasetHash);
});
