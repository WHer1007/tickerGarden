...
def run(root, data, env, rpc):
    ...
    for signature, value in {"canonicalPoolKey(bytes32)": pool_key, "canonicalPoolId(bytes32)": bytes.fromhex(pool_id), "canonicalRoute(bytes32)": route, "swapRouter()": words[7], "quoter()": words[8], "activeFeeSource(bytes32)": bytes.fromhex(curve_word + format(1, "064x"))}.items():
    ...
    if asset_fixture:
        ...
        asset_entries = {selector("asset(bytes32)"): bytes.fromhex(stock_word + vault_word) + payload(18, 1),
    ...
    if holder:
        ...
        treasury_entries = {selector("marketRegistry()"): words[5],
                            selector("EPOCH_DURATION()"): payload(60 * 60),
                            selector("TWAB_SCHEMA()"): bytes.fromhex(rpc("web3_sha3", ["0x" + b"TRANSFER_LOG_TWAB_1H_R6_TEST_ONLY".hex()])[2:]),
    ...
    rpc("anvil_setCode", [curve, getter_runtime({selector("accruedCurveFees()"): payload(7), selector("sweepCurveFees()"): payload(7)})])
