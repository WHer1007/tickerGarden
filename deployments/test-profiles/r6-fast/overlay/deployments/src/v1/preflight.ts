...
import { assertV1Deployable, assertV1DeploymentManifest, assertV1ProductionReady } from "../index.ts";

type JsonRecord = Record<string, unknown>;

export interface V1ReadOnlyRpc {
  request(method: string, params: readonly unknown[]): Promise<unknown>;
}

export type V1LivePreflightReport = Readonly<{
  chainId: number;
  blockNumber: string;
  blockHash: string;
...
export class HttpV1ReadOnlyRpc implements V1ReadOnlyRpc {
  ...
  constructor(url: string, timeoutMs = 15_000) {
    ...
    if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
      throw new Error(`V1 preflight HTTP transport requires http(s), received ${parsed.protocol}`);
    }
    if (!Number.isSafeInteger(timeoutMs) || timeoutMs <= 0) {
      throw new Error("V1 preflight RPC timeout must be a positive integer");
    }
  ...
  async request(method: string, params: readonly unknown[]): Promise<unknown> {
    if (!READ_ONLY_METHODS.has(method)) {
      throw new Error(`V1 preflight rejected non-read-only RPC method: ${method}`);
    }
    const id = ++this.#id;
    ...
    if (!response.ok) throw new Error(`V1 preflight RPC HTTP ${response.status}`);
    const payload = (await response.json()) as JsonRecord;
    if (payload.jsonrpc !== "2.0" || payload.id !== id) {
      throw new Error(`V1 preflight RPC ${method} returned a mismatched response envelope`);
    }
    if (payload.error !== undefined) {
      throw new Error(`V1 preflight RPC ${method} failed: ${JSON.stringify(payload.error)}`);
    }
    if (!("result" in payload)) throw new Error(`V1 preflight RPC ${method} omitted result`);
    return payload.result;
  }
}

function fail(label: string, expected: unknown, actual: unknown): never {
  throw new Error(`V1 live preflight mismatch at ${label}: expected=${String(expected)} actual=${String(actual)}`);
}

function stringField(record: JsonRecord, key: string): string {
  const value = record[key];
  if (typeof value !== "string") throw new Error(`V1 live preflight invalid string: ${key}`);
  return value;
}
...
function numberField(record: JsonRecord, key: string): number {
  ...
  if (typeof value !== "number" || !Number.isSafeInteger(value)) {
    throw new Error(`V1 live preflight invalid integer: ${key}`);
  }
  return value;
}
...
function normalizedHex(value: unknown, label: string): string {
  if (typeof value !== "string" || !/^0x(?:[0-9a-fA-F]{2})*$/.test(value)) {
    throw new Error(`V1 live preflight invalid hex from ${label}`);
  }
...
function words(value: unknown, label: string, minimum: number): string[] {
  ...
  if (hex.length % 64 !== 0 || hex.length / 64 < minimum) {
    throw new Error(`V1 live preflight malformed ABI result at ${label}`);
  }
...
function verifyCreate2Evidence(manifest: Manifest): void {
  ...
  if (stringField(gauge, "actualAddress").toLowerCase() === stringField(gauge, "implementationAddress").toLowerCase()) {
    throw new Error("V1 live preflight Gauge clone address aliases its implementation");
  }
  ...
  if (identityCheck === undefined) {
    throw new Error("V1 live preflight Gauge clone lacks gaugeIdentity() evidence");
  }
  same(
...
function verifyLaunchConfigResolverEvidence(manifest: Manifest): void {
  ...
  for (const [getter, registryName] of bindings) {
    ...
    if (check === undefined) {
      throw new Error(`V1 live preflight LaunchConfigResolver lacks ${getter} evidence`);
    }
...
function verifyProxyEvidence(manifest: Manifest): void {
  ...
  for (const [index, stock] of manifest.officialStocks.entries()) {
    ...
    if (uidHash !== expectedUidHash) {
      fail(`officialStocks.${index}.uidGetter`, expectedUidHash, uidHash);
    }
    ...
    if (decimalsHash !== expectedDecimalsHash) {
      fail(`officialStocks.${index}.decimalsGetter`, expectedDecimalsHash, decimalsHash);
    }
  }
  for (const [index, quote] of manifest.quoteAssets.entries()) {
...
function comparePermissionSemantics(manifest: Manifest): void {
  ...
  for (const [name, module] of Object.entries(manifest.protocolModules)) {
    ...
    if (deploymentAddresses.has(deployedAddress)) throw new Error(`V1 live preflight aliased protocol module: ${name}`);
  ...
  const expectedRoles: Readonly<Record<string, { member: string; delay: number }>> = {
    ...
    UNPAUSE_ROLE: { member: stringField(manifest.roleHandoff, "securityOrGovernanceSafe"), delay: 1200 },
  ...
  for (const [index, role] of manifest.accessManager.roles.entries()) {
    ...
    if (expected === undefined) throw new Error(`V1 live preflight unexpected AccessManager role: ${name}`);
    const roleId = stringField(role, "roleId");
    if (roleId === "0" || [...roleIds.values()].includes(roleId)) {
      throw new Error(`V1 live preflight invalid or duplicate roleId at accessManager.roles.${index}`);
    }
...
async function verifyMarketProbe(manifest: Manifest, rpc: V1ReadOnlyRpc, blockTag: string): Promise<number> {
  ...
  const registry = stringField(manifest.protocolModules.MarketRegistryV1!, "deployedAddress");
...
function sortedLogs(value: unknown): RpcLog[] {
  if (!Array.isArray(value)) throw new Error("V1 live preflight eth_getLogs returned non-array");
  const logs = value.map((entry) => {
    if (entry === null || typeof entry !== "object") throw new Error("V1 live preflight malformed log");
    const record = entry as JsonRecord;
    if (!Array.isArray(record.topics) || record.topics.some((topic) => typeof topic !== "string")) throw new Error("V1 live preflight malformed log topics");
...
async function verifyAccessManager(manifest: Manifest, rpc: V1ReadOnlyRpc, blockTag: string): Promise<{ permissions: number; administrativePermissions: number; memberships: number; revoked: number; events: number }> {
  ...
  if (!revokedAccounts.some((account) => account.toLowerCase() === stringField(manifest.roleHandoff, "deployer").toLowerCase())) {
    throw new Error("V1 live preflight revokedAccounts must include roleHandoff.deployer");
  }
  let revoked = 0;
  ...
  for (const [index, log] of logs.entries()) {
    if (log.topics.length !== 3) throw new Error(`V1 live preflight malformed AccessManager event ${index}`);
  ...
  for (const [index, log] of roleLogs.entries()) {
    if (log.topics.length !== 3) throw new Error(`V1 live preflight malformed AccessManager role event ${index}`);
    ...
    if (eventTopic === roleGrantedTopic) {
      ...
      throw new Error(`V1 live preflight unexpected AccessManager role event ${index}`);
    }
  }
...
async function verifyRoleHandoffReceipts(
  ...
  for (const expected of expectedReceipts) {
    ...
    if (raw === null || typeof raw !== "object") {
      throw new Error(`V1 live preflight missing transaction receipt: ${expected.label}`);
    }
...
export async function verifyV1LiveState(candidate: unknown, rpc: V1ReadOnlyRpc): Promise<V1LivePreflightReport> {
  assertV1DeploymentManifest(candidate);
  ...
  if (typeof rawChainId !== "string" || !/^0x[0-9a-fA-F]+$/.test(rawChainId)) {
    throw new Error("V1 live preflight invalid quantity from eth_chainId");
  }
  ...
  if (rawBlock === null || typeof rawBlock !== "object") throw new Error("V1 live preflight finalized block unavailable");
  ...
  for (const check of [
    ...
    if (!codeExpectations.has(target)) {
      throw new Error(`V1 live preflight probe target is not a manifested contract: ${target}`);
    }
  }
...
export async function preflightV1Deployment(candidate: unknown, rpc: V1ReadOnlyRpc): Promise<V1LivePreflightReport> {
  assertV1Deployable();
