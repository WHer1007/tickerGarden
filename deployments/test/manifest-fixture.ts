...
function moduleDeployment(name: string): JsonRecord {
  return {
    moduleKind: ["TickerMemeTokenV2", "PonsCompatibleCurve", "MemeStockGauge", "LaunchLocker"].includes(name) ? "PER_MARKET_IMPLEMENTATION" : name === "UserStockVault" ? "PER_ASSET_IMPLEMENTATION" : "SHARED_IMMUTABLE",
...
function create2Component(kind: string): JsonRecord {
  return {
    deployer: address(`${kind}:deployer`), saltDomain: hash(`${kind}:domain`), salt: hash(`${kind}:salt`), initCodeHash: hash(`${kind}:init`), runtimeCodeHash: hash(`${kind}:runtime`), constructorArgsHash: hash(`${kind}:constructor`), immutableArgsHash: hash(`${kind}:immutable`), predictedAddress: address(`${kind}:predicted`), actualAddress: address(`${kind}:actual`), collisionPolicy: "REVERT_NO_NONCE_FALLBACK",
  };
}
...
export function validManifest(): JsonRecord {
  ...
  return {
    schemaVersion: 1, manifestKind: "TICKERGARDEN_V2_NETWORK_DEPLOYMENT", executionSpecId: "V2-EXEC-3", releaseStatus: "DEPLOYMENT_CANDIDATE",
    ...
    create2: { marketId: hash("market-id"), creator: address("creator"), components: Object.fromEntries(["TOKEN", "CURVE", "GAUGE", "LOCKER"].map((kind) => [kind, create2Component(kind)])), predictionVectorHash: hash("prediction-vector"), predictionMatchesDeployment: true },
