...
function moduleDeployment(name: string): JsonRecord {
  const moduleKind = name === "MemeStockGauge"
    ...
    : ["TickerMemeTokenV1", "PonsCompatibleCurve", "LaunchLocker"].includes(name)
...
export function validManifest(): JsonRecord {
  ...
  return {
    schemaVersion: 1, manifestKind: "TICKERGARDEN_V1_NETWORK_DEPLOYMENT", executionSpecId: "V1-EXEC-10", releaseStatus: "DEPLOYMENT_CANDIDATE",
    ...
    livePreflight: {
      ...
      keyGetterChecks: [
        ...
        { label: "pons-baseline-registry-access-manager-authority", category: "IMMUTABLE_BINDING", target: String((protocolModules.PonsBaselineRegistry as JsonRecord).deployedAddress), callData: functionSelector("authority()"), expectedReturnDataHash: addressReturnHash(address("access-manager")) },
