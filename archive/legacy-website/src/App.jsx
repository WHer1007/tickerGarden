...
import { marketHref, parseSiteRoute, routeIsActive, SITE_NAVIGATION } from "./site/model.ts";
import { FaqPage, HomePage, MarketsPage, NotFoundPage, StatsPage } from "./site/ContentPages.jsx";
import {
...
function pageCopy(route) {
  switch (route.name) {
    case "home": return { eyebrow: "V1 market infrastructure · V1-EXEC-8", lead: "Markets that begin", accent: "from visible state.", description: "Launch, inspect and participate through one canonical view of Robinhood Chain." };
    case "create": return { eyebrow: "Create a Meme market", lead: "Approved roots.", accent: "Permissionless growth.", description: "Bind official STOCK context, Quote economics and launch components before deployment." };
    case "markets": return { eyebrow: "Market directory", lead: "One revision.", accent: "Every market.", description: "Discover the canonical lifecycle and open a deep-linked market surface." };
    case "market": return { eyebrow: "Market detail & trade", lead: "Live reserves.", accent: "Fresh intent.", description: `Canonical market ${shortHex(route.marketId, 10, 8)}.` };
    case "portfolio": return { eyebrow: "STOCK Vault & earnings", lead: "Principal visible.", accent: "Exit always yours.", description: "Deposit, allocate, claim, close—or return principal immediately through Rage Quit." };
    case "stats": return { eyebrow: "Protocol statistics", lead: "Count what is known.", accent: "Name what is not.", description: "Finalized lifecycle counts and denomination-safe raw balance totals." };
    case "faq": return { eyebrow: "Protocol field guide", lead: "Understand first.", accent: "Then sign.", description: "Clear answers about launch, trading, STOCK allocation, rewards and safety gates." };
    default: return { eyebrow: "Unknown route", lead: "No action taken.", accent: "Find a known path.", description: "The requested page does not map to a TickerGarden product surface." };
  }
}
...
async function readAllMarkets(revision) {
  const items = [];
  let cursor;
  for (let page = 0; page < 16; page += 1) {
    const response = await readApi.listMarkets({ limit: 100, ...(cursor ? { cursor } : {}) });
    assertSameRevision(response.sync, revision, "market snapshot");
    items.push(...response.items);
    if (!response.nextCursor) return items;
    cursor = response.nextCursor;
  }
...
function App() {
  const [route, setRoute] = useState(() => parseSiteRoute(window.location.pathname));
  ...
  useEffect(() => {
    const handlePopState = () => setRoute(parseSiteRoute(window.location.pathname));
    window.addEventListener("popstate", handlePopState);
    return () => window.removeEventListener("popstate", handlePopState);
  }, []);

  useEffect(() => {
    const labels = { home: "Overview", create: "Create", markets: "Markets", market: "Market", portfolio: "Portfolio", stats: "Stats", faq: "FAQ", "not-found": "Not found" };
    document.title = `${labels[route.name]} — TickerGarden V1`;
  }, [route.name]);
  ...
  const followSiteLink = useCallback((event, href) => {
  ...
  const loadFoundation = useCallback(async () => {
    ...
    try {
      ...
      const [assets, quotes, pons, templates, markets, rawBindings, launchFee] = await Promise.all([
      ...
      setFoundation({ status: "ready", health, sync: health.sync, assets, quotes, pons, templates, markets, launchFee, bindings });
    } catch (error) {
  ...
  const handleLaunch = async (event) => {
    ...
    try {
      ...
      navigate(marketHref(created.marketId));
  ...
  useEffect(() => {
    if (route.name !== "market" || foundation.status !== "ready") return;
    const marketId = route.marketId.trim().toLowerCase();
    if (marketState.status === "ready" && marketState.view.marketId === marketId) return;
    void loadMarket(marketId);
  }, [route.name, route.name === "market" ? route.marketId : "", foundation.status]);
  ...
  const hero = pageCopy(route);
  return (
    <main className="app-shell" id="main-content" tabIndex="-1">
      <a className="skip-link" href="#main-content">Skip to content</a>
      <header className="topbar">
        <a className="brand" href="/" aria-label="TickerGarden home" onClick={(event) => followSiteLink(event, "/")}><img src="/assets/tickergarden-mark.png" alt="" /><span>TickerGarden</span></a>
        <nav className="primary-nav" aria-label="Primary navigation">
          {SITE_NAVIGATION.map((item) => <a href={item.href} key={item.name} aria-current={routeIsActive(route, item.name) ? "page" : undefined} onClick={(event) => followSiteLink(event, item.href)}>{item.label}</a>)}
        </nav>
      ...
      <section className="workspace-heading" aria-labelledby="page-title">
        <div><p className="eyebrow">{hero.eyebrow}</p><h1 id="page-title">{hero.lead}<br /><em>{hero.accent}</em></h1><p className="page-description">{hero.description}</p></div>
      ...
      {route.name === "home" ? <HomePage foundation={foundation} onLink={followSiteLink} /> : null}

      {route.name === "create" ? (
        <section className="product-layout create-layout">
          <article className="panel panel-form">
            ...
            <form onSubmit={handleLaunch}>
              <div className="form-grid">
                <label htmlFor="launch-stock"><span>Official STOCK</span><select id="launch-stock" name="assetUid" value={launchForm.assetUid} onChange={(e) => setLaunchForm({ ...launchForm, assetUid: e.target.value })} disabled={!runtimeReady}>{activeAssets.map((asset) => <option value={asset.id} key={asset.id}>{shortHex(asset.id)} · {shortHex(asset.values.stockToken)}</option>)}</select></label>
                <label htmlFor="launch-quote"><span>Actual Quote</span><select id="launch-quote" name="quoteId" value={launchForm.quoteId} onChange={(e) => setLaunchForm({ ...launchForm, quoteId: e.target.value })} disabled={!runtimeReady}>{activeQuotes.map((quote) => <option value={quote.id} key={quote.id}>{quote.values.quoteAsset === ZERO_ADDRESS ? "Native ETH" : shortHex(quote.values.quoteAsset)} · {shortHex(quote.id)}</option>)}</select></label>
                <label htmlFor="launch-template"><span>Launch template</span><select id="launch-template" name="templateId" value={launchForm.templateId} onChange={(e) => setLaunchForm({ ...launchForm, templateId: e.target.value })} disabled={!runtimeReady}>{activeTemplates.map((template) => <option value={template.id} key={template.id}>{shortHex(template.id)}</option>)}</select></label>
                <label htmlFor="launch-beneficiary"><span>Creator revenue beneficiary</span><input id="launch-beneficiary" name="beneficiary" value={launchForm.beneficiary} onChange={(e) => setLaunchForm({ ...launchForm, beneficiary: e.target.value })} placeholder={wallet?.account || "0x…"} autoComplete="off" required /></label>
                <label htmlFor="launch-name"><span>Meme name</span><input id="launch-name" name="name" value={launchForm.name} maxLength={64} onChange={(e) => setLaunchForm({ ...launchForm, name: e.target.value })} placeholder="Garden Market" autoComplete="off" required /></label>
                <label htmlFor="launch-symbol"><span>Symbol</span><input id="launch-symbol" name="symbol" value={launchForm.symbol} maxLength={16} onChange={(e) => setLaunchForm({ ...launchForm, symbol: e.target.value })} placeholder="GDN" autoCapitalize="characters" autoComplete="off" required /></label>
                <label className="wide" htmlFor="launch-metadata"><span>Metadata URI</span><input id="launch-metadata" name="metadataURI" value={launchForm.metadataURI} maxLength={512} onChange={(e) => setLaunchForm({ ...launchForm, metadataURI: e.target.value })} placeholder="ipfs://…" autoComplete="off" required /></label>
                <label className="wide" htmlFor="launch-salt"><span>Identity salt</span><div className="input-action"><input id="launch-salt" name="salt" value={launchForm.salt} onChange={(e) => setLaunchForm({ ...launchForm, salt: e.target.value })} autoComplete="off" required /><button type="button" className="secondary" onClick={() => setLaunchForm({ ...launchForm, salt: randomSalt() })}>Regenerate</button></div></label>
              </div>
              ...
              {launchForm.mode === "launch-buy" ? <div className="form-grid compact"><label htmlFor="launch-quote-in"><span>First-buy Quote · raw units</span><input id="launch-quote-in" name="quoteIn" inputMode="numeric" value={launchForm.quoteIn} onChange={(e) => setLaunchForm({ ...launchForm, quoteIn: e.target.value })} placeholder="10000000000000000" required /></label><label htmlFor="launch-min-out"><span>Minimum Meme output · raw units</span><input id="launch-min-out" name="minTokensOut" inputMode="numeric" value={launchForm.minTokensOut} onChange={(e) => setLaunchForm({ ...launchForm, minTokensOut: e.target.value })} placeholder="Set from an independent quote" required /></label></div> : null}
          ...
          <aside className="launch-guide">
            <div><span>01</span><h3>Choose only registered inputs.</h3><p>Active STOCK, Quote and template options come from one finalized Read API revision.</p></div>
            <div><span>02</span><h3>Bind derived economics.</h3><p>The Factory preview—not a browser estimate—produces the exact economics hash used onchain.</p></div>
            <div><span>03</span><h3>Simulate before signing.</h3><p>Allowance and launch calls are independently simulated; wallet and snapshot changes invalidate the intent.</p></div>
            <div><span>04</span><h3>Verify the deployment.</h3><p>The receipt must emit the canonical MarketCreated event and fresh Curve code must expose the selected Quote.</p></div>
          </aside>
        </section>
      ) : null}

      {route.name === "markets" ? <MarketsPage foundation={foundation} activeAssets={activeAssets} onLink={followSiteLink} /> : null}

      {route.name === "market" ? (
        <section className="market-detail-page">
          <div className="market-jump panel">
            <label htmlFor="market-id"><span>Open canonical market ID</span><div className="input-action"><input id="market-id" name="marketId" value={marketIdInput} onChange={(e) => setMarketIdInput(e.target.value)} list="market-options" placeholder="0x…" autoComplete="off" /><button type="button" className="secondary" onClick={() => { const value = marketIdInput.trim().toLowerCase(); if (HEX32.test(value)) navigate(marketHref(value)); else void loadMarket(); }} disabled={!runtimeConfig.available || marketState.status === "loading"}>Open market</button></div></label>
          ...
          {marketState.status === "loading" ? <div className="empty-state market-loading" role="status">Reading indexed identity and checking live Registry bindings…</div> : marketState.status === "error" ? <p className="inline-error" role="alert">{marketState.error}</p> : marketState.status === "ready" ? <>
            <section className="market-overview panel" aria-labelledby="market-overview-title">
              <div className="panel-heading"><div><p className="eyebrow">Canonical identity</p><h2 id="market-overview-title">{shortHex(marketState.view.marketId, 10, 8)}</h2></div><span className={`phase phase-${marketState.view.launchPhase}`}>{launchPhases[marketState.view.launchPhase] || `Phase ${marketState.view.launchPhase}`}</span></div>
              ...
              <dl className="identity-grid"><div><dt>Meme</dt><dd>{shortHex(marketState.view.memeToken, 9, 7)}</dd></div><div><dt>Curve</dt><dd>{shortHex(marketState.view.curve, 9, 7)}</dd></div><div><dt>Gauge</dt><dd>{shortHex(marketState.response.market.gauge, 9, 7)}</dd></div><div><dt>Quote</dt><dd>{marketState.view.quoteAssetKind === "native" ? "Native ETH" : shortHex(marketState.view.quoteAsset, 9, 7)}</dd></div><div><dt>STOCK context</dt><dd>{shortHex(marketState.response.market.assetUid, 9, 7)}</dd></div><div><dt>Snapshot block</dt><dd>{marketState.response.sync.blockNumber}</dd></div></dl>
            </section>

            <section className="trade-layout">
              <article className="panel trade-ticket">
                <div className="panel-heading"><div><p className="eyebrow">Curve ticket</p><h2>Quote live reserves before signing.</h2></div><span className="pill">30s intent</span></div>
                {marketState.view.curveTradingEnabled ? <>
                  <fieldset className="trade-side"><legend className="sr-only">Trade side</legend><label className={tradeSide === "buy" ? "active" : ""}><input type="radio" name="tradeSide" value="buy" checked={tradeSide === "buy"} onChange={(e) => { setTradeSide(e.target.value); setTradeQuote(null); }} /> Buy Meme</label><label className={tradeSide === "sell" ? "active" : ""}><input type="radio" name="tradeSide" value="sell" checked={tradeSide === "sell"} onChange={(e) => { setTradeSide(e.target.value); setTradeQuote(null); }} /> Sell Meme</label></fieldset>
                  <label htmlFor="trade-amount"><span>{tradeSide === "buy" ? "Quote input" : "Meme input"} · raw units</span><input id="trade-amount" name="tradeAmount" inputMode="numeric" value={tradeAmount} onChange={(e) => { setTradeAmount(e.target.value); setTradeQuote(null); }} placeholder="Enter a positive raw-unit integer" autoComplete="off" /></label>
                  {tradeQuote ? <div className="quote-card"><span>{tradeSide === "buy" ? "Meme output" : "Quote output"}: <strong>{tradeQuote.output.toString()} raw</strong></span><span>Minimum at 1% tolerance: <strong>{tradeQuote.minimum.toString()} raw</strong></span>{tradeQuote.refund !== null ? <span>Possible refund: <strong>{tradeQuote.refund.toString()} raw Quote</strong></span> : null}{tradeQuote.fee !== null ? <span>Curve fee: <strong>{tradeQuote.fee.toString()} raw Quote</strong></span> : null}<small>Expires in {Math.max(0, Math.ceil((tradeQuote.expiresAt - now * 1000) / 1000))}s</small></div> : <div className="quote-placeholder"><span>1</span><p>Read current Curve state</p><i /><span>2</span><p>Simulate exact transaction</p><i /><span>3</span><p>Confirm in wallet</p></div>}
                  <div className="split-actions"><button type="button" className="secondary" onClick={quoteCurveTrade} disabled={actionDisabled}>Read onchain quote</button><button type="button" onClick={executeCurveTrade} disabled={actionDisabled || !tradeQuote}>{busy.startsWith("curve:") ? "Trading…" : "Simulate & trade"}</button></div>
                </> : <div className="empty-state"><strong>Curve trading has ended for this market.</strong><span>The interface will not route a graduated market back through its Curve.</span></div>}
              </article>

              <article className="panel pool-route">
                <div className="panel-heading"><div><p className="eyebrow">Graduated pool</p><h2>Canonical route, explicit gate.</h2></div><span className="pill pill-warning">Swap unavailable</span></div>
                {marketState.response.market.poolKey ? <><div className="metric-grid"><Metric label="Pool ID" value={shortHex(marketState.response.market.poolId)} /><Metric label="Router" value={shortHex(marketState.response.market.canonicalRoute.router)} /><Metric label="Quoter" value={shortHex(marketState.response.market.canonicalRoute.quoter)} /><Metric label="Hook" value={shortHex(marketState.response.market.canonicalRoute.hook)} /></div><div className="pool-key"><code>{JSON.stringify(marketState.response.market.poolKey, null, 2)}</code></div></> : <div className="empty-state"><strong>No PoolCreated route yet.</strong><span>This market remains in {launchPhases[marketState.view.launchPhase] || `phase ${marketState.view.launchPhase}`}.</span></div>}
                <p className="risk-copy">Pool facts are read-only in this release. A pool swap button will not appear until pinned v4 fork evidence, exact quoting, approval, simulation and fresh-state confirmation are implemented together.</p>
              </article>
            </section>
          </> : <div className="empty-state"><strong>Canonical market data is unavailable.</strong><span>Complete runtime configuration and a finalized synchronized snapshot are required.</span></div>}
        </section>
      ) : null}
      ...
      {route.name === "stats" ? <StatsPage foundation={foundation} /> : null}
      {route.name === "faq" ? <FaqPage /> : null}
      {route.name === "not-found" ? <NotFoundPage onLink={followSiteLink} /> : null}
      ...
      <footer className="footer"><span>Execution spec V1-EXEC-8 · readiness IMPLEMENTATION_ALLOWED</span><span>Pool swaps and production publishing remain closed until their explicit gates are satisfied.</span></footer>
    </main>
  );
}
