...
import {
  ...
  type ExecuteV2Transaction,
  type TransactionUpdate,
  type V2TransactionClients,
} from "../src/v2/transaction.ts";
...
function clients(options: {
  ...
  wait?: Promise<void>;
} = {}): { clients: V2TransactionClients; calls: string[] } {
  const calls: string[] = [];
  return {
    calls,
    clients: {
...
function input(overrides: Partial<ExecuteV2Transaction<string>> = {}): ExecuteV2Transaction<string> {
  return {
...
test("generated ABI bridge exactly matches all nineteen compiled E102 interface artifacts", async () => {
  ...
  for (const source of V2_ABI_SOURCES) {
    ...
    assert.deepEqual(v2Abis[source.module], artifact.abi);
  }
});
...
test("transaction path simulates before signature, waits for success, then reconciles fresh facts", async () => {
  ...
  const result = await new V2TransactionExecutor(harness.clients).execute(input({
    confirmations: 2,
...
test("duplicate operation keys share one in-flight wallet request", async () => {
  ...
  const executor = new V2TransactionExecutor(harness.clients);
...
test("preflight distinguishes chain, account, quote, snapshot, and Indexer failures before simulation", async () => {
  const cases: Array<[V2TransactionClients, ExecuteV2Transaction<string>, string]> = [
    ...
    [clients().clients, input({ snapshot: { ...snapshot, executionSpecId: "V2-EXEC-2" } }), "stale_snapshot"],
  ...
  for (const [clientSet, transaction, code] of cases) {
    await assert.rejects(new V2TransactionExecutor(clientSet).execute(transaction), (error: unknown) => {
      assert.ok(error instanceof V2TransactionError);
...
test("wallet or snapshot drift after simulation prevents a signature", async () => {
  const harness = clients();
  let reads = 0;
  await assert.rejects(new V2TransactionExecutor(harness.clients).execute(input({
    currentRevision: async () => ++reads === 1 ? snapshot.revision : "101:0xbb",
  })), (error: unknown) => error instanceof V2TransactionError && error.code === "stale_snapshot");
...
test("simulation, rejection, revert, cancellation, and confirmation failures remain distinct", async () => {
  const scenarios: Array<[V2TransactionClients, ExecuteV2Transaction<string>, string]> = [
