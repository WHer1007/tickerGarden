...
import { V2_EXECUTION_SPEC_ID } from "./generated/abis.ts";
...
export class V2TransactionError extends Error {
...
export interface TransactionUpdate {
  ...
  readonly error?: V2TransactionError;
}
...
export interface V2TransactionClients {
...
export interface ExecuteV2Transaction<T> {
  readonly operationKey: string;
...
export class V2TransactionExecutor {
  readonly #inflight = new Map<string, Promise<unknown>>();
  readonly clients: V2TransactionClients;

  constructor(clients: V2TransactionClients) {
    this.clients = clients;
  }

  execute<T>(input: ExecuteV2Transaction<T>): Promise<T> {
  ...
  async #execute<T>(input: ExecuteV2Transaction<T>): Promise<T> {
    ...
    try {
      ...
      try {
        ...
        throw new V2TransactionError("confirmation_failed", "transaction receipt could not be reconciled with fresh chain facts", error);
      }
    } catch (error) {
      const typed = error instanceof V2TransactionError ? error : new V2TransactionError("submission_failed", "transaction workflow failed", error);
      emit({ stage: "failed", error: typed });
      throw typed;
    }
  }

  #assertPreflight<T>(input: ExecuteV2Transaction<T>): void {
    const chainId = this.clients.walletClient.chain?.id;
    if (chainId !== ROBINHOOD_CHAIN_ID) throw new V2TransactionError("unsupported_chain", `wallet must use chain ${ROBINHOOD_CHAIN_ID}`);
    const connected = accountAddress(this.clients.walletClient.account);
    if (!connected || !sameAddress(connected, input.expectedAccount)) throw new V2TransactionError("wrong_account", "connected wallet does not match the expected account");
    if (input.snapshot.executionSpecId !== V2_EXECUTION_SPEC_ID) throw new V2TransactionError("stale_snapshot", "snapshot execution spec does not match the generated ABI");
    if (input.snapshot.syncStatus === "lagging") throw new V2TransactionError("indexer_lagging", "Indexer snapshot is lagging");
    if (input.snapshot.syncStatus === "unavailable") throw new V2TransactionError("indexer_unavailable", "Indexer snapshot is unavailable");
    if (!input.snapshot.revision) throw new V2TransactionError("stale_snapshot", "snapshot revision is required");
    if (input.quoteExpiresAtMs !== undefined && (input.now ?? Date.now)() >= input.quoteExpiresAtMs) {
      throw new V2TransactionError("stale_quote", "quote expired before simulation");
    }
  }

  async #assertFresh<T>(input: ExecuteV2Transaction<T>, phase: string): Promise<void> {
    this.#assertPreflight(input);
    if (input.currentRevision && await input.currentRevision() !== input.snapshot.revision) {
      throw new V2TransactionError("stale_snapshot", `the reconciled read snapshot changed ${phase}`);
    }
  }

  async #sendAndConfirm<T>(
    request: ContractWriteRequest,
    input: ExecuteV2Transaction<T>,
    ...
    try {
      ...
      throw new V2TransactionError("simulation_failed", approval ? "approval simulation failed" : "transaction simulation failed", error);
    }
    ...
    try {
      ...
      if (isUserRejected(error)) throw new V2TransactionError("user_rejected", "wallet signature was rejected", error);
      throw new V2TransactionError("submission_failed", approval ? "approval submission failed" : "transaction submission failed", error);
    }
    ...
    try {
      ...
      if (isReceiptTimeout(error)) throw new V2TransactionError("receipt_timeout", "timed out waiting for a canonical receipt", error);
      throw new V2TransactionError("submission_failed", "receipt wait failed", error);
    }
    if (cancelled) throw new V2TransactionError("replacement_cancelled", "transaction was replaced by a cancellation");
    if (receipt.status !== "success") {
      throw new V2TransactionError(approval ? "approval_reverted" : "transaction_reverted", approval ? "approval reverted" : "transaction reverted");
    }
