...
from generate_v1_canonical_abi import (
...
CANONICAL_PATH = ROOT / "v1_canonical_abi.json"
ARTIFACT_ROOT = PROJECT_ROOT / "contracts" / "out-v1" / "IV1Protocol.sol"
OUTPUT_PATH = ROOT / "v1_compiled_interface_manifest.json"
...
def build():
    ...
    for module in surface["modules"]:
        ...
        current_source_hash = compiled_artifact["metadata"]["sources"][
            "src/v1/interfaces/IV1Protocol.sol"
        ]["keccak256"]
        ...
        modules.append(
            {
                "target": module_name,
                "artifact": f"contracts/out-v1/IV1Protocol.sol/I{module_name}.json",
    ...
    compiled_errors = {
        ...
        for item in artifact("IV1Errors")["abi"]
        if item["type"] == "error"
    }
    expected_errors = {
