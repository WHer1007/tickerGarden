"""Pure-integer reference model for frozen TickerGarden V2 arithmetic.
...
COMPONENT_SALT_DOMAIN_LABEL = "TICKERGARDEN_V2_COMPONENT_SALT"
QUOTE_ECONOMICS_DOMAIN_LABEL = "TICKERGARDEN_V2_QUOTE_ECONOMICS"
UINT256_MAX = 2**256 - 1
UINT192_MAX = 2**192 - 1
UINT160_MAX = 2**160 - 1
UINT128_MAX = 2**128 - 1
...
def quote_economics_hash(
    ...
    """Content-address one immutable V2 Quote configuration."""
...
def validate_minimum_allocation(stock_decimals: int, minimum_allocation: int) -> int:
    """Validate one V2-EXEC-5 per-asset minimum in canonical raw units."""
...
def accumulator_lifetime_bound() -> AccumulatorLifetimeBound:
    """Prove the base fee-credit index bound for every V2-EXEC-5 asset."""
...
def partition_pool_fee(base: int, active_stock: int) -> PoolFeePartition:
    """Partition a V2-EXEC-5 pool fee with a fixed active-staker share."""
