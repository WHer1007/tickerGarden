...
class CurrentContractSurfaceTest(unittest.TestCase):
    ...
    def test_current_compiled_abis_have_only_current_claim_and_conversion_surface(self):
        ...
        self.assertIn("claimSnapshot(bytes32,uint64,uint256,uint256,uint8,bytes32[])", holder)
        self.assertNotIn("checkpointTransfer(bytes32,address,address,uint256)", holder)
        self.assertNotIn("enableContinuousRewards()", token)
