# TickerGarden V2 Readiness 与部署门禁
...
状态是某个不可变 `executionSpecId` 的发布证书，只允许逐级前进。若已签字证据失效，不把旧证书“降级后继续使用”；应立即暂停相应部署/市场、撤销发布资格并创建新的 execution spec/release 记录重新走门禁。
...
零值例外必须同时由 schema 路径和业务语义限定。当前只预留 native Quote 的 `address(0)`、市场初始 `poolId/sweptAt/recoveryEpoch` 和初始 binding status；不能因为“某字段可能为零”而全局放行零地址或零 hash。
...
- `assertV2Deployable` 只接受 `DEPLOYMENT_ELIGIBLE/PRODUCTION_READY`；生产发布还必须单独通过 `assertV2ProductionReady`。
