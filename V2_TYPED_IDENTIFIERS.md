# TickerGarden V2 Typed ID、Hash 与发现接口
...
launchTemplateHash = keccak256(abi.encode(
  keccak256(bytes("TICKERGARDEN_V2_LAUNCH_TEMPLATE")), 1,
...
quoteEconomicsHash = keccak256(abi.encode(
  keccak256(bytes("TICKERGARDEN_V2_QUOTE_ECONOMICS")), 1,
  chainId,
  ponsBaselineId,
  quoteAsset,
  quoteDecimals,
...
`status`、symbol/name、代理 implementation 和观测区块不进入 economics hash：前者是可变准入状态，后三者属于部署证据。Registry 必须校验 ID 与重算结果一致，不能接受调用者提供的任意别名；更新任一 economics 字段都必须新增 config。精确首发值见 [`spec/v2_initial_quote_configs.json`](./spec/v2_initial_quote_configs.json)，schema 与固定向量见 [`spec/v2_hash_schemas.json`](./spec/v2_hash_schemas.json)。
...
ponsBaselineHash = keccak256(abi.encode(
  keccak256(bytes("TICKERGARDEN_V2_PONS_BASELINE")), 1,
...
componentSalt = keccak256(abi.encode(
  keccak256(bytes("TICKERGARDEN_V2_COMPONENT_SALT")), 1,
...
预测与实际部署必须调用同一内部库，并逐地址断言相等。Token/Curve/Gauge 在创建交易部署；Locker 只在毕业子调用中由 GraduationExecutor 部署，但地址在创建前即可预测。地址已有代码、init code hash 不匹配或初始化失败时整笔回滚，不允许自动更换 nonce。Hook 是 template 级共享地址，单独执行 Uniswap v4 权限位 salt mining。公式、部署顺序和固定向量见 [V2_PONS_BEHAVIOR_BASELINE.md](./V2_PONS_BEHAVIOR_BASELINE.md) 与 `spec/v2_pons_behavior_vectors.json`。

`V2Identifiers` 与 `V2Create2` 已在 `contracts/src/v2/shared/` 实现上述 marketId、component salt、EIP-1014 预测及零值 CREATE2 部署路径。当前通过的是冻结算法向量与本地真实部署一致性；各产品组件的最终 constructor args、initCodeHash、部署者地址和 production manifest 仍须由 `V2-C-105-B/C`、Graduation 与 deployment evidence 闭合。
...
具体字段顺序与测试向量以 `spec/v2_hash_schemas.json` 为机器权威。改变 domain、类型、顺序或字段必须升级 schemaVersion 与 executionSpecId，旧 hash 永不复用。

## 7. Canonical discovery

`MarketRegistryV2` 必须直接提供：
