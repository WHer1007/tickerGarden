...
- optimizer 开启，`optimizer_runs = 200`，使用 `FOUNDRY_PROFILE=v2`；
- 数值是测试内围绕目标外部调用的 `gasleft()` 差值，不包含 fixture 部署、注入和断言；
- 本报告用于证明算法复杂度与建立回归阈值。目标链最终入口预算仍属于 `V2-T-702-A`，不得把本地数值直接当成生产上限。
...
```bash
cd contracts
FOUNDRY_PROFILE=v2 forge test --match-path test/v2/shared/MemeStockGaugeActivationWheel.t.sol -vv
FOUNDRY_PROFILE=v2 forge test --match-path test/v2/shared/MemeStockGaugeActivationSnapshots.t.sol -vv
FOUNDRY_PROFILE=v2 forge test --match-path test/v2/shared/MemeStockGaugePendingPositions.t.sol
```
