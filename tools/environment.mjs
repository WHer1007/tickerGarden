...
export function serviceEnvironment(values,service,inherited=process.env){
 ...
 const serviceKeys={
  'read-api':/^(TG_RPC_CONTROL_|TG_RPC_BUDGET|TG_READ_|ZEROX_API_KEY$|TG_CURSOR_SECRET$|TG_ALLOWED_ORIGINS$|TG_ENVIRONMENT$|TG_ENV$|TG_CHAIN_ID$|TG_DATABASE_SCHEMA$|TG_DB_|TG_RPC_URL$|TG_RPC_VERIFICATION_MODE$|TG_SECONDARY_RPC_URL$)/,
...
export function checkSource(values,directory=root){
 ...
 if(values.TG_PROFILE==='test'&&values.TG_RUNTIME_CONFIGURED==='true'){
  if(!values.TG_DEPLOYMENT_BOOTSTRAP_FILE)throw Error('Configured test environment requires a server-side deployment bootstrap');
  const file=path.isAbsolute(values.TG_DEPLOYMENT_BOOTSTRAP_FILE)?values.TG_DEPLOYMENT_BOOTSTRAP_FILE:path.join(directory,values.TG_DEPLOYMENT_BOOTSTRAP_FILE);
