...
const dir='outputs/reviews/continuous-v4-public-2026-09-07';const s=JSON.parse(fs.readFileSync(dir+'/results.json')),p=JSON.parse(fs.readFileSync('deployments/releases/'+candidate.releaseId+'/arbitrum-sepolia-421614.v1.deployed.json')),roles=JSON.parse(fs.readFileSync(dir+'/roles.json')).roles;
 ...
 for(const e of events){
  if(e.eventName==='CurveBuy'||e.eventName==='CurveSell'){
   ...
   let anti=0n;if(e.eventName==='CurveBuy'&&a.recipient.toLowerCase()!==roles.creator.toLowerCase()){
    const elapsed=header.timestamp-launchTimestamps.get(m.id);const raw=elapsed>=5n?0n:[9900n,2475n,309n,19n,1n][Number(elapsed)];
   ...
   const supply=1000000000n*10n**18n,phantom=m.quote===zero?168000000000000000n:40n*10n**18n,threshold=m.quote===zero?420000000000000000n:100n*10n**18n;
...
for(const m of markets){const view=await c.readContract({address:p.ordinaryComponents[10],abi:abi('MarketRegistryV1'),functionName:'market',args:[m.id],blockNumber:block.number});
 ...
 else {assert.equal(actual,0n,'Graduated curve reserve not emptied');const threshold=m.quote===zero?420000000000000000n:100000000000000000000n;const supply=1000000000n*10n**18n,phantom=m.quote===zero?168000000000000000n:40n*10n**18n,reserved=supply*phantom/(phantom+threshold),minimum=((supply-reserved)*phantom+reserved-1n)/reserved;assert.equal(curveTokens.get(m.id),reserved,'Graduation sells exactly the curve inventory');assert.ok(expected>=minimum,'Canonical graduation minimum');}
 curveReserveRows.push({marketId:m.id,key:m.key,netQuoteFromTrades:expected,actualCurveReserve:actual,phase:view.runtime.launchPhase,nominalThresholdRoundingExcess:view.runtime.launchPhase===1?expected-(m.quote===zero?420000000000000000n:100n*10n**18n):null});}
