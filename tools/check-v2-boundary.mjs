...
const requiredPaths = [
  "contracts/src/v2/shared/V2Scaffold.sol",
  "contracts/test/v2/V2Scaffold.t.sol",
...
const forbiddenProductionImports = ["spec/interfaces/IV2MutationSurfaceDraft.sol"];

const sourceExtensions = new Set([".sol", ".ts", ".tsx", ".js", ".jsx"]);
const ignoredDirectories = new Set(["node_modules", "dist", "out", "out-v2", "cache", "cache-v2"]);
...
for (const retiredPath of retiredPaths) {
  if (await exists(retiredPath)) failures.push(`retired V1 path is present: ${retiredPath}`);
}
...
for (const sourceRoot of sourceRoots) {
  ...
  for (const sourceFile of await collectSourceFiles(sourceRoot)) {
    ...
    if (sourceFile.startsWith("contracts/src/v2/")) {
    ...
    for (const symbol of forbiddenSymbols) {
      if (contents.includes(symbol)) failures.push(`${sourceFile}: forbidden V1 symbol \`${symbol}\``);
    }
  }
}

if (failures.length > 0) {
  process.stderr.write(`V2 boundary check failed:\n- ${failures.join("\n- ")}\n`);
  process.exitCode = 1;
} else {
  process.stdout.write("V2 scaffold boundary verified: required paths exist and retired V1 logic is absent.\n");
}
