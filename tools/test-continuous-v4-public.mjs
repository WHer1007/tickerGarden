...
const out = 'outputs/reviews/continuous-v4-public-2026-09-07';
...
const state=fs.existsSync(statePath)?JSON.parse(fs.readFileSync(statePath)):{runId:'CONTINUOUS-V4-PUBLIC-2026-09-07',chainId:421614,releaseId:p.releaseId,transactions:[],checks:[],markets:{}};
...
async function matrix(){
 ...
 for(const native of [false])for(const staking of [false,true])for(const sharing of [true])for(const tax of [500]){
...
async function graduated(){
 check(Object.keys(state.markets).length===2,'Incomplete creation matrix');
 ...
 // ERC20-only changed-path matrix; no native graduation capital transfer.
 for(const m of Object.values(state.markets).filter(m=>m.quote!==zero||m.key==='ETH-S1-H1-T500')){
  ...
  if(m.params.creatorFeesToHolders){
   ...
   m.releaseState=await read('HolderRewardsDistributorV1',distributor,'releaseState',[m.id]);m.holderFunded=String((await read('HolderRewardsDistributorV1',distributor,'marketState',[m.id])).funded);check(BigInt(m.holderFunded)>0n,'No funded holder quote');
  }
...
async function handover(){
 const m=state.markets['ERC20-S0-H1-T500'];check(done('graduated-'+m.key),'Graduate first');
 if(done('creator-handover-public'))return;
 const amount=await snapshot('handover-sell-amount',async()=>({amount:String((await balance(m.token,accounts.buyer.address))/2000n)}));
 await swap('handover-old-sell',m,false,BigInt(amount.amount));
 await strictReject('handover-outsider-propose','outsider','CreatorRevenueRegistry',creators,'transferCreatorRevenueBeneficiary',[m.id,accounts.buyer.address],'UnauthorizedCurrentBeneficiary');
 await call('handover-propose-cancel','creator','CreatorRevenueRegistry',creators,'transferCreatorRevenueBeneficiary',[m.id,accounts.buyer.address]);
 if(!state.transactions.some(t=>t.id==='handover-accept'))check(await read('CreatorRevenueRegistry',creators,'currentCreatorEpoch',[m.id])===1,'Proposal changed epoch');
 await call('handover-cancel','creator','CreatorRevenueRegistry',creators,'cancelCreatorRevenueBeneficiaryTransfer',[m.id]);
 await call('handover-propose','creator','CreatorRevenueRegistry',creators,'transferCreatorRevenueBeneficiary',[m.id,accounts.buyer.address]);
 await strictReject('handover-outsider-accept','outsider','CreatorRevenueRegistry',creators,'acceptCreatorRevenueBeneficiary',[m.id],'UnauthorizedPendingBeneficiary');
 await call('handover-accept','buyer','CreatorRevenueRegistry',creators,'acceptCreatorRevenueBeneficiary',[m.id]);
 check(await read('CreatorRevenueRegistry',creators,'currentCreatorEpoch',[m.id])===2,'Acceptance missing epoch 2');
 state.beneficiaryTransfers??={};state.beneficiaryTransfers[m.id]={1:accounts.creator.address,2:accounts.buyer.address};save();
 await swap('handover-new-sell',m,false,BigInt(amount.amount));
 const before=await snapshot('handover-claims',async()=>({old:String(await read('ProtocolFeeVault',fees,'creatorLiability',[m.id,1,m.quote])),next:String(await read('ProtocolFeeVault',fees,'creatorLiability',[m.id,2,m.quote])),oldBalance:String(await balance(m.quote,accounts.creator.address)),nextBalance:String(await balance(m.quote,accounts.buyer.address))}));
 check(BigInt(before.old)>0n&&BigInt(before.next)>0n,'Epoch fee missing');
 await call('handover-old-claim','outsider','ProtocolFeeVault',fees,'claimCreator',[m.id,1,m.quote]);
 await call('handover-new-claim','outsider','ProtocolFeeVault',fees,'claimCreator',[m.id,2,m.quote]);
 check(await balance(m.quote,accounts.creator.address)===BigInt(before.oldBalance)+BigInt(before.old),'Old beneficiary payout');
 check(await balance(m.quote,accounts.buyer.address)===BigInt(before.nextBalance)+BigInt(before.next),'New beneficiary payout');
 record('creator-handover-public',{marketId:m.id,oldQuote:before.old,newQuote:before.next,proposalCancelAccept:true,recipientDeltasVerified:true});
}
check(p.contracts.some(x=>x.name==='HolderRewardsDistributorV1'),'Wrong holder mode');
if(!['matrix','graduated','handover'].includes(stage))throw Error('Expected matrix or graduated stage');
for(const role of ['creator','buyer','staker','outsider']){
 const f=await snapshot('gas-funding-'+role,async()=>({amount:String((await c.getBalance({address:accounts[role].address}))<parseEther('0.006')?parseEther('0.012'):0n)}));
 if(BigInt(f.amount)>0n)await send('gas-funding-'+role,'admin',accounts[role].address,'0x',BigInt(f.amount));
}
if(stage==='matrix')await matrix();else if(stage==='graduated')await graduated();else await handover();
console.log(JSON.stringify({status:state.status,transactions:state.transactions.length,checks:state.checks.length}));
