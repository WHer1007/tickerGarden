...
const plan=JSON.parse(fs.readFileSync('deployments/manifests/arbitrum-sepolia-421614.v1.preview.json'));
...
const txs=JSON.parse(fs.readFileSync('deployments/manifests/arbitrum-sepolia-421614.v1.transactions.json'));
if(!txs.length||txs.some(t=>t.status!=='CONFIRMED'))throw Error('Unconfirmed transactions');
for(const t of txs){const r=await client.getTransactionReceipt({hash:t.transactionHash});if(r.status!=='success')throw Error('Receipt failed');}
let activation={quote:false,baseline:false,template:false};
const activationPath='deployments/manifests/arbitrum-sepolia-421614.v1.activation.json';
...
fs.writeFileSync('deployments/manifests/arbitrum-sepolia-421614.v1.deployed.json',JSON.stringify(report,null,2)+'\n');
