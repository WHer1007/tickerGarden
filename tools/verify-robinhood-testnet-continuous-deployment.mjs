...
const receiverArtifact=artifact('RobinhoodTestnetTreasury');
...
fs.writeFileSync(statusPath,JSON.stringify({...prior,schemaVersion:1,status:report.status,chainId:46630,productionTargetChainId:4663,releaseId:plan.releaseId,deployer:plan.deployer,platformTreasury:treasury.address,settlementOperator,orchestrator:plan.orchestrator,factory:plan.factory,hook:plan.hook,graduationExecutor:plan.executor,payloadHash:plan.payloadHash,components:Object.fromEntries(entries.map(x=>[x.name,x.address])),transactions:{...prior.transactions,core:txs.map(x=>x.hash)},publicTestnetE2E:false},null,2)+'\n');
