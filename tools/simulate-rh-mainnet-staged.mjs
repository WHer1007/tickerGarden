...
const base='docs/reviews/evidence/rh-mainnet-preparation-2026-09-13/';
...
// These router fields are explicit simulation candidates, not a claim of all-Stock pool coverage.
const configuration={chainId:4663,addresses:m.addresses,dependencies:deps.dependencies,nativeQuotePoolFee:10000,nativeQuoteTickSpacing:200,holderMode:'dual-asset-24h-v4',feePolicyId:hash('TICKERGARDEN_V1_FEE_POLICY_40_30_30'),routeCoverageVerified:false};
const releaseId=hash(JSON.stringify({domain:'TICKERGARDEN_RH_MAINNET_STAGED_PREPARATION_2026_09_13',configuration}));
const env={...process.env,V1_EXPECTED_CHAIN_ID:'4663',V1_EXPECTED_DEPLOYER:m.addresses.deployer,V1_INITIAL_ADMIN:m.addresses.initialAdmin,V1_PLATFORM_TREASURY:m.addresses.platformTreasury,V1_PLATFORM_TREASURY_CODEHASH:accounts.accounts.treasury.codeHash,V1_NATIVE_QUOTE_POOL_FEE:String(configuration.nativeQuotePoolFee),V1_NATIVE_QUOTE_TICK_SPACING:String(configuration.nativeQuoteTickSpacing),V1_FEE_POLICY_ID:configuration.feePolicyId,V1_RELEASE_ID:releaseId,V1_DEPLOYMENT_HOLDER_MODE:configuration.holderMode};
for(const [key,{address,codeHash}]of Object.entries(deps.dependencies)){env['V1_'+key]=address;env['V1_'+key+'_CODEHASH']=codeHash;}
fs.writeFileSync(base+'simulation-inputs.json',JSON.stringify({status:'SIMULATION_ONLY_NOT_SIGNABLE',releaseId,configuration,pin:m.forkPin},null,2)+'\n');
