...
data=pathlib.Path(tempfile.mkdtemp(prefix='tg-live-r6-',dir='/tmp'))
...
r=subprocess.run([str(root/'.codex_tmp/r6-fast-test/services/backend-go/bin/migrate'),'up'],env=dict(os.environ,TG_MIGRATION_DATABASE_URL=url),check=True,capture_output=True,text=True)
