...
const server=http.createServer(async(req,res)=>{const origin=req.headers.origin;if(origin==='http://127.0.0.1:5178'){res.setHeader('Access-Control-Allow-Origin',origin);res.setHeader('Vary','Origin');res.setHeader('Access-Control-Allow-Methods','POST, OPTIONS');res.setHeader('Access-Control-Allow-Headers','Content-Type');}if(req.method==='OPTIONS'){res.writeHead(origin==='http://127.0.0.1:5178'?204:403);res.end();return;}try{if(fs.existsSync(cfg.cacheDir+'/FAIL_RPC'))throw Error('controlled test outage');let body='';for await(const part of req){body+=part;if(body.length>1048576)throw Error('request too large');}const input=JSON.parse(body);const calls=Array.isArray(input)?input:[input];if(calls.length===0||calls.length>256)throw Error('invalid batch size');const answers=await Promise.all(calls.map(async c=>{const result=await limited(c,req.url==='/independent');return {jsonrpc:'2.0',id:c.id,result}}));res.setHeader('content-type','application/json');res.end(JSON.stringify(Array.isArray(input)?answers:answers[0]));}catch(e){res.writeHead(502);res.end('read observation failed');}});
