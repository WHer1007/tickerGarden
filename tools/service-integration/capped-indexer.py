...
root=pathlib.Path(__file__).resolve().parents[2];out=root/'outputs/reviews/formal-clock-service-integration-2026-09-06/service-live';url=json.loads((out/'database.json').read_text())['url'];child=None
failures=0
try:
 while True:
  ...
  child=subprocess.Popen(['python3',str(root/'tools/service-integration/run-service.py'),'indexer','--run'],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
