...
function failed(track, reason) {
  throw new Error(`V2 ${track} track FAILED: ${reason}`);
}
...
export function classifyForkTrack(inventory) {
  ...
  if (!fixtureManifest) {
    failed("fork", "contracts/test/v2/fixtures/v2-fork-fixtures.json is required");
  }
  if (!fixtureValidator) failed("fork", "spec/generate_v2_test_fixtures.py is required");
...
function runFixtureValidator() {
  const result = spawnSync("python3", ["spec/generate_v2_test_fixtures.py", "--check"], {
    cwd: repositoryRoot,
    stdio: "inherit",
  });
...
function runProductArtifactValidator() {
  const result = spawnSync("python3", ["spec/generate_v2_product_artifacts.py", "--check"], {
    cwd: repositoryRoot,
    stdio: "inherit",
  });
...
async function productStatus() {
  const sources = await filesBelow("contracts/src/v2/modules", (file) => file.endsWith(".sol"));
  const tests = await filesBelow("contracts/test/v2/product", (file) => file.endsWith(".t.sol"));
  ...
  if (status.state === "ACTIVE") {
    runForgeTests("product", "test/v2/product/**/*.t.sol");
...
async function forkStatus() {
  const tests = await filesBelow("contracts/test/v2/fork", (file) => file.endsWith(".t.sol"));
  const fixtureTests = await filesBelow(
    "contracts/test/v2/fixtures",
  ...
  const fixtureManifest = fixtures.includes(
    "contracts/test/v2/fixtures/v2-fork-fixtures.json",
  );
  ...
  const status = classifyForkTrack({
    ...
    fixtureManifest,
    fixtureValidator: existsSync(path.join(repositoryRoot, "spec/generate_v2_test_fixtures.py")),
  ...
  if (status.state === "FIXTURES_ACTIVE" || status.state === "ACTIVE") {
    runForgeTests("fork", "test/v2/fixtures/**/*.t.sol");
    runFixtureValidator();
  }
  if (status.state === "ACTIVE") runForgeTests("fork", "test/v2/fork/**/*.t.sol", rpcUrl);
  return status;
}
...
async function deploymentStatus() {
  ...
  const status = classifyDeploymentTrack({
    ...
    preflightSources: existsSync(path.join(repositoryRoot, "deployments/src/v2/preflight.ts")) ? 1 : 0,
...
function publish(statuses) {
  const lines = statuses.map(
    ({ track, state, detail }) => `V2 ${track} track: ${state} (${detail})`,
  );
  process.stdout.write(`${lines.join("\n")}\n`);
  if (process.env.GITHUB_STEP_SUMMARY) {
    appendFileSync(
      process.env.GITHUB_STEP_SUMMARY,
      `## V2 implementation tracks\n\n${statuses
