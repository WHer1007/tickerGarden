...
if(!p.contracts.some(c=>c.name==='HolderRewardsDistributorV1'))throw Error('Not continuous release');
...
async function send(id,to,data){
 ...
 if(entry){if(entry.to!==to||entry.inputHash!==keccak256(data))throw Error('Activation payload changed '+id);}
  ...
  if(limit*fees.maxFeePerGas>5000000000000000n)throw Error('Activation per-transaction test budget exceeded');
...
// Keep the active legacy manifest untouched.
p.status='DEPLOYED_VERIFIED_ACTIVE_TEST_ONLY';p.activation={quote:true,baseline:true,template:true};p.settlementOperator=account.address;p.operatorConfigured=true;p.activationManifest='activation.json';p.balanceWei=String(await c.getBalance({address:account.address}));
fs.writeFileSync(isolated+'/arbitrum-sepolia-421614.v1.deployed.json',JSON.stringify(p,null,2)+'\n');
