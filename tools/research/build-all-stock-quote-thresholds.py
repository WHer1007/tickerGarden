...
REPORT=ROOT/'docs/references/RH_ALL_STOCK_QUOTE_THRESHOLDS_2026-09-12.md'
...
def build():
    ...
    rows=[]
    selected=[dict(assetKind='OFFICIAL_STOCK',tokenAddress=next(d['contractAddress'] for d in a['deployments'] if d['chainId']==4663).lower(),symbol=a['tokenSymbol'],assetUid=a['id'],decimals=a['tokenDecimals']) for a in assets if any(d['chainId']==4663 for d in a['deployments'])]
    ...
    assert len(rows)==194 and len({r['tokenAddress'] for r in rows})==194
    ...
    lines+=['','完整原始价格、乘数、地址、时间和 raw 数量见 [计算 JSON](./data/rh-all-stock-quote-thresholds-2026-09-12/thresholds.json)。来源：[官方价格 API 口径](https://docs.robinhood.com/chain/stock-token-apis/)、[官方资产目录](https://api.robinhood.com/rhj/assets)。','', '复现：`python3 tools/research/build-all-stock-quote-thresholds.py --check`，随后 `python3 tools/generate-v1-paired-assets.py --check`。两者均不请求网络。','']
    return result,'\n'.join(lines)
...
if __name__=='__main__':
    ...
    print('Verified 194 Stock thresholds, two-decimal rounding and exact 40% reserves')
