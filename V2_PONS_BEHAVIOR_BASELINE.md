...
> 修订说明：Pons链上行为证据仍保留原始版本标识；TickerGarden自身的质押与费用差异已按当前`V2-EXEC-5`修订，不再使用旧版10 STOCK线性规则。
...
TickerGarden V2 的发行主链路以 Pons V2 当前活跃部署的已确认链上行为为参考。产品层的主要差异只有两类：
...
6. V2 首发冻结两项 ACTIVE 配置：`NATIVE_ETH_V1` 与 `USDG_V1`。配置内容、`quoteAssetConfigId == economicsHash` 规则和链上快照见 [`spec/v2_initial_quote_configs.json`](./spec/v2_initial_quote_configs.json)。USDG 为可升级代理，部署前必须重新取得代理/实现地址、runtime bytes/hash 与实现变更状态；指纹漂移时暂停新市场并重新审查。后续 Quote 只能 append-only 新建配置，不修改历史市场。

首发配置的冻结值为：

| 配置 | Quote 地址 | decimals | phantomQuote | graduationThreshold | configId / economicsHash |
|---|---|---:|---:|---:|---|
| `NATIVE_ETH_V1` | `0x0000000000000000000000000000000000000000` | 18 | `1680000000000000000` | `4200000000000000000` | `0xb5a193a9938ac71aa0e78e36c38ea3ad011d894b671a25baaa5ef746f29f8472` |
...
Quote economics hash 使用 domain `TICKERGARDEN_V2_QUOTE_ECONOMICS`、schema version `1` 和 `keccak256(abi.encode(domain,schemaVersion,chainId,ponsBaselineId,quoteAsset,quoteDecimals,phantomQuote,graduationThreshold))`。USDG 的 canonical 来源、代理/实现 runtime 指纹和 drift 处理规则以首发配置机器文件为准。
...
`TickerMemeTokenV2` 为 18 decimals 的普通固定供应 ERC-20。标准
...
首发 ABI 只提供单市场入口；不提供 batch create、batch buy、batch graduate 或批量 staking/claim。单市场入口为永久兼容面。批量操作明确不属于 V2 initial release，避免把未冻结的数组失败语义、gas 上限和部分成功语义带入协议。

### 8.3 数值域与精度边界

通用数值域见 [`spec/v2_numeric_bounds.json`](./spec/v2_numeric_bounds.json)。首发实现限定资产 decimals 为 `6..18`；供应、reward/accounting 及 v4 returned delta 均须在 Solidity 对应有符号/无符号边界内。所有乘除、部分成交比较、毕业分区、手续费、V4 价格/流动性和质押累计器使用 full-precision mulDiv；每一步的 floor/ceil 与 residual 归属按机器文件冻结。激活延迟为30秒、最短锁定为86400秒，所有时间加法必须先检查 `uint64` 溢出，失败不得改写状态。
...
```text
COMPONENT_SALT_DOMAIN = keccak256("TICKERGARDEN_V2_COMPONENT_SALT")
...
- Hook、GraduationExecutor、FeeVault、MarketController 与 PoolManager 为 execution-spec 级共享不可变部署；Token/Curve/Gauge 是创建时的 per-market CREATE2 instance，LaunchLocker 是毕业时的 per-market CREATE2 instance。Hook 单独执行权限位 salt mining，并在 manifest 中冻结地址、salt、initCodeHash 和 runtime codehash。

固定 CREATE2 示例向量位于 [`spec/v2_pons_behavior_vectors.json`](./spec/v2_pons_behavior_vectors.json)。
...
本基线随当前`V2-EXEC-5`保持`IMPLEMENTATION_ALLOWED`。反狙击runtime、首发native/USDG Quote配置、通用数值域、毕业成功/失败语义、当前观测194项官方STOCK全量可选为质押Base、动态最低仓位，以及`S=0/S>0`固定质押者份额均已冻结。最终artifact/fork、目标链身份取证、安全审计、许可与法律仍约束部署和生产上线。
