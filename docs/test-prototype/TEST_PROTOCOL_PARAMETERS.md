# TickerGarden Test Prototype 协议参数

> 历史归档：本文已重新归类为 Test Prototype 参数，不是当前 canonical V1 参数。

> 文档状态：Test Prototype 机制草案
> 更新时间：2026-09-02
> 品牌名称：TickerGarden
> 品牌口号：Stake the ticker. Grow the culture.
> 本次修订：同步最新 Test Prototype 产品确认及 P-002/P-005 评审稿——供应上限三档、48 个月期末永久取消未释放额度、10,000 美元 Stock Token 软门槛、LP 延后 7 天启动、首笔至少实际到账 500 USDC、NVDA 首发、网站直连边界，以及权限/退出矩阵。
> 技术实现基线：参见 [TEST_TECHNICAL_ARCHITECTURE.md](./TEST_TECHNICAL_ARCHITECTURE.md)。
...
Test Prototype 首发 Stock Token 已确定为 NVDA。以下以首发市场为例；`GPU` 只是初始 Symbol 示例，最终首发 Symbol 仍须写入 launch manifest：
...
5. 在 Test Prototype 的 Robinhood Chain 部署中，同一个 Asset UID 只能创建一次官方 Ticker Meme 市场。
...
17. Test Prototype 只支持 Robinhood Chain；Stock Token 质押、mStock 发行与转账、官方池、LP 挖矿、手续费执行和国库全部位于同一条链。
18. Test Prototype 不部署 BSC 合约、跨链桥、wrapped mStock、跨链消息、跨链奖励会计或双链流动性池。
19. Robinhood Chain 是 mStock 的 canonical 发行链。任何 V1 多链扩展不得改变已经发行的 mStock 经济含义或重新打开已消耗的排放额度。
20. TGARD 回购销毁是 Test Prototype 必备功能，不延后到 Test Prototype.1 或 V1。
...
27. Test Prototype 只奖励 canonical PositionManager、canonical PoolId 且精确匹配官方全区间的 LP NFT；有效份额按该仓位的原始 `liquidity` 与有效订阅时间计算，不使用美元价值、预言机、交易量或 Vault NAV。
...
36. Test Prototype 首发 Stock Token 为 Registry 认证的 NVDA；正式部署前仍须完成 canonical 地址、Asset UID、代码、multiplier、Feed 和公司行动状态的 manifest 校验。
...
Test Prototype 的协议状态只有一个权威来源：Robinhood Chain。前端和索引器可以缓存或聚合链上事件，但不得成为余额、奖励、市场状态或供应上限的最终依据。

Test Prototype 的链边界如下：

| 能力 | Test Prototype 规则 |
|---|---|
...
| BSC、跨链桥和 wrapped mStock | Test Prototype 不支持 |

前端在 Test Prototype 只能发起 Robinhood Chain 交易。用户连接其他网络时，应提示切换到 Robinhood Chain，而不是展示尚未支持的跨链入口。

V1 可以重新评估 BSC 扩展。当前候选方向为：Robinhood Chain 保持 canonical 发行链，通过 Chainlink CCIP CCT 的 lock/mint、burn/unlock 模型在 BSC 创建 1:1 映射资产；BSC 初始阶段不参与 Stock Token 质押或 mStock 排放。该方向不是 Test Prototype 的部署承诺，也不授权在 Test Prototype 合约中预置桥接管理员、铸币角色或可用的跨链入口。V1 必须另行完成威胁建模、权限设计、速率限制、异常恢复、测试和专项审计。
...
Factory 必须使用枚举或等价常量映射档位，不能接受任意 `MaximumSupply`。供应档位一经市场创建即永久冻结。Ticker Meme 固定使用 18 decimals；`TotalMiningBudget = MaximumSupply`，Test Prototype 不存在预挖、团队份额、空投或额外创世铸造。

`MintedSupply` 表示该市场历史实际铸造量，必须单调递增。另以 `PoolReleased` 记录各 Gauge 在截止时间前已经分配给用户、但可能尚未领取铸造的额度。必须始终满足 `PoolMinted <= PoolReleased <= PoolBudget`。mStock 的普通转账、进入合约托管、未来可能发生的跨链锁定或销毁，都不得降低 `MintedSupply`，也不得重新释放已经消耗的铸币额度。Test Prototype 的供应上限检查不得仅依赖当前 `totalSupply()`。

mStock 不一次性释放。两个 Gauge 都使用 `block.timestamp` 和整数累计函数，不按区块、不使用浮点数，也不逐秒循环。完整公式、舍入和状态机以 [Test Prototype 数学与状态机参考规格](./TEST_MATH_AND_STATE_MACHINES.md) 为准。
...
Test Prototype 最终采用 `CanonicalLPNFTGauge`，由每个 mStock 市场独立部署并实现 Uniswap v4 `ISubscriber`。本方案不托管 LP NFT、不发行 tgLP，也不实现标准区间 Vault、自动再平衡、NAV 或 ERC-4626 份额会计。
...
Robinhood 官方提供测试网 Chain ID 46630，但该网络截至核验日未出现在 Uniswap 官方 v4 测试网部署清单。主网地址不得复制到测试网。canonical 集成测试优先使用固定区块的 RH 主网 Fork；如 Test Prototype 需要 RH 公共测试网端到端演练，则部署并锁定仅供测试的 v4 core/periphery 版本及独立地址清单，或在 Uniswap 后续发布 RH 测试网官方地址后切换并重新验证。
...
Test Prototype 的结算资产已经固定为 Robinhood Chain canonical bridge 映射的 USDC，不再保留可切换的通用稳定币参数。主网绑定：
...
> **G0-07 实现口径待签字：** P-003 已证明，如果 Hook 先按 gross input 收取严格 0.40%，v4 原生 0.60% LP fee 只能对剩余 99.60% 输入收费，组合费率为 0.9976% 而非严格 1%。本章的 1% 与 60%/40% 表述仍代表产品目标，不代表生产整数公式已经批准。推荐将协议费改为补足总费到 1% 的 remainder（约 gross 0.402414%）；详见 [`TEST_V4_FEE_AND_SUBSCRIBER_SPIKE.md`](./TEST_V4_FEE_AND_SUBSCRIBER_SPIKE.md)。签字前不得实现生产 Hook。
...
此前讨论的成熟后降至 0.5% 方案取消。Test Prototype 使用长期固定 1% 有效手续费，不依赖预言机、时间、区块阶段或外部数据。
...
TGARD 回购销毁仍是 Test Prototype 必备功能，但只由买入侧获得的稳定币协议手续费提供资金。
...
不同股票国库不得自动混用。当前 Test Prototype 推荐方向为：
...
## 12. Test Prototype 合约模块

以下合约全部部署在 Robinhood Chain。Test Prototype 不包含 BridgeAdapter、CCIP TokenPool、LayerZero OFT、BSC wrapped token 或任何跨链消息接收器。
...
Test Prototype 另有一个非合约运维组件：Off-chain Execution Worker。它负责读取 Bucket、获取报价、模拟并提交 FeeExecutor 交易、确认结果、重试和告警；它不持有协议资产，也不拥有绕过 FeeExecutor 限制的权限。

## 13. 非目标与风险披露

Test Prototype 不提供以下承诺：
...
- Test Prototype 不支持将 mStock 转移到 BSC 或其他链。
- 第三方自行部署的同名 token、包装资产或桥接资产不属于 TickerGarden canonical mStock。
- Test Prototype 的 RH 质押仓位、LP 仓位和奖励资格不会同步到其他链。
...
| BSC 与跨链 | Test Prototype 不启用，V1 另行评审 |
| AMM | Uniswap v4 |
...
| TGARD 回购 | Test Prototype 必须实现，由买入侧稳定币协议费提供资金 |
...
1. **P-002 数学规格签字**：decimals、预算关系、1460/1453 天时间轴、衰减累计函数、错过排放、`PoolReleased/PoolMinted`、`1e27` scaled 会计、7 天 claim cooldown、8 槽 Escrow、软门槛快照与 Bucket 舍入已写入 `TEST_MATH_AND_STATE_MACHINES.md` 并有可执行测试；仍须产品与安全负责人对同一版本签字，且每市场精确开始时间留在 launch manifest。
...
17. **P-005 权限规格签字与真实地址**：3/5 Protocol Safe、48 小时 Timelock、独立 2/3 pause-only Guardian、7 天退休/国库迁移、Worker 专用函数和永久退出路径已写入 `TEST_PERMISSIONS_AND_PAUSE_MATRIX.md` 并有机器检查；仍须产品/安全签字及提供真实 Safe/Guardian/Timelock/Treasury 地址和成员重叠政策。
