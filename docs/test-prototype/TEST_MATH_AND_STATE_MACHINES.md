# Test Prototype 数学与状态机参考规格

> 历史归档：本文模型不属于当前 V1 的经济或状态机规范。

> 任务：`P-002`
>
> 状态：`REVIEW`。公式、舍入、边界和可执行测试向量已经完成；在产品与安全负责人签字前，不得把对应 G0 项标记为 `APPROVED`。
>
> 历史参考实现路径：`spec/reference_math.py`；测试路径：`spec/test_reference_math.py`（当前仓库已不再保留）。Solidity 实现必须做差分测试，不能重新解释本文公式。

## 1. 冻结结论

| 主题 | Test Prototype 决定 |
|---|---|
