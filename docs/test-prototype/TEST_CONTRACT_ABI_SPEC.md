# Test Prototype 合约 ABI、错误与事件基线

> 历史归档：本文描述 canonical V1 之前的 Test Prototype，不是当前 V1 ABI 或部署输入。

> 状态：`REVIEW`
>
> 历史输入路径：`contracts/src/interfaces`、`spec/contract_abi_surface.json`、`spec/test_contract_abi_surface.py`（当前仓库已不再保留）。
>
