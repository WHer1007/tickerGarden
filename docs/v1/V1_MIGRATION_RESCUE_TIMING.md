...
> 规格：`V1-EXEC-11`；旧 `Swept` / retry / 七日 rescue 方案已 superseded。
...
旧文件中的管理型市场计时和七日终态救援均已删除；实现、生成物、确定性部署与固定区块 Fork/E2E 已按 V1-EXEC-11 验证。尚未广播目标链交易，部署后 receipt/源码验证与生产 E2E 仍需单独完成。
