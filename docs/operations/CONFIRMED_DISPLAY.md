...
The worker updates only affected markets; head advancement proves unchanged values remain current. Raw event reads use shared topic filters and authenticated emitters rather than one RPC scan per market. Trades, transfer balances and fee deltas are applied incrementally. The latest chart interval is open and updates with executions. The retained rolling history supports all three chart ranges and the 24-hour volume window.
