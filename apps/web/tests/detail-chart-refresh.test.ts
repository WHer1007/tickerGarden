...
test('period switches fetch only candles, preserve summary DOM and reject late responses',async()=>{
 ...
 const detail={version:1,chainId:46630,displayOnly:true,...id,period:'1H',statistics:null,holders:null,trades:[],fees:[],reasons:{},chart:{...initial,points:Array.from({length:60},(_,i)=>({timestamp:initial.from+i*60,price:null}))},sources:Object.fromEntries(['chart','trades','fees'].map(key=>[key,{provider:'indexer',asOf,blockNumber:'10',blockHash:hash}]))};
 const calls:URL[]=[];let release12:((value:Response)=>void)|undefined;let slow12:Response|undefined;
 ...
 install('fetch',async(input:string|URL)=>{
  ...
  if(url.pathname.endsWith('/detail'))return Response.json(detail);
 ...
 try{
  ...
  assert.equal(calls.filter(url=>url.pathname.endsWith('/detail')).length,1);
  assert.deepEqual(calls.slice(1).map(url=>url.searchParams.get('interval')),['5m','15m']);
  ...
  buttons[0]!.onclick!();await flush();assert.equal(calls.length,3,'cached 1H needs no request');
