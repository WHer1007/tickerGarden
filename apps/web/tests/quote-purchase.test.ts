...
test('refresh can reduce the shortfall but cannot increase accepted ETH or asset purchase',()=>{
 ...
 assert.doesNotThrow(()=>assertPurchaseWithinApproval({...quote,amountIn:'110'},quote));
 for(const fresh of [{...quote,amountIn:'111'},{...quote,amountOut:'201'},{...quote,token:'0x1111111111111111111111111111111111111111' as const}])assert.throws(()=>assertPurchaseWithinApproval(fresh,quote),/Purchase cost changed/);
