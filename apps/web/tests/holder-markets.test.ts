...
test('holder search validates directory and caches queries',async()=>{
 ...
 globalThis.fetch=async()=>new Response(JSON.stringify({chainId:46630,complete:true,items:[]}));assert.deepEqual(await searchHolderMarkets('http://localhost:1234',46630,'empty'),[]);
