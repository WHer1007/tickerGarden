...
function fixture(){
 ...
 const element=()=>{const e:any={textContent:'',style:{},append(...parts:any[]){this.textContent=parts.map(p=>typeof p==='string'?p:p.textContent).join('');}};return e;};
 const child:any={};const node:any={textContent:'',title:'',dataset:{},replaceChildren(...parts:any[]){this.textContent=parts.map(p=>typeof p==='string'?p:p.textContent).join('');},ownerDocument:{createElement:element,getSelection:()=>selection,addEventListener:(k:string,fn:Function)=>listeners.set(k,fn)},contains:(n:unknown)=>n===child,
...
test('compact visible price keeps exact hover/accessibility/copy and follows updates',()=>{
 ...
 assert.equal(f.node.textContent,'0.08853515 AAPL / SEED');assert.equal(f.node.title,'0.00000000853515 AAPL / SEED');assert.equal(f.attrs.get('aria-label'),f.node.title);
