...
const answers = [
  ...
  ['trading', 'How Are Trading Fees Shared?', 'A completed trade creates the fee; indexing and allocation can appear later, so an allocated total is not the same as an immediately claimable balance. The token page shows the current split between Creator, Holder, Staker and Platform. Stakers only earn after Bloom with active stake. Creator tax goes entirely to the creator wallet and is separate from the base fee split. Any Pool Protocol Fee shown in the quote is charged separately by PoolManager and is excluded from these shares.'],
  ...
  ['rewards', 'How Does Holder Fee Sharing Work?', 'Enabled tokens share half of the creator’s base fees with holders; creator tax is excluded. On updated markets, Quote and Meme rewards release separately over 24 hours using your eligible balance. No staking is needed, and earned rewards remain yours after selling. Creator, Staker and Holder claims let you choose conversion or original assets. You can authorize original Meme as a fallback for failed or partial conversion. There is no price protection; actual output may differ. Staking locks still apply. Older markets follow the distribution status shown on Claim.'],
