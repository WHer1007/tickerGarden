...
export function mountTrades(element:HTMLElement,baseUrl:string|null,chain:number){
 ...
 const render=()=>{
  ...
  const table=document.createElement('table');const head=table.createTHead().insertRow();for(const name of ['Time (UTC)','Venue / side','Category','Token amount','Paired-asset amount','Price (paired asset/token)','Caller identity']){const th=document.createElement('th');th.scope='col';th.textContent=name;head.append(th);}
  const body=table.createTBody();for(const t of page.items){const row=body.insertRow();for(const value of [new Date(Number(t.timestamp)*1000).toISOString().replace('T',' ').slice(0,19),`${t.venue} / ${t.side}`,t.classification==='unclassified'?'Unclassified':t.classification==='internal_reward_conversion'?'Internal rewards':'Internal Holder rewards',candleVolume(t.memeRaw,18),candleVolume(t.quoteRaw,t.quoteDecimals),candlePriceLabel(t.price),t.actor?`${t.actor} (contract caller; wallet unverified)`:'-'])row.insertCell().textContent=value;}
