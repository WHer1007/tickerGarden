...
export function validateGlobalHolders(value:unknown,chain:number):GlobalHolderCountsResponse{
 ...
 const validCounts=(g:{marketCount:number;positiveMarketAddressPairs:number;positiveAddressCount:number;includedAddressCount:number})=>[g.marketCount,g.positiveMarketAddressPairs,g.positiveAddressCount,g.includedAddressCount].every(count)&&g.includedAddressCount<=g.positiveAddressCount&&g.positiveAddressCount<=g.positiveMarketAddressPairs&&g.positiveMarketAddressPairs<=g.marketCount*g.positiveAddressCount;
