...
export class DirectMarkets {
 ...
 constructor(deployment:IntegrationBootstrap,read:ReadState,head:()=>Promise<{number:bigint;hash:Hex}>,storage?:Pick<Storage,'getItem'|'setItem'>){
  this.deployment=deployment;this.read=read;this.head=head;this.storage=storage;
 ...
 receipt(receipt:TransactionReceipt){for(const log of receipt.logs)if(log.blockNumber!==null&&log.blockHash&&log.transactionHash&&log.transactionIndex!==null&&log.logIndex!==null)this.observe({...log,blockNumber:log.blockNumber,blockHash:log.blockHash,transactionHash:log.transactionHash,transactionIndex:log.transactionIndex,logIndex:log.logIndex});this.cache.clear();}
 async market(id:Hex):Promise<DirectMarket>{
  const old=this.cache.get(id);if(old&&Date.now()-old.at<15_000)return old.value;
  const source=this.sources.get(id);if(!source)throw Error('Market creation has not been observed in this integration session');
  const head=await this.head(),registry=this.deployment.bindings.marketRegistry;
  ...
  const [reserve,sellable,reserved,fees,ready,executor]=await Promise.all(['realQuoteReserve','sellableTokens','reservedTokens','accruedCurveFees','readyToGraduate'].map(name=>this.read(curve,v1Abis.TickerGardenCurve,name,[],head.number)).concat([this.read(this.deployment.factory,v1Abis.TickerGardenFactoryV1,'graduationExecutor',[],head.number)]));
