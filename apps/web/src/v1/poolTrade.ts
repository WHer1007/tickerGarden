...
export const STAKING_POOL_TRADE_GAS_LIMIT=8_000_000n;
...
export function poolTradeRoute(market:MarketReadModel,side:'buy'|'sell'){
 ...
 if(currency0>=currency1||![currency0,currency1].includes(input)||![currency0,currency1].includes(output)||input===output||k.hooks.toLowerCase()!==r.hook.toLowerCase()||![0,1000,2000,3000].includes(k.fee))throw Error('Invalid canonical pool binding');
 const poolKey={...k,currency0,currency1,hooks:k.hooks.toLowerCase() as Address};
...
export function poolTransactionDeadline(blockTimestamp:bigint,ttlSeconds=3600n):bigint{
 if(blockTimestamp<0n||ttlSeconds<=0n)throw Error('Invalid pool transaction clock');
 const deadline=blockTimestamp+ttlSeconds;
 if(deadline>=(1n<<256n))throw Error('Pool transaction deadline is outside the supported range');
 return deadline;
}
export function poolPermit2Expiration(blockTimestamp:bigint,ttlSeconds=600n):number{
 if(blockTimestamp<0n||ttlSeconds<=0n)throw Error('Invalid Permit2 clock');
 const expiration=blockTimestamp+ttlSeconds;
 if(expiration>=(1n<<48n)||expiration>BigInt(Number.MAX_SAFE_INTEGER))throw Error('Permit2 expiration is outside the supported range');
 return Number(expiration);
}
export function poolPermit2AllowanceFresh(expiration:number,blockTimestamp:bigint,minimumRemainingSeconds=120n):boolean{
 if(!Number.isSafeInteger(expiration)||expiration<0||blockTimestamp<0n||minimumRemainingSeconds<0n)throw Error('Invalid Permit2 allowance clock');
 return BigInt(expiration)>=blockTimestamp+minimumRemainingSeconds;
}
export function buildPoolTrade(market:MarketReadModel,side:'buy'|'sell',amount:bigint,minimum:bigint,deadline:bigint):ContractWriteRequest{
 ...
 return {abi:poolRouterAbi,address:route.router,functionName:'execute',args:[route.input===ZERO_ADDRESS?'0x1004':'0x10',route.input===ZERO_ADDRESS?[input,sweep]:[input],deadline],value:route.input===ZERO_ADDRESS?amount:0n,gas:market.stakingEnabled?STAKING_POOL_TRADE_GAS_LIMIT:undefined};
}
...
export function poolProtocolFee(raw:Hex,zeroForOne:boolean,expectedLpFee=0):number {
 if(!/^0x[0-9a-fA-F]{64}$/.test(raw))throw Error('Invalid pool state');
 const packed=BigInt(raw),protocol=Number((packed>>184n)&0xffffffn);
 if(![0,1000,2000,3000].includes(expectedLpFee)||packed>>232n || ((packed>>208n)&0xffffffn)!==BigInt(expectedLpFee) || (protocol&0xfff)>1000 || (protocol>>12)>1000)throw Error('Unsupported pool fees');
...
export function formatPoolFeeSummary(lpFeePips:number,protocolFeePips:number):string {
 if(![0,1000,2000,3000].includes(lpFeePips))throw Error('Invalid LP fee');
 const lp=`${lpFeePips/10000}%`,protocol=formatPoolProtocolFee(protocolFeePips);
 if(lpFeePips>0&&protocolFeePips>0)return `Quote includes a ${lp} LP fee and a ${protocol} pool protocol fee.`;
 if(lpFeePips>0)return `Quote includes a ${lp} LP fee.`;
 if(protocolFeePips>0)return `Quote includes a ${protocol} pool protocol fee.`;
 return 'No pool fee is added to this quote.';
}
