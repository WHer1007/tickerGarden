...
export function candleTable(data:MarketCandlesResponse):HTMLDetailsElement {
 ...
 const caption=document.createElement('caption');caption.textContent='UTC · Prices in Quote per Meme (up to 8 decimal places; rounded down). Volumes in whole tokens. Internal conversions are included in totals.';table.append(caption);
 const head=table.createTHead().insertRow();
 for(const label of ['Hour (UTC)','Open','High','Low','Close','Meme volume','Quote volume','Internal Quote volume','Executions','Internal executions']){const th=document.createElement('th');th.scope='col';th.textContent=label;head.append(th);}
