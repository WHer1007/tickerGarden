...
export function mountTokenDetail(root:HTMLElement,base:string|null,chain:number,explorer:string){
 ...
 let id:DetailIdentity|null=null,period:DetailPeriod='1H',data:TokenDetailResponse|null=null,fees:FeeConfig|null=null,controller:AbortController|null=null,generation=0,tradeLimit=30,holderLimit=20;
 ...
 const draw=(force=false)=>{
  ...
  ctx.fillStyle='#718092';ctx.textAlign='left';const label=(ts:number)=>{const date=new Date(ts*1000);return date.toDateString()===new Date().toDateString()?date.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit',hourCycle:'h23'}):date.toLocaleString('en-GB',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit',hourCycle:'h23'});};ctx.fillText(label(chart.from),left,h-8);ctx.textAlign='right';ctx.fillText(label(chart.to),right,h-8);canvas.setAttribute('aria-label',`${period} ${id?.quoteSymbol} price history; ${active.length} observed intervals. Lines connect recorded trades.`);
 };
