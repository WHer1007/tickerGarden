...
export function mountGlobalStatistics(element:HTMLElement,baseUrl:string|null,chain:number){
 ...
 const status=document.createElement('p');status.setAttribute('role','status');const body=document.createElement('div');const refresh=document.createElement('button');refresh.type='button';refresh.textContent='Retry';element.append(status,body,refresh);refresh.hidden=true;
 element.style.minWidth='0';element.style.overflowWrap='anywhere';
 const load=async(force=false)=>{
  if(controller)return;
  refresh.hidden=true;const own=++generation;controller=new AbortController();const abort=controller;body.replaceChildren();
  if(!baseUrl){controller=null;status.textContent='Trading Data Is Not Available Yet';return;}
  const to=Math.floor(Date.now()/3600000)*3600-3600,from=to-86400;status.textContent='Loading Trading Data…';const timer=setTimeout(()=>abort.abort(),10000);
  try{
   const api=new TickerGardenV1Client(baseUrl,(input,init)=>fetch(input,{...init,signal:abort.signal}));const p=await cachedStatistics(`${baseUrl}:${chain}:overview:${from}:${to}`,async()=>validateGlobalStatistics(await api.getGlobalStatistics({from:String(from),to:String(to)}),chain,from,to),force);if(own!==generation)return;if(abort.signal.aborted)throw new Error('Timeout');
  ...
  }catch{if(own===generation){body.replaceChildren();status.textContent='Trading Data Is Not Available Yet';refresh.hidden=false;}}
  finally{clearTimeout(timer);if(own===generation)controller=null;}
 };
 refresh.addEventListener('click',()=>void load(true));void load();return{refresh:()=>load(),stop(){generation++;controller?.abort();controller=null;body.replaceChildren();status.textContent='Statistics paused or unavailable. Waiting for a verified snapshot.';}};
}
