...
export async function searchHolderMarkets(base:string,chainId:number,q:string,signal?:AbortSignal):Promise<HolderMarket[]>{
 ...
 const key=`${chainId}:${url}`;const prior=cache.get(key);if(prior&&Date.now()-prior.at<600000)return prior.items;
 ...
 if(cache.size>=100)cache.delete(cache.keys().next().value!);cache.set(key,{at:Date.now(),items:page.items});return page.items;
}
