...
export function mountGlobalHolders(element:HTMLElement,baseUrl:string|null,chain:number,summaryCount:HTMLElement|null=null){
 ...
 const body=document.createElement('div'),refresh=document.createElement('button');refresh.type='button';refresh.textContent='Retry';element.append(status,body,refresh);refresh.hidden=true;
 element.style.minWidth='0';element.style.overflowWrap='anywhere';
 const load=async(force=false)=>{
  if(controller)return;
  refresh.hidden=true;const own=++generation;controller=new AbortController();const abort=controller;body.replaceChildren();if(summaryCount){summaryCount.textContent='-';summaryCount.removeAttribute('title');}
  if(!baseUrl){controller=null;status.textContent='Holder Data Is Not Available Yet';return;}
  status.textContent='Loading Holders…';const timer=setTimeout(()=>abort.abort(),10000);
  try{
   const api=new TickerGardenV1Client(baseUrl,(input,init)=>fetch(input,{...init,signal:abort.signal}));
   const p=await cachedStatistics(`${baseUrl}:${chain}:holders`,async()=>validateGlobalHolders(await api.getGlobalHolderCounts(),chain),force);if(own!==generation)return;if(abort.signal.aborted)throw new Error('Timeout');
   ...
   const source=document.createElement('p');source.textContent=`Finalized block ${p.sourceBlockNumber} · ${p.sourceBlockHash}. Addresses are not people or airdrop eligibility. Balances from different created tokens are not added together.`;body.append(source);
  ...
  }catch{if(own===generation){body.replaceChildren();status.textContent='Holder Data Is Not Available Yet';refresh.hidden=false;}}
  finally{clearTimeout(timer);if(own===generation)controller=null;}
 };
 refresh.addEventListener('click',()=>void load(true));void load();return{refresh:()=>load(),stop(){generation++;controller?.abort();controller=null;body.replaceChildren();if(summaryCount){summaryCount.textContent='-';summaryCount.removeAttribute('title');}status.textContent='Statistics paused or unavailable. Waiting for a verified snapshot.';}};
}
