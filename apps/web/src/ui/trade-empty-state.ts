export function renderTradeEmptyState(state: 'missing' | 'invalid' | 'unavailable' | null): void {
  ...
  if (title) title.textContent = state === 'missing' ? 'Choose a market to trade' : state === 'invalid' ? 'This market link is invalid' : 'This market could not be loaded';
  if (description) description.textContent = state === 'missing'
    ...
    : 'Try again shortly, or choose another market from Explore.';
