...
import {TRADE_NATIVE,TRADE_USDG,conversionRequest,type ConversionIntent,type ConversionQuote} from '../../../../services/backend-ts/packages/chain/src/quote-purchase/conversion.ts';
...
export class ConversionAssetUnavailableError extends Error {
 constructor(){super('Automatic exchange is unavailable for this asset. Pay with the paired asset.');this.name='ConversionAssetUnavailableError';}
}
export function paymentAssets(chainId:number,pair:PaymentAsset):PaymentAsset[]{
 return chainId!==4663?[pair]:[pair,...([{address:TRADE_NATIVE,symbol:'ETH',decimals:18}] as PaymentAsset[]).filter(a=>a.address.toLowerCase()!==pair.address.toLowerCase())];
}
export async function fetchConversion(base:string,intent:ConversionIntent):Promise<ConversionQuote>{
 ...
 if(!r.ok){
  ...
  if(body?.error==='conversion_asset_unavailable')throw new ConversionAssetUnavailableError();
  throw Error('This payment route is unavailable. Try again or use the paired asset.');
 }
