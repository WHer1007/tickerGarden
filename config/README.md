...
**7d epoch 与 5s anti-snipe 是合约策略升级，不是测试加速。** 合并 test 到 master 前，必须明确审阅这两项及同步 ABI、参考模型、TWAB schema、后端结算，而不是只把环境名改成 master。上表的 master 金额/可配置时间是源码规范或候选基线，不代表已有主网合约部署事实。
