# V1 Uniswap v4 手续费与 Subscriber Spike（P-003）
...
Robinhood Chain 主网 canonical Uniswap v4 `PositionManager` 支持 V1 所需的 NFT Subscriber 生命周期。使用主网真实未订阅仓位进行 live fork 验证后，订阅、取消订阅、调用者校验和“取消订阅回调失败仍不锁 NFT”均通过；完整 canonical `PositionManager.unsubscribe` 路径实测为 `68,528 Gas`，低于 G0-10 的 `150,000 Gas` 目标，也明显低于链上 `unsubscribeGasLimit = 300,000`。
...
不建议 V1 为同时追求严格 `0.40% + 0.60% = 1.00%` 而实现自定义全额收费后再向 LP donation 的方案。该方案会引入额外 transient accounting、donation/JIT 捕获、边界 tick 和结算风险，明显违背 V1 不过度复杂化的原则。
...
隔离工程位于 [`spikes/v4-subscriber`](./spikes/v4-subscriber/README.md)：
