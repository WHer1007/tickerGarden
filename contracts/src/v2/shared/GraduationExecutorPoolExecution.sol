...
import {ILaunchLocker, ITickerGardenMemeHook, MarketView, PoolBinding, PoolKey} from "../interfaces/IV2Protocol.sol";
import {GraduationPoolMath} from "../libraries/GraduationPoolMath.sol";
import {V2Identifiers} from "./V2Identifiers.sol";
import {V2Create2} from "./V2Create2.sol";
...
abstract contract GraduationExecutorPoolExecution is GraduationExecutorAssetAccounting {
    ...
    function predictLaunchLocker(bytes32 marketId) public view returns (address) {
        ...
        return V2Create2.predict(address(this), _lockerSalt(marketId), keccak256(initCode));
    }
    ...
    function _deployLocker(bytes32 marketId, PoolExecutionContext memory context) private {
        context.launchLocker = V2Create2.deploy(_lockerSalt(marketId), _launchLockerInitCode(marketId));
    ...
    function _lockerSalt(bytes32 marketId) private view returns (bytes32) {
        return
            V2Identifiers.componentSalt(block.chainid, _graduationFactory, marketId, V2Identifiers.ComponentKind.LOCKER);
    }
