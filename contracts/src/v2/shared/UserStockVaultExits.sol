...
abstract contract UserStockVaultExits is UserStockVaultLedger {
    ...
    constructor(
        ...
        bytes32 assetUid_,
        address stockToken_
    ) UserStockVaultLedger(officialStockRegistry_, marketRegistry_, allocationManager_, assetUid_, stockToken_) {}

    function _withdrawFreeStock(address user, uint256 amount) internal nonReentrant {
        if (amount == 0) revert InvalidWithdrawalAmount(amount);
        _canonicalAsset();

        uint256 available = _freeBalanceOf(user);
        if (amount > available) revert InsufficientFreeBalance(amount, available);

        _deposited[user] -= amount;
        _totalDeposited -= amount;

        uint256 vaultBalanceBefore = _stockToken.balanceOf(address(this));
        uint256 userBalanceBefore = _stockToken.balanceOf(user);
        (bool success, bytes memory result) = address(_stockToken).call(abi.encodeCall(IERC20.transfer, (user, amount)));
        if (!success) revert StockWithdrawalCallFailed(address(_stockToken));
        if (result.length != 32 || abi.decode(result, (uint256)) != 1) {
            revert InvalidStockWithdrawalReturn(address(_stockToken));
        }

        uint256 vaultBalanceAfter = _stockToken.balanceOf(address(this));
        uint256 userBalanceAfter = _stockToken.balanceOf(user);
        ...
        if (vaultDecrease != amount || userIncrease != amount) {
            revert InexactStockWithdrawalDelta(address(_stockToken), amount, vaultDecrease, userIncrease);
        }
    }

    function _forceReleaseAllocation(address user, bytes32 marketId)
        internal
        nonReentrant
        returns (uint256 amount, uint32 recoveryEpoch)
    {
        MarketView memory marketView = _canonicalMarket(marketId);
        ...
        amount = _allocation[user][marketId];
        if (amount == 0) revert NoAllocationToForceRelease(user, marketId);

        _allocation[user][marketId] = 0;
