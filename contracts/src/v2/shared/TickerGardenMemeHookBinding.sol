...
abstract contract TickerGardenMemeHookBinding is ITickerGardenMemeHook {
    ...
    uint8 internal constant BINDING_DISABLED = 4;

    IMarketRegistryV2 internal immutable _hookMarketRegistry;
    ...
    address internal immutable _hookMarketController;
    ...
    constructor(
        ...
        if (
            !_isContract(marketRegistry_) || !_isContract(poolManager_) || !_isContract(protocolFeeVault_)
                || !_isContract(graduationExecutor_) || !_isContract(marketController_)
                || marketRegistry_ == poolManager_ || marketRegistry_ == protocolFeeVault_
                || marketRegistry_ == graduationExecutor_ || marketRegistry_ == marketController_
                || poolManager_ == protocolFeeVault_ || poolManager_ == graduationExecutor_
                || poolManager_ == marketController_ || protocolFeeVault_ == graduationExecutor_
                || protocolFeeVault_ == marketController_ || graduationExecutor_ == marketController_
        ) {
            revert InvalidHookDependencies(
                marketRegistry_, poolManager_, protocolFeeVault_, graduationExecutor_, marketController_
            );
        }

        _hookMarketRegistry = IMarketRegistryV2(marketRegistry_);
        _hookPoolManager = poolManager_;
        _hookProtocolFeeVault = protocolFeeVault_;
        _hookGraduationExecutor = graduationExecutor_;
        _hookMarketController = marketController_;
    }
        ...
        returns (bytes32 poolId)
    {
        ...
        if (
            marketId == bytes32(0) || value.runtime.launchPhase != LAUNCH_PHASE_SWEPT
                || value.runtime.marketStatus != MARKET_STATUS_ACTIVE || value.runtime.poolId != bytes32(0)
        ) {
        ...
        returns (bytes32 poolId, PoolBinding storage binding, MarketView memory value)
    {
        ...
        if (
            value.runtime.launchPhase != LAUNCH_PHASE_POOL_CREATED || value.runtime.marketStatus != MARKET_STATUS_ACTIVE
                || value.runtime.poolId != poolId || value.runtime.sourceVersion != binding.sourceVersion
                || value.config.graduatedHook != address(this)
