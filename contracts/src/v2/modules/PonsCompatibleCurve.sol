...
/// @notice Per-market tracked-reserve Pons curve with exact native/ERC-20 settlement.
...
contract PonsCompatibleCurve is IPonsCompatibleCurve, ReentrancyGuard {
    ...
    bytes32 private constant CURVE_SWEEP_DOMAIN = keccak256("TICKERGARDEN_V2_CURVE_SWEEP");
    uint256 private constant CURVE_SWEEP_SCHEMA_VERSION = 1;

    bytes32 private immutable _marketId;
    bytes32 private immutable _ponsBaselineId;
    bytes32 private immutable _quoteAssetConfigId;
    IMarketRegistryV2 private immutable _marketRegistry;
    ...
    error MarketNotTradable(uint8 launchPhase, uint8 marketStatus, bool completed);
    ...
    error LaunchSweepNotCommitted(bytes32 marketId, uint8 launchPhase, uint8 marketStatus);
    ...
    constructor(address factory_) {
        ...
        _ponsBaselineId = init.ponsBaselineId;
        _quoteAssetConfigId = init.quoteAssetConfigId;
        _marketRegistry = IMarketRegistryV2(init.marketRegistry);
        ...
        returns (uint256 quoteOut, uint256 fee)
    {
        _requireTradable();
        _requireRecipient(recipient);
        PonsCurveMath.SellQuote memory quote = _sellQuote(tokensIn, minQuoteOut);
    ...
    function _finalizeLaunch() private {
        ...
        if (
            sweptMarket.runtime.launchPhase != LAUNCH_PHASE_SWEPT
                || sweptMarket.runtime.marketStatus != MARKET_STATUS_ACTIVE
        ) {
            revert LaunchSweepNotCommitted(_marketId, sweptMarket.runtime.launchPhase, sweptMarket.runtime.marketStatus);
        }
        ...
        returns (uint256 tokensOut, uint256 quoteSpent, uint256 refund)
    {
        _requireTradable();
        _requireRecipient(recipient);
        PonsCurveMath.BuyQuote memory quote = _buyQuote(quoteIn, 0, recipient, msg.sender).curveQuote;
    ...
    function getReserves() external view override returns (uint256 quoteReserve, uint256 tokenReserve) {
        ...
        return PonsSupplyMath.pricingReserves(_reserves, _phantomQuote);
    }
    ...
    function sellableTokens() external view override returns (uint256) {
        ...
        return PonsSupplyMath.sellableTokens(_reserves);
    }
    ...
    function _requireTradable() private view {
        MarketView memory marketView = _requireRegisteredCurve();
        if (
            marketView.runtime.launchPhase != LAUNCH_PHASE_NOT_GRADUATED
                || marketView.runtime.marketStatus != MARKET_STATUS_ACTIVE || _completed
        ) {
            revert MarketNotTradable(marketView.runtime.launchPhase, marketView.runtime.marketStatus, _completed);
        }
    }
    ...
    function _validateInitialization(CurveInitialization memory init, address factory_) private view {
        if (
            init.marketId == bytes32(0) || init.ponsBaselineId == bytes32(0) || init.quoteAssetConfigId == bytes32(0)
        ...
        ITickerMemeTokenV2 token = ITickerMemeTokenV2(init.memeToken);
        if (
