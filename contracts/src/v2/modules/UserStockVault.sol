...
contract UserStockVault is IUserStockVault, UserStockVaultExits {
    ...
    function forceReleaseAllocation(bytes32 assetUid, bytes32 marketId) external override returns (uint256 amount) {
        uint32 recoveryEpoch;
        (amount, recoveryEpoch) = _forceReleaseAllocation(assetUid, msg.sender, marketId);
        emit AllocationForceReleased(assetUid, msg.sender, marketId, amount, recoveryEpoch);
    }
