...
import {
    ITickerMemeTokenV2,
    ITreasuryDistributorV2,
    RootServiceFeeV2,
    TreasuryEpochStatusV2,
    TreasuryEpochV2,
    TreasuryMarketV2
} from "../interfaces/ITreasuryV2.sol";
import {TreasuryClaimLeafV2} from "../libraries/TreasuryClaimLeafV2.sol";
import {ImmutableAccessManagedV2} from "../shared/ImmutableAccessManagedV2.sol";

struct TreasuryDistributorInitV2 {
    address authority;
...
/// @notice Shared, market-isolated Quote treasury and attested TWAB Merkle distributor for a future V2 release.
/// @dev This contract deliberately contains no fee percentages and has no V1 import or mutation path.
contract TreasuryDistributorV2 is ITreasuryDistributorV2, ImmutableAccessManagedV2, ReentrancyGuard {
    using SafeERC20 for IERC20;

    uint32 public constant EPOCH_DURATION = 30 days;
    bytes32 public constant EXECUTION_SPEC_ID = keccak256("V2-TREASURY-EXEC-1");
    bytes32 public constant TWAB_SCHEMA = keccak256("TRANSFER_LOG_TWAB_30D_V1");
    bytes32 public constant CLAIM_LEAF_DOMAIN = keccak256("TICKERGARDEN_V2_TREASURY_CLAIM_V1");

    bytes32 private constant FUND_OPERATION_DOMAIN = keccak256("V2_TREASURY_FUND_QUOTE");
    bytes32 private constant BURN_OPERATION_DOMAIN = keccak256("V2_TREASURY_BURN_MEME");
    ...
    RootServiceFeeV2 private _rootServiceFee;

    mapping(bytes32 marketId => TreasuryMarketV2 value) private _markets;
    mapping(bytes32 marketId => mapping(uint32 epochId => uint256 amount)) private _epochQuoteAmounts;
    mapping(bytes32 marketId => mapping(uint32 epochId => TreasuryEpochV2 value)) private _epochs;
    ...
    error RootAlreadyRequested(bytes32 marketId, uint32 epochId, TreasuryEpochStatusV2 status);
    ...
    error InvalidEpochStatus(TreasuryEpochStatusV2 current, TreasuryEpochStatusV2 required);
    ...
    constructor(TreasuryDistributorInitV2 memory init) ImmutableAccessManagedV2(init.authority) {
        if (
        ...
        _rootServiceFee = RootServiceFeeV2({asset: init.rootServiceFeeAsset, amount: init.rootServiceFeeAmount});
    }
        ...
        restricted
    {
        ...
        if (
            _readBytes32(memeToken, ITickerMemeTokenV2.marketId.selector) != marketId
                || _readAddress(memeToken, ITickerMemeTokenV2.treasuryDistributor.selector) != address(this)
                || _readUint256(memeToken, IERC20.totalSupply.selector) == 0
        ) revert InvalidMemeToken(memeToken);

        _markets[marketId] = TreasuryMarketV2({
    ...
    /// @notice One-way graduation seam. A future V2 GraduationExecutor receives this selector; V1 never calls it.
    function activateMarket(bytes32 marketId) external override restricted {
        TreasuryMarketV2 storage value = _registeredMarket(marketId);
    ...
    function setRootServiceFee(address asset, uint128 amount) external override restricted {
        ...
        _rootServiceFee = RootServiceFeeV2({asset: asset, amount: amount});
    ...
    {
        TreasuryMarketV2 storage value = _activeMarket(marketId);
    ...
    function burnMeme(bytes32 marketId, uint256 amount, bytes32 burnId) external override nonReentrant {
        ...
        ITickerMemeTokenV2(value.memeToken).burnTreasury(amount);
    ...
    function requestRoot(bytes32 marketId, uint32 epochId) external payable override nonReentrant {
        ...
        TreasuryEpochV2 storage valueEpoch = _epochs[marketId][epochId];
        if (valueEpoch.status != TreasuryEpochStatusV2.UNREQUESTED) {
        ...
        RootServiceFeeV2 memory fee = _rootServiceFee;
        ...
        valueEpoch.status = TreasuryEpochStatusV2.REQUESTED;
    ...
    function publishRoot(
        ...
        TreasuryEpochV2 storage valueEpoch = _epochs[marketId][epochId];
        _requireEpochStatus(valueEpoch, TreasuryEpochStatusV2.REQUESTED);
        ...
        valueEpoch.status = TreasuryEpochStatusV2.ROOT_PENDING;
        ...
        restricted
    {
        ...
        TreasuryEpochV2 storage valueEpoch = _epochs[marketId][epochId];
        _requireEpochStatus(valueEpoch, TreasuryEpochStatusV2.ROOT_PENDING);
    ...
    function finalizeRoot(bytes32 marketId, uint32 epochId) external override nonReentrant {
        ...
        valueEpoch.status = TreasuryEpochStatusV2.CLAIMING;
    ...
    function claim(
        ...
        TreasuryMarketV2 storage value = _activeMarket(marketId);
        TreasuryEpochV2 storage valueEpoch = _epochs[marketId][epochId];
        _requireEpochStatus(valueEpoch, TreasuryEpochStatusV2.CLAIMING);
        ...
        _totalQuoteLiabilities[value.quoteToken] -= amount;
        _payExactToken(value.quoteToken, account, amount);
        _assertSolvent(value.quoteToken);
        ...
        returns (uint32 toEpochId, uint256 amount)
    {
        ...
        valueEpoch.status = TreasuryEpochStatusV2.ROLLED_OVER;
    ...
    function withdrawServiceCredit(address asset) external override nonReentrant returns (uint256 amount) {
        ...
        if (asset == address(0)) {
            ...
            _payExactToken(asset, msg.sender, amount);
        }
        _assertSolvent(asset);
        emit ServiceCreditWithdrawn(asset, msg.sender, amount);
    }

    function market(bytes32 marketId) external view override returns (TreasuryMarketV2 memory) {
        TreasuryMarketV2 storage value = _registeredMarket(marketId);
        return value;
    }

    function epoch(bytes32 marketId, uint32 epochId) external view override returns (TreasuryEpochV2 memory) {
        _registeredMarket(marketId);
        if (epochId == 0) revert InvalidEpochId(epochId);
        return _epochs[marketId][epochId];
    }

    function rootServiceFee() external view override returns (RootServiceFeeV2 memory) {
    ...
    function _claimLeaf(
        TreasuryMarketV2 storage value,
        TreasuryEpochV2 storage valueEpoch,
        bytes32 marketId,
        uint32 epochId,
        uint256 leafIndex,
        address account,
        ...
        return TreasuryClaimLeafV2.hash(context, leafIndex, account, twab, amount);
    }
    ...
    function _collectServiceFee(RootServiceFeeV2 memory fee) private {
    ...
    function _refundAndReset(bytes32 marketId, uint32 epochId, TreasuryEpochV2 storage valueEpoch) private {
        address requester = valueEpoch.requester;
    ...
    function _payExactToken(address token, address beneficiary, uint256 amount) private {
        IERC20 asset = IERC20(token);
    ...
    function _currentEpochId(bytes32 marketId, TreasuryMarketV2 storage value) private view returns (uint32 epochId) {
    ...
    function _epochWindow(bytes32 marketId, TreasuryMarketV2 storage value, uint32 epochId)
        private
        view
    ...
    function _registeredMarket(bytes32 marketId) private view returns (TreasuryMarketV2 storage value) {
        value = _markets[marketId];
        if (value.memeToken == address(0)) revert MarketNotRegistered(marketId);
    }

    function _activeMarket(bytes32 marketId) private view returns (TreasuryMarketV2 storage value) {
        value = _registeredMarket(marketId);
        if (value.activatedAt == 0) revert MarketNotActive(marketId);
    }

    function _requireEpochStatus(TreasuryEpochV2 storage value, TreasuryEpochStatusV2 required) private view {
