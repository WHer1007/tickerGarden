...
import {GaugeIdentity} from "../interfaces/IV1Protocol.sol";

/// @notice Deterministic immutable-argument clone codec and deployment helpers for MemeStockGauge.
/// @dev Gauge identity is seven static ABI words. The 45-byte OpenZeppelin proxy runtime appends those
///      words verbatim, so every market keeps immutable identity while reward storage remains isolated at
///      the clone address.
library MemeStockGaugeClone {
    uint256 internal constant ARGS_LENGTH = 7 * 32;
    ...
    error InvalidGaugeDependencies(address allocationManager, address protocolFeeVault);
    ...
    function _validate(GaugeIdentity memory identity, bool requireMemeCode) private view {
        ...
        if (
            identity.allocationManager.code.length == 0 || identity.protocolFeeVault.code.length == 0
                || identity.allocationManager == identity.protocolFeeVault
                || identity.allocationManager == identity.memeToken || identity.protocolFeeVault == identity.memeToken
                || identity.allocationManager == identity.quoteAsset || identity.protocolFeeVault == identity.quoteAsset
        ) {
            revert InvalidGaugeDependencies(identity.allocationManager, identity.protocolFeeVault);
        }
    }
