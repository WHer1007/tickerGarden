...
`PonsAntiSnipe.sol` freezes the observed three-second runtime lookup table instead of fitting an unverified decay formula. It applies the fixed 100 bps minimum-net clip, retains a zero-tax reference quote helper; the product Curve clips with base fee plus creator tax and computes each fee leg independently through `PonsCurveMath`, and recognizes only the frozen creator, creation-time beneficiary, or an authenticated Launch Router's atomic first-buy recipient.
