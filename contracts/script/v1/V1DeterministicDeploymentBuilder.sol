...
library V1DeterministicDeploymentBuilder {
    ...
    uint8 internal constant TREASURY_DISTRIBUTOR = 14;
        ...
        returns (bytes[] memory initCodes)
    {
        ...
        initCodes[TREASURY_DISTRIBUTOR] = bytes.concat(
            type(TreasuryDistributorV1).creationCode,
            abi.encode(
                TreasuryDistributorInitV1({
                    authority: components[ACCESS_MANAGER],
                    marketRegistry: components[MARKET_REGISTRY],
                    rootServiceTreasury: config.rootServiceTreasury,
                    rootServiceFeeAsset: config.rootServiceFeeAsset,
                    rootServiceFeeAmount: config.rootServiceFeeAmount,
                    finalityDelaySeconds: config.finalityDelaySeconds,
                    finalityDelayBlocks: config.finalityDelayBlocks,
                    rootPublicationWindow: config.rootPublicationWindow,
                    rootReviewDelay: config.rootReviewDelay,
        ...
        return bytes.concat(
            ...
            abi.encode(
                TickerGardenFactoryInit({
                    ...
                    platformTreasury: config.platformTreasury,
                    treasuryDistributor: components[TREASURY_DISTRIBUTOR],
