...
import {
    V1RobinhoodMainnetDeploymentOrchestrator as V1DeterministicDeploymentOrchestrator
} from "./V1RobinhoodMainnetDeploymentOrchestrator.sol";
...
/// @notice Read-only planning and local simulation for the RH mainnet staged candidate.
...
contract DeployV1RobinhoodMainnetStaged is V1ReleaseGate {
        ...
        returns (V1BootstrapContext memory context, V1DeploymentPlan memory plan, V1DeploymentPayload memory payload)
    {
        ...
        if (expectedChainId != 4663) revert InvalidConfiguration("V1_EXPECTED_CHAIN_ID");
    ...
    /// @notice Local simulation only: no private key or broadcast cheatcode is used.
    function simulate() external returns (V1DeploymentPlan memory plan) {
        ...
        vm.startPrank(context.deployer);
    ...
    function _executeStaged(
        ...
        if (context.orchestrator.code.length == 0) {
            ...
            console2.log("orchestrator execution gas", startGas - gasleft());
        ...
        if (!orchestrator.completed()) {
            for (uint8 i = orchestrator.nextComponent(); i < 16; ++i) {
                ...
                if (usedGas >= 28_000_000) revert InvalidConfiguration("COMPONENT_GAS_BUDGET");
                console2.log("component execution gas", i, usedGas);
            }
            uint256 startGas = gasleft();
            orchestrator.finish(payload);
            uint256 usedGas = startGas - gasleft();
            if (usedGas >= 28_000_000) revert InvalidConfiguration("FINISH_GAS_BUDGET");
            console2.log("finish execution gas", usedGas);
        }
    }
