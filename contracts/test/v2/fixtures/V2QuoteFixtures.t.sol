...
import {
    ...
    MockV2ExactBalanceReceiver
} from "../mocks/MockV2QuoteAssets.sol";

contract V2QuoteFixturesTest is Test {
    address internal constant USER = address(0xA11CE);
    uint256 internal constant AMOUNT = 1_000_000;

    MockV2ExactBalanceReceiver internal receiver;

    function setUp() public {
        receiver = new MockV2ExactBalanceReceiver();
    }
    ...
    function test_feeOnTransferFailsExactBalanceDelta() public {
        ...
        token.approve(address(receiver), AMOUNT);
        vm.expectRevert(
            abi.encodeWithSelector(MockV2ExactBalanceReceiver.InexactBalanceDelta.selector, AMOUNT, AMOUNT - 10_000)
        );
        receiver.pull(address(token), USER, AMOUNT);
    }
    ...
    function test_transferCallbackCannotEnterExactBalanceReceiver() public {
        ...
        vm.expectRevert(MockV2ExactBalanceReceiver.TransferCallFailed.selector);
        receiver.pull(address(token), USER, AMOUNT);
    ...
    function test_falseNoDataAndMalformedReturnAreRejected() public {
        ...
        vm.expectRevert(MockV2ExactBalanceReceiver.InvalidTransferReturn.selector);
        receiver.pull(address(token), USER, AMOUNT);
