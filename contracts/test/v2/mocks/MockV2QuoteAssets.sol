...
interface IV2QuoteTransferCallback {
    function onV2QuoteTransfer(address token, address from, uint256 amount) external;
}
...
contract MockCallbackQuoteToken is MockExactQuoteToken {
    ...
    function _transfer(address owner, address recipient, uint256 amount) internal override {
        ...
        if (callbackEnabled) {
            IV2QuoteTransferCallback(recipient).onV2QuoteTransfer(address(this), owner, amount);
        }
    }
}
...
contract MockV2ExactBalanceReceiver is IV2QuoteTransferCallback {
    ...
    function onV2QuoteTransfer(address, address, uint256) external view {
