...
import {TreasuryEpochStatusV2, TreasuryEpochV2, TreasuryMarketV2} from "../../../src/v2/interfaces/ITreasuryV2.sol";
...
import {MockQuoteTokenV2} from "../mocks/MockV2TreasuryAssets.sol";

contract TreasuryDistributorV2InvariantHandler is Test {
    ...
    bytes32 internal constant MARKET_ID = keccak256("V2-TREASURY-INVARIANT-MARKET");
    ...
    TreasuryDistributorV2 public immutable distributor;
    TickerMemeTokenV2 public immutable meme;
    MockQuoteTokenV2 public immutable quote;
    ...
    constructor(TreasuryDistributorV2 distributor_, TickerMemeTokenV2 meme_, MockQuoteTokenV2 quote_) {
    ...
    function rollover(uint8 rawEpoch) external {
        (bool found, uint32 epochId) = _epochWithStatus(rawEpoch, TreasuryEpochStatusV2.CLAIMING);
        if (!found) return;
    ...
    function requestRoot(uint8 rawEpoch, uint8 rawHolder) external {
        ...
        TreasuryEpochV2 memory valueEpoch = distributor.epoch(MARKET_ID, epochId);
        if (
            valueEpoch.status != TreasuryEpochStatusV2.UNREQUESTED
    ...
    function publishRoot(uint8 rawEpoch) external {
        (bool found, uint32 epochId) = _epochWithStatus(rawEpoch, TreasuryEpochStatusV2.REQUESTED);
        if (!found) return;
        TreasuryEpochV2 memory valueEpoch = distributor.epoch(MARKET_ID, epochId);
    ...
    function finalizeRoot(uint8 rawEpoch) external {
        (bool found, uint32 epochId) = _epochWithStatus(rawEpoch, TreasuryEpochStatusV2.ROOT_PENDING);
        if (!found) return;
        TreasuryEpochV2 memory valueEpoch = distributor.epoch(MARKET_ID, epochId);
    ...
    function _epochWithStatus(uint8 rawEpoch, TreasuryEpochStatusV2 status)
        private
        view
...
contract TreasuryDistributorV2InvariantTest is Test {
    ...
    bytes32 private constant MARKET_ID = keccak256("V2-TREASURY-INVARIANT-MARKET");
    ...
    TreasuryDistributorV2InvariantHandler private handler;
    ...
    function setUp() public {
        ...
        distributor = new TreasuryDistributorV2(
            TreasuryDistributorInitV2({
        ...
        meme = new TickerMemeTokenV2(
            MARKET_ID,
            CREATOR,
            CURVE,
            address(distributor),
            "Ticker Garden V2 Invariant",
            "TGI",
            "ipfs://v2-invariant",
            INITIAL_SUPPLY
        );
        ...
        handler = new TreasuryDistributorV2InvariantHandler(distributor, meme, quote);
    ...
    function invariant_allQuoteLiabilityRemainsAssignedToAnEpoch() public view {
        ...
        for (uint256 i; i < fundedEpochCount; ++i) {
            ...
            } else if (valueEpoch.status != TreasuryEpochStatusV2.ROLLED_OVER) {
    ...
    function invariant_randomTimeAndEpochArithmeticStayInRange() public view {
        ...
        for (uint256 i; i < fundedEpochCount; ++i) {
            ...
            TreasuryEpochV2 memory valueEpoch = distributor.epoch(MARKET_ID, epochId);
            if (valueEpoch.status == TreasuryEpochStatusV2.ROLLED_OVER) {
                assertEq(distributor.epochQuoteAmount(MARKET_ID, epochId), 0);
            } else if (valueEpoch.status != TreasuryEpochStatusV2.UNREQUESTED) {
