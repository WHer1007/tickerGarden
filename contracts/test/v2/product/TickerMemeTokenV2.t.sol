...
import {ITickerMemeTokenV2} from "../../../src/v2/interfaces/IV2Protocol.sol";
...
contract TokenFactoryHarness {
    function deploy(
        ...
        uint256 supply
    ) external returns (TickerMemeTokenV2 token) {
        token = new TickerMemeTokenV2{salt: salt}(marketId, creator, predictedCurve, name, symbol, metadataURI, supply);
    }
...
contract TickerMemeTokenV2Test is Test {
    ...
    TickerMemeTokenV2 internal token;
    ...
    function test_constructorRejectsIncompleteIdentityOrZeroSupply() public {
        ...
        new TickerMemeTokenV2(bytes32(0), CREATOR, CURVE, "Name", "SYM", "uri", SUPPLY);

        vm.expectRevert(TickerMemeTokenV2.InvalidTokenIdentity.selector);
        new TickerMemeTokenV2(MARKET_ID, address(0), CURVE, "Name", "SYM", "uri", SUPPLY);

        vm.expectRevert(TickerMemeTokenV2.InvalidTokenIdentity.selector);
        new TickerMemeTokenV2(MARKET_ID, CREATOR, address(0), "Name", "SYM", "uri", SUPPLY);

        vm.expectRevert(TickerMemeTokenV2.InvalidTokenIdentity.selector);
        new TickerMemeTokenV2(MARKET_ID, CREATOR, CURVE, "Name", "SYM", "uri", 0);
    }

    function test_customSelectorsMatchCanonicalInterface() public pure {
        assertEq(TickerMemeTokenV2.marketId.selector, ITickerMemeTokenV2.marketId.selector);
        assertEq(TickerMemeTokenV2.creator.selector, ITickerMemeTokenV2.creator.selector);
        assertEq(TickerMemeTokenV2.factory.selector, ITickerMemeTokenV2.factory.selector);
        assertEq(TickerMemeTokenV2.metadataURI.selector, ITickerMemeTokenV2.metadataURI.selector);
        assertEq(TickerMemeTokenV2.initialSupply.selector, ITickerMemeTokenV2.initialSupply.selector);
    }
}
