...
import {ILaunchTemplateRegistry, LaunchTemplate} from "../../../src/v2/interfaces/IV2Protocol.sol";
import {LaunchTemplateRegistry} from "../../../src/v2/modules/LaunchTemplateRegistry.sol";
...
contract LaunchTemplateRegistryTest is Test {
    ...
    function test_hashMatchesFrozenAbiEncodeVector() public pure {
        ...
        assertEq(_templateHash(value), 0x03a3cb9b9088d621b7dc7c5dc1d658bf44b76e8472892fb51a7b3f67ecf7b9ca);
    }
    ...
    function test_rejectsZeroIdentityPolicySpecAndNonActiveStatus() public {
        ...
        _expectInvalid(TEMPLATE_ID, value);
        value = _validTemplate();
        value.executionSpecId = keccak256("V2-EXEC-4");
        _expectInvalid(TEMPLATE_ID, value);
        value = _validTemplate();
        value.status = 0;
        _expectInvalid(TEMPLATE_ID, value);
    ...
    function _templateHash(LaunchTemplate memory value) private pure returns (bytes32) {
        bytes memory first = abi.encode(
            keccak256("TICKERGARDEN_V2_LAUNCH_TEMPLATE"),
            uint256(1),
