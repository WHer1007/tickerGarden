...
import {PonsSupplyMath} from "../../../src/v2/libraries/PonsSupplyMath.sol";

/// @notice Machine-linked Solidity replay of frozen Pons vectors plus directly observed runtime evidence.
contract V2PonsDifferentialTest is Test {
    string private _vectors;
    string private _runtime;

    function setUp() public {
        string memory root = vm.projectRoot();
        _vectors = vm.readFile(string.concat(root, "/../spec/v2_pons_behavior_vectors.json"));
        _runtime = vm.readFile(string.concat(root, "/../spec/v2_pons_runtime_evidence.json"));
    }
    ...
    function test_supplyAndGraduationVectorsMatchFrozenReferenceOutputsExactly() public view {
        for (uint256 i; i < 2; ++i) {
            (uint256 reserved, uint256 sellable) = PonsSupplyMath.supplyPartition(
        ...
        uint256 index = 13;
        (uint256 poolMeme, uint256 lockedExcess) = PonsSupplyMath.graduationPartition(
    ...
    function test_curveTradeVectorsMatchFrozenReferenceOutputsExactly() public view {
        ...
        assertEq(
            PonsCurveMath.proportionalSlippagePass(
    ...
    function test_antiSnipeVectorsMatchObservedRuntimeScheduleAndFrozenOutputsExactly() public view {
        for (uint256 offset; offset < 5; ++offset) {
            ...
            (uint256 rawBps, uint256 effectiveBps) = PonsAntiSnipe.effectiveSnipeBps(elapsed, exempt, feeBps);
            ...
            PonsCurveMath.BuyQuote memory quote = PonsCurveMath.quoteBuy(
    ...
    function test_runtimeDifferentialAntiSnipeScheduleMatchesExactly() public view {
        for (uint256 index = 8; index < 13; ++index) {
            ...
            (uint256 rawBps,) = PonsAntiSnipe.effectiveSnipeBps(elapsed, exempt, 100);
    ...
    function _assertSell(uint256 index) private view {
        PonsCurveMath.SellQuote memory quote = PonsCurveMath.quoteSell(
