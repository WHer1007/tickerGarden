...
import {MarketView, QuoteAssetConfig} from "../../../src/v2/interfaces/IV2Protocol.sol";
import {GraduationExecutorEntry} from "../../../src/v2/shared/GraduationExecutorEntry.sol";
import {GraduationExecutorRetryAndRescue} from "../../../src/v2/shared/GraduationExecutorRetryAndRescue.sol";
import {
...
contract GraduationExecutorRetryAndRescueTest is Test {
    ...
    function test_retryRejectsPausedMarketWithoutChangingEscrow() public {
        quote.mint(address(executor), SWEPT_QUOTE);
        meme.mint(address(executor), SWEPT_MEME);
        registry.setMarketStatus(1);

        vm.expectRevert(
            abi.encodeWithSelector(GraduationExecutorEntry.GraduationNotRetryable.selector, MARKET_ID, 1, 1)
        );
        executor.retryGraduation(MARKET_ID);
    ...
    function test_rescueIsPermissionlessInclusiveAndPreservesForcedBalances() public {
        ...
        assertEq(rescued.runtime.marketStatus, 2);
        ...
        vm.expectRevert(
            abi.encodeWithSelector(GraduationExecutorEntry.GraduationNotRetryable.selector, MARKET_ID, 3, 2)
        );
        executor.retryGraduation(MARKET_ID);
    }
