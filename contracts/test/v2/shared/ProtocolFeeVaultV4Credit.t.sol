...
import {MockExactQuoteToken, MockFeeOnTransferQuoteToken} from "../mocks/MockV2QuoteAssets.sol";
...
contract FeeCreditSourceMock {
    ...
    function creditErc20WithVersionDrift(
        ...
        vault.beginV4Credit(marketId, address(token), 80, sourceVersion, feeId);
        registry.configure(
            marketId, address(this), address(token), memeToken, 2, 0, sourceVersion + 1, keccak256("POOL")
        );
        require(token.transfer(address(vault), 80), "TRANSFER");
...
contract ProtocolFeeVaultV4CreditTest is Test {
    ...
    function setUp() public {
        ...
        registry.configure(MARKET_ID, address(source), address(quote), address(meme), 2, 0, SOURCE_VERSION, POOL_ID);
    }
    ...
    function test_exactNativeArrivalAcceptsOnlyPoolManagerDuringPendingWindow() public {
        ...
        registry.configure(MARKET_ID, address(source), address(0), address(meme), 2, 0, SOURCE_VERSION, POOL_ID);
    ...
    function test_feeOnTransferMismatchRollsBackTransferBeginAndConsumption() public {
        ...
        registry.configure(MARKET_ID, address(source), address(taxed), address(meme), 2, 0, SOURCE_VERSION, POOL_ID);
    ...
    function test_everyInactiveLifecycleShapeRejectsBegin() public {
        _expectInactiveBegin(1, 0, SOURCE_VERSION, POOL_ID);
        _expectInactiveBegin(2, 1, SOURCE_VERSION, POOL_ID);
        _expectInactiveBegin(2, 0, SOURCE_VERSION, bytes32(0));
        _expectInactiveBegin(2, 0, SOURCE_VERSION + 1, POOL_ID);
    }
    ...
    function test_nativeReceiveAndMalformedBalanceReadFailClosed() public {
        ...
        registry.configure(MARKET_ID, address(source), address(malformed), address(meme), 2, 0, SOURCE_VERSION, POOL_ID);
        vm.prank(address(source));
        vm.expectRevert(
    ...
    function _expectInactiveBegin(uint8 launchPhase, uint8 marketStatus, uint32 configuredVersion, bytes32 poolId)
        private
    {
        registry.configure(
            MARKET_ID,
        ...
        vm.prank(address(source));
        vault.beginV4Credit(
            MARKET_ID, address(quote), 80, SOURCE_VERSION, keccak256(abi.encode(launchPhase, marketStatus, poolId))
        );
    }
}
