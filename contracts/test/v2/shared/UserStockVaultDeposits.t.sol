...
interface IVaultDepositHarness {
    function depositStockFor(address user, uint256 amount) external;
}

contract MockVaultAllocationManager {
    function depositForCaller(IVaultDepositHarness vault, uint256 amount) external {
        vault.depositStockFor(msg.sender, amount);
    }
}
...
contract UserStockVaultDepositsHarness is UserStockVaultDeposits, IV2QuoteTransferCallback, IVaultDepositHarness {
    ...
    constructor(
        ...
        bytes32 assetUid,
        address stockToken
    ) UserStockVaultDeposits(registry, marketRegistry, allocationManager, assetUid, stockToken) {}
...
contract UserStockVaultDepositsTest is Test {
    ...
    function setUp() public {
        ...
        vault = _deployAndRegister(address(stockToken));
    }
    ...
    function test_directDepositPullsFromAndCreditsOnlyTheCaller() public {
        ...
        vm.prank(USER);
        vault.depositStock(600);

        assertEq(stockToken.balanceOf(USER), 400);
        assertEq(stockToken.balanceOf(address(vault)), 600);
        assertEq(vault.deposited(USER), 600);
        assertEq(vault.deposited(address(this)), 0);
        assertEq(vault.totalDeposited(), 600);
    }
    ...
    function test_composedDepositUsesOuterCallerAndOnlyVaultAllowance() public {
        ...
        vm.prank(USER);
        allocationManager.depositForCaller(vault, 700);

        assertEq(vault.deposited(USER), 700);
    ...
    function test_publicCannotUseDepositForOrReplaceTheCreditedUser() public {
        vm.expectRevert();
        vm.prank(USER);
        vault.depositStockFor(address(0xB0B), 1);
        assertEq(vault.deposited(address(0xB0B)), 0);
    }
    ...
    function test_feeOnTransferAndPositiveRebaseDeltasAreRejectedAtomically() public {
        ...
        UserStockVaultDepositsHarness feeVault = _deployAndRegisterNewAsset(address(feeToken), keccak256("fee"));
        ...
        vm.prank(USER);
        feeVault.depositStock(1_000);
        assertEq(feeToken.balanceOf(USER), 1_000);
        assertEq(feeVault.totalDeposited(), 0);

        MockPositiveRebaseStockToken rebaseToken = new MockPositiveRebaseStockToken();
        UserStockVaultDepositsHarness rebaseVault =
            _deployAndRegisterNewAsset(address(rebaseToken), keccak256("rebase"));
        ...
        vm.prank(USER);
        rebaseVault.depositStock(1_000);
        assertEq(rebaseToken.balanceOf(USER), 1_000);
        assertEq(rebaseVault.totalDeposited(), 0);
    }
    ...
    function test_falseNoDataMalformedAndRevertingTransferFromAreRejected() public {
        ...
        UserStockVaultDepositsHarness anomalyVault = _deployAndRegisterNewAsset(address(anomaly), keccak256("anomaly"));

        vm.expectRevert(
            abi.encodeWithSelector(UserStockVaultDeposits.InvalidStockTransferReturn.selector, address(anomaly))
        );
        vm.prank(USER);
        anomalyVault.depositStock(1);
    ...
    function test_transferCallbackCannotReenterOrLeavePartialCredit() public {
        ...
        UserStockVaultDepositsHarness callbackVault =
            _deployAndRegisterNewAsset(address(callbackToken), keccak256("callback"));
        ...
        vm.prank(USER);
        callbackVault.depositStock(100);
    ...
    function _deployAndRegister(address token) private returns (UserStockVaultDepositsHarness deployedVault) {
        _configureRegisterRole();
        deployedVault = new UserStockVaultDepositsHarness(
            address(registry), address(marketRegistry), address(allocationManager), ASSET_UID, token
        );
        registry.registerAsset(ASSET_UID, token, 18, address(deployedVault));
    }
    ...
    {
        deployedVault = new UserStockVaultDepositsHarness(
            address(registry), address(marketRegistry), address(allocationManager), assetUid, token
        );
        registry.registerAsset(assetUid, token, 18, address(deployedVault));
    }
