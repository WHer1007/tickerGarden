...
contract GraduationExecutorEntryTest is Test {
    ...
    function setUp() public {
        ...
        registry.configure(MARKET_ID, address(curve), 1, 0, 7, SWEPT_AT);
    }
    ...
    function test_onlySweptActiveMarketCanReachGraduationAlgorithm() public {
        registry.configure(MARKET_ID, address(curve), 0, 0, 7, 0);
        vm.expectRevert(
            abi.encodeWithSelector(
                GraduationExecutorEntry.GraduationNotRetryable.selector, MARKET_ID, uint8(0), uint8(0)
            )
        );
        curve.graduate(executor, MARKET_ID);

        registry.configure(MARKET_ID, address(curve), 1, 1, 7, SWEPT_AT);
        vm.expectRevert(
            abi.encodeWithSelector(
                GraduationExecutorEntry.GraduationNotRetryable.selector, MARKET_ID, uint8(1), uint8(1)
            )
        );
        curve.graduate(executor, MARKET_ID);
        assertEq(executor.attemptCount(), 0);
