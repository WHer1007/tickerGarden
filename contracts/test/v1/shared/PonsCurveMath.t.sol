...
import {PonsCurveMath} from "../../../src/v1/libraries/PonsCurveMath.sol";

contract PonsCurveMathHarness {
    function amountOut(uint256 amountIn_, uint256 reserveIn, uint256 reserveOut, uint256 feeBps)
        external
        pure
        returns (uint256)
    {
        return PonsCurveMath.amountOut(amountIn_, reserveIn, reserveOut, feeBps);
    }
    ...
    function amountIn(uint256 amountOut_, uint256 reserveIn, uint256 reserveOut, uint256 feeBps)
        ...
        returns (uint256)
    {
        return PonsCurveMath.amountIn(amountOut_, reserveIn, reserveOut, feeBps);
    }
    ...
    ) external pure returns (PonsCurveMath.BuyQuote memory) {
        return PonsCurveMath.quoteBuy(
    ...
    function quoteSell(
        ...
        uint256 additionalQuoteFeeBps,
        uint256 minQuoteOut
    ) external pure returns (PonsCurveMath.SellQuote memory) {
        return PonsCurveMath.quoteSell(tokensIn, tokenReserve, quoteReserve, feeBps, additionalQuoteFeeBps, minQuoteOut);
    }

    function proportionalSlippagePass(uint256 a, uint256 b, uint256 c, uint256 d) external pure returns (bool) {
        return PonsCurveMath.proportionalSlippagePass(a, b, c, d);
    }
}

contract PonsCurveMathTest is Test {
    PonsCurveMathHarness internal harness;

    function setUp() public {
        harness = new PonsCurveMathHarness();
    }
    ...
    function test_buyChargesFeeBeforePricing() public view {
        PonsCurveMath.BuyQuote memory quote = harness.quoteBuy(100, 1_000, 1_000_000, 1, 100, 0, 0);
    ...
    function test_buyFloorsTwoFeeLegsSeparately() public view {
        PonsCurveMath.BuyQuote memory quote = harness.quoteBuy(1_000, 1_000, 1_000_000, 1, 100, 50, 0);
    ...
    function test_sellChargesFeesAfterGrossPricing() public view {
        PonsCurveMath.SellQuote memory quote = harness.quoteSell(500_000, 1_000_000, 1_000, 100, 50, 0);
    ...
    function test_tailQuoteRepricesFeesAndReturnsUnusedQuote() public view {
        PonsCurveMath.BuyQuote memory quote = harness.quoteBuy(1_000, 1_000, 400_000, 333_333, 100, 50, 100_000);
    ...
    function test_tailQuoteUsesProportionalSlippage() public view {
        PonsCurveMath.BuyQuote memory quote = harness.quoteBuy(1_000, 1_000, 400_000, 333_333, 100, 50, 400_000);
        assertTrue(quote.partialFill);
        assertFalse(quote.slippagePass);
    }

    function test_quoteAndExecutionMathUseTheSameResultPath() public view {
        PonsCurveMath.BuyQuote memory preview = harness.quoteBuy(777, 5_000, 2_000_000, 100, 123, 77, 1);
        PonsCurveMath.BuyQuote memory execution = harness.quoteBuy(777, 5_000, 2_000_000, 100, 123, 77, 1);
        assertEq(keccak256(abi.encode(preview)), keccak256(abi.encode(execution)));

        PonsCurveMath.SellQuote memory sellPreview = harness.quoteSell(555, 2_000_000, 5_000, 123, 77, 1);
        PonsCurveMath.SellQuote memory sellExecution = harness.quoteSell(555, 2_000_000, 5_000, 123, 77, 1);
    ...
    function test_invalidAmountsReservesAndFeesFailClosed() public {
        vm.expectRevert(abi.encodeWithSelector(PonsCurveMath.InvalidAmountIn.selector, uint256(0)));
        harness.amountOut(0, 1, 1, 0);

        vm.expectRevert(abi.encodeWithSelector(PonsCurveMath.InvalidAmountOut.selector, uint256(0)));
        harness.amountIn(0, 1, 2, 0);

        vm.expectRevert(abi.encodeWithSelector(PonsCurveMath.InvalidReserves.selector, uint256(1), uint256(1)));
        harness.amountIn(1, 1, 1, 0);

        vm.expectRevert(abi.encodeWithSelector(PonsCurveMath.InvalidFeeBps.selector, uint256(10_000)));
        harness.amountOut(1, 1, 2, 10_000);

        vm.expectRevert(
            abi.encodeWithSelector(PonsCurveMath.InvalidCombinedFeeBps.selector, uint256(9_999), uint256(1))
        );
        harness.quoteBuy(10_000, 1, 10_000, 1, 9_999, 1, 0);
    }

    function test_zeroRoundedOutputAndEmptySellableLiquidityFailClosed() public {
        vm.expectRevert(PonsCurveMath.AmountOutRoundsToZero.selector);
        harness.amountOut(1, type(uint128).max, 1, 0);

        vm.expectRevert(
            abi.encodeWithSelector(PonsCurveMath.InsufficientSellableLiquidity.selector, uint256(100), uint256(100))
        );
        harness.quoteBuy(1, 1, 100, 100, 0, 0, 0);
    }

    function test_scaledIntermediateOverflowFailsClosed() public {
        vm.expectRevert(PonsCurveMath.ArithmeticOverflow.selector);
    ...
    function testFuzz_amountInUsesPonsFloorPlusOneAndMeetsDesiredOutput(
        ...
        if (required > 1) {
            try harness.amountOut(required - 1, reserveIn, reserveOut, feeBps) returns (uint256 previous) {
                // Pons always adds one even when the inverse division is exact, so equality is permitted here.
                assertLe(previous, desired);
            } catch (bytes memory reason) {
                assertEq(bytes4(reason), PonsCurveMath.AmountOutRoundsToZero.selector);
            }
        }
    }
}
