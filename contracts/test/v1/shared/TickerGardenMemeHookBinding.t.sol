...
contract TickerGardenMemeHookBindingTest is Test {
    ...
    function test_constructorRejectsWrongAddressBitsBeforeAcceptingDependencies() public {
        bytes memory initCode =
            _initCode(address(registry), address(poolManager), address(feeVault), address(graduation));
        bytes32 salt;
    ...
    function test_constructorRejectsNoCodeAndAliasedDependenciesAtAValidHookAddress() public {
        bytes memory noCodeInit =
            _initCode(address(0xBAD), address(poolManager), address(feeVault), address(graduation));
        bytes32 noCodeSalt = _mineSalt(keccak256(noCodeInit));
        vm.expectRevert();
        deployer.deploy(noCodeInit, noCodeSalt);

        bytes memory aliasInit =
            _initCode(address(registry), address(poolManager), address(poolManager), address(graduation));
    ...
    function test_wrongSourceVersionOrLaunchPhaseCannotRegister() public {
        vm.expectRevert(
    ...
    function _configure(uint8 launchPhase, bytes32 poolId, PoolKey memory canonicalKey) private {
        _configure(launchPhase, poolId, canonicalKey, 1);
    }

    function _configure(uint8 launchPhase, bytes32 poolId, PoolKey memory canonicalKey, uint32 sourceVersion) private {
    ...
    function _deployHook(address registry_, address poolManager_, address feeVault_, address graduation_)
        private
        returns (TickerGardenMemeHookBindingHarness result)
    {
        bytes memory initCode = _initCode(registry_, poolManager_, feeVault_, graduation_);
        bytes32 salt = _mineSalt(keccak256(initCode));
        result = TickerGardenMemeHookBindingHarness(deployer.deploy(initCode, salt));
    }

    function _initCode(address registry_, address poolManager_, address feeVault_, address graduation_)
        private
        pure
        returns (bytes memory)
    {
        return abi.encodePacked(
            ...
            abi.encode(registry_, poolManager_, feeVault_, graduation_)
        );
    }
