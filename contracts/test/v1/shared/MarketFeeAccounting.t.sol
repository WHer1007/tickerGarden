...
contract MarketFeeAccountingTest is Test {
    ...
    function test_zeroActiveStockProducesExactFortyZeroFortyTwentySplit() public view {
        MarketFeeAccounting.V4Buckets memory buckets = accounting.splitV4(10 ether, 2 ether, 8 ether, 0);
        assertEq(buckets.creatorAmount, 4 ether);
        assertEq(buckets.stakerAmount, 0);
        assertEq(buckets.platformAmount, 4 ether);
    }

    function test_oneActiveStockProducesFixedFortyTwentyTwentySplit() public view {
        MarketFeeAccounting.V4Buckets memory buckets = accounting.splitV4(10 ether, 2 ether, 8 ether, 1);
        assertEq(buckets.creatorAmount, 2 ether);
        assertEq(buckets.stakerAmount, 4 ether);
        assertEq(buckets.platformAmount, 2 ether);
    }

    function test_maximumActiveStockProducesSameFixedSplit() public view {
        MarketFeeAccounting.V4Buckets memory atCap = accounting.splitV4(10 ether, 2 ether, 8 ether, 1);
        MarketFeeAccounting.V4Buckets memory aboveCap =
            accounting.splitV4(10 ether, 2 ether, 8 ether, type(uint256).max);
    ...
    function test_allIntegerRemaindersGoToPlatform() public view {
        MarketFeeAccounting.V4Buckets memory buckets = accounting.splitV4(7, 1, 6, 1);
    ...
    function test_eachApprovedStockDecimalsHasExactZeroAndPositiveActiveVectors() public view {
        for (uint8 decimals = 6; decimals <= 18; ++decimals) {
            ...
            assertEq(zero.creatorAmount, nonLpAmount / 2);
            assertEq(zero.platformAmount, nonLpAmount - zero.creatorAmount);
            assertEq(one.stakerAmount, nonLpAmount / 2);
            assertEq(one.creatorAmount + one.platformAmount, nonLpAmount / 2);
        ...
        view
    {
        ...
        uint256 expectedStaker = activeStock == 0 ? 0 : nonLpAmount / 2;
