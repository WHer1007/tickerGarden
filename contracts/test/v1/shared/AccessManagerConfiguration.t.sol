...
contract AccessManagerConfigurationTest is Test {
    ...
    function test_handoffLeavesOnlyFrozenSafeRolesAndLocksGlobalAdminSurface() public {
        ...
        vm.expectRevert();
        manager.setRoleGuardian(UNPAUSE_ROLE, PROTOCOL_ADMIN_ROLE);
    ...
    function test_guardianCancelsDelayedUnpauseAndGovernanceRetainsOnlyDelayedConfiguration() public {
        bytes memory unpauseData = abi.encodeCall(target.unpauseAction, ());
        uint48 unpauseReadyAt = uint48(block.timestamp + OPERATIONS_DELAY);
        vm.prank(SECURITY_SAFE);
        manager.schedule(address(target), unpauseData, unpauseReadyAt);
        vm.prank(GUARDIAN_SAFE);
        manager.cancel(SECURITY_SAFE, address(target), unpauseData);
        vm.warp(unpauseReadyAt);
        vm.prank(SECURITY_SAFE);
        vm.expectRevert();
        manager.execute(address(target), unpauseData);
