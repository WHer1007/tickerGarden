...
contract TickerGardenSupplyMathHarness {
    TickerGardenSupplyMath.TrackedReserves internal _state;
    ...
    function initialize(uint256 supply, uint256 phantomQuote, uint256 graduationThreshold) external {
        _state = TickerGardenSupplyMath.initialize(supply, phantomQuote, graduationThreshold);
    }
    ...
    {
        return TickerGardenSupplyMath.supplyPartition(supply, phantomQuote, graduationThreshold);
    }
    ...
    function graduationPartition(uint256 sweptTokens, uint256 sweptQuote, uint256 phantomQuote)
        ...
        returns (uint256, uint256)
    {
        return TickerGardenSupplyMath.graduationPartition(sweptTokens, sweptQuote, phantomQuote);
    }
    ...
    function canonicalGraduationQuote(uint256 supply, uint256 phantomQuote, uint256 graduationThreshold)
        ...
        returns (uint256)
    {
        return TickerGardenSupplyMath.canonicalGraduationQuote(supply, phantomQuote, graduationThreshold);
    }

    function sellableTokens() external view returns (uint256) {
        return TickerGardenSupplyMath.sellableTokens(_state);
    }

    function pricingReserves(uint256 phantomQuote) external view returns (uint256, uint256) {
        return TickerGardenSupplyMath.pricingReserves(_state, phantomQuote);
    }

    function state() external view returns (TickerGardenSupplyMath.TrackedReserves memory) {
        return _state;
    }
}

contract TickerGardenSupplyMathTest is Test {
    ...
    TickerGardenSupplyMathHarness internal harness;

    function setUp() public {
        harness = new TickerGardenSupplyMathHarness();
    }

    function test_nativeSupplyPartitionMatchesFrozenTickerGardenVector() public view {
    ...
    function test_usdgSupplyPartitionMatchesFrozenTickerGardenVector() public view {
        (uint256 reserved, uint256 sellable) = harness.supplyPartition(SUPPLY, 3_236_000_000, 8_090_000_000);
        assertEq(reserved, RESERVED);
        assertEq(sellable, SELLABLE);
    }

    function test_graduationPartitionMatchesFrozenTickerGardenVector() public view {
    ...
    function test_canonicalGraduationQuoteUsesCeilingRatherThanUnconditionalPlusOne() public view {
        ...
        // TickerGardenCurveMath.amountIn deliberately returns floor(...)+1 (=5), which is suitable for exact-output
    ...
    function test_initialTrackedAccountingEqualsSupplyAndNeverReadsForcedBalances() public {
        ...
        TickerGardenSupplyMath.TrackedReserves memory state = harness.state();
    ...
    function test_trackedTokensCannotCrossReservedFloor() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(TickerGardenSupplyMath.TrackedTokensBelowReserve.selector, RESERVED - 1, RESERVED)
        );
    ...
    function _expectInvalidSupply(uint256 supply, uint256 phantom, uint256 threshold) private {
        vm.expectRevert(
            abi.encodeWithSelector(TickerGardenSupplyMath.InvalidSupplyPartition.selector, supply, phantom, threshold)
        );
