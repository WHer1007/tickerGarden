...
} from "../../../script/v1/V1RobinhoodMainnetDeploymentOrchestrator.sol";
...
contract V1RobinhoodMainnetDeploymentOrchestratorTest is Test {
    ...
    function test_stagedGraphHasExactBindingsAndBoundedTransactions() public {
        ...
        for (uint8 i; i < 16; ++i) {
            ...
            vm.expectRevert();
            uint256 componentGas = gasleft();
            orchestrator.deployComponent(i, codes[i]);
            assertLt(componentGas - gasleft(), 28_000_000, "component execution gas budget");
            vm.expectRevert();
        ...
        assertLt(finishGas - gasleft(), 28_000_000, "finish execution gas budget");
        vm.expectRevert();
        orchestrator.finish(payload);
        vm.expectRevert();
        orchestrator.begin(plan.payloadHash, hashes, keccak256(abi.encode(payload)));
    ...
    function test_testnetRejected() public {
        vm.chainId(46630);
        vm.expectRevert();
        new V1DeterministicDeploymentOrchestrator(address(this), RELEASE_ID);
    }
