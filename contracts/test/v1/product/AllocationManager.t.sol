...
contract AllocationManagerTest is Test {
    ...
    function setUp() public {
        ...
        marketRegistry.configure(MARKET_ID, ASSET_UID, address(gauge));
    ...
    function test_pausedAndRetiredAssetStillAllowWholeClose() public {
        for (uint8 assetStatus = 2; assetStatus <= 3; ++assetStatus) {
            address user = address(uint160(0xCAFE + assetStatus));
            _seedAllocationFor(user, 1 ether);
            vm.warp(gauge.positionOf(user).unlockAt);
            officialRegistry.setStatus(ASSET_UID, assetStatus);
