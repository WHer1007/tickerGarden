...
import {
    ...
    MarketRegistryQuoteMock,
    MarketRegistryBaselineMock,
    MarketRegistryTemplateMock
} from "./MarketRegistryV1.t.sol";
...
contract C405CurveAutonomyInvariantTest is Test {
    ...
    MarketRegistryStockMock internal assets;
    MarketRegistryQuoteMock internal quotes;
    MarketRegistryBaselineMock internal baselines;
    MarketRegistryTemplateMock internal templates;
    ...
    function test_nativeCurveBalancesRemainTradableBecauseInterventionSelectorsAreAbsent() public {
        ...
        _assertInterventionSelectorsAbsent(NATIVE_MARKET_ID);
        ...
        assertEq(registry.market(NATIVE_MARKET_ID).runtime.launchPhase, 0);
    }

    function test_erc20CurveBalancesRemainTradableBecauseInterventionSelectorsAreAbsent() public {
        ...
        _assertInterventionSelectorsAbsent(ERC20_MARKET_ID);
        ...
        assertGt(curve.realQuoteReserve(), trackedQuote);
        assertEq(registry.market(ERC20_MARKET_ID).runtime.launchPhase, 0);
    }
    ...
    function _assertInterventionSelectorsAbsent(bytes32 marketId) internal {
        (bool paused,) =
            address(registry).call(abi.encodeWithSignature("setMarketPaused(bytes32,bytes32)", marketId, bytes32(0)));
        (bool retired,) =
            address(registry).call(abi.encodeWithSignature("setMarketRetired(bytes32,bytes32)", marketId, bytes32(0)));
        (bool emergency,) = address(registry)
            .call(
                abi.encodeWithSignature(
                    "commitEmergencyExit(bytes32,uint64,bytes32)", marketId, uint64(block.number), bytes32(0)
                )
            );
        assertFalse(paused);
        assertFalse(retired);
