-- Event-only display facts. Never financial reconciliation or publication evidence.
-- +goose Up
CREATE TABLE tickergarden.event_projection_checkpoints (
...
CREATE TABLE tickergarden.event_projection_inputs (
 chain_id bigint NOT NULL REFERENCES tickergarden.event_projection_checkpoints(chain_id),
...
CREATE TABLE tickergarden.event_projection_rows (
 chain_id bigint NOT NULL REFERENCES tickergarden.event_projection_checkpoints(chain_id),
 table_name text NOT NULL CHECK(table_name IN ('events','configs','markets','pools','poolEvents','curveTrades','stockPositions','allocations','activationBuckets','gaugePositions','rewardExits','feeCredits','feeClaims','swaps','observations','feeClaimTotals')),
...
CREATE VIEW tickergarden.canonical_event_projection_rows AS
...
-- +goose Down
DROP VIEW tickergarden.canonical_event_projection_rows;
DROP TABLE tickergarden.event_projection_rows;
DROP TABLE tickergarden.event_projection_inputs;
DROP TABLE tickergarden.event_projection_checkpoints;
