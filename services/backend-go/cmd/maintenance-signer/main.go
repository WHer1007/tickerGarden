...
func run(args []string, out io.Writer) error {
	if len(args) != 1 {
		return errors.New("usage: maintenance-signer --describe | --once | --run")
	}
	if args[0] == "--describe" {
		return json.NewEncoder(out).Encode(map[string]any{"service": "maintenance-signer", "implemented": true, "durableScheduling": true, "signingImplemented": true, "transactionSubmission": false, "signerConfigured": false})
	}
	...
	signerPath := os.Getenv("TG_MAINTENANCE_SIGNER")
	if !filepath.IsAbs(signerPath) {
		return errors.New("TG_MAINTENANCE_SIGNER must be an absolute executable path")
	}
	info, e := os.Stat(signerPath)
	if e != nil || !info.Mode().IsRegular() || info.Mode().Perm()&0111 == 0 {
		return errors.New("TG_MAINTENANCE_SIGNER must reference an executable regular file")
	}
	...
	defer pool.Close()
	worker := maintenance.SigningWorker{Store: maintenance.Store{Pool: pool, ChainID: cfg.ChainID}, Signer: maintenance.ExecSigner{Path: signerPath}, From: os.Getenv("TG_MAINTENANCE_FROM"), GenesisHash: manifest.GenesisHash}
	for {
		...
		if e != nil {
			...
			if args[0] == "--once" {
				if result.Action != "idle" && result.Status != "signed_stored" {
					return errors.New("maintenance signature requires attention; check status before recovery")
				}
				return nil
			}
		}
