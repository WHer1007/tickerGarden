...
func runAuthorization(out io.Writer, path, from string, request deployment.MaintenanceRequest, digest, owner, token string, generation int64, recoveryID string) error {
	cfg, e := config.Load()
	if e != nil {
		return e
	}
	...
	result, e := (maintenance.Store{Pool: pool, ChainID: cfg.ChainID}).RecoverAuthorization(ctx, rpc, preview, digest, owner, token, generation, recoveryID)
	if e != nil {
		return e
	}
	return json.NewEncoder(out).Encode(map[string]any{"authorizationRecovery": result, "transactionSubmission": false, "executionComplete": false})
}
