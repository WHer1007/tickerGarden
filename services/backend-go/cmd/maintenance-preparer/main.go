...
func run(args []string, out io.Writer) error {
	if len(args) != 1 {
		return errors.New("usage: maintenance-preparer --describe | --once | --run")
	}
	if args[0] == "--describe" {
		return json.NewEncoder(out).Encode(map[string]any{"service": "maintenance-preparer", "implemented": true, "durableScheduling": true, "atomicPreparation": true, "transactionSubmission": false, "signerConfigured": false})
	}
	...
	defer pool.Close()
	worker := maintenance.Preparer{Store: maintenance.Store{Pool: pool, ChainID: cfg.ChainID}, RPC: rpc, Manifest: manifest, From: os.Getenv("TG_MAINTENANCE_FROM"), Fees: maintenance.Fees{GasLimit: os.Getenv("TG_MAINTENANCE_GAS_LIMIT"), MaxFeePerGas: os.Getenv("TG_MAINTENANCE_MAX_FEE_PER_GAS"), MaxPriorityFeePerGas: os.Getenv("TG_MAINTENANCE_PRIORITY_FEE_PER_GAS")}}
	for {
