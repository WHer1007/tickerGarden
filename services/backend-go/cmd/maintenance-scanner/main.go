...
func run(args []string, out io.Writer) error {
	if len(args) != 1 {
		return errors.New("usage: maintenance-scanner --describe | --once | --run")
	}
	if args[0] == "--describe" {
		return json.NewEncoder(out).Encode(map[string]any{"service": "maintenance-scanner", "implemented": true, "durableScheduling": true, "marketDiscovery": true, "accountDiscovery": true, "transactionSubmission": false, "signerConfigured": false})
	}
	...
	defer pool.Close()
	worker := maintenance.Scanner{Store: maintenance.Store{Pool: pool, ChainID: cfg.ChainID}, RPC: rpc, Manifest: manifest, From: os.Getenv("TG_MAINTENANCE_FROM")}
	for {
