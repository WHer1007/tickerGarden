...
func run() error {
	if len(os.Args) == 2 && os.Args[1] == "--describe" {
		return json.NewEncoder(os.Stdout).Encode(map[string]any{"service": "event-worker", "implemented": true, "stage": "finalized-event-display-facts", "businessFactProjectionImplemented": true, "blockObservationScope": deployment.HolderObservationScope, "activityBackfillImplemented": false, "financialReconciliation": false, "publicationEligible": false, "automaticReconciliationImplemented": false, "transactionSubmission": false})
	}
	if len(os.Args) != 2 || (os.Args[1] != "--once" && os.Args[1] != "--run") {
		return errors.New("usage: event-worker --describe | --once | --run")
	}
	cfg, err := config.Load()
	if err != nil {
		return err
	...
	dsn := os.Getenv("TG_EVENT_DATABASE_URL")
	if dsn == "" {
		return errors.New("TG_EVENT_DATABASE_URL is required")
	}
	start, err := strconv.ParseUint(os.Getenv("TG_EVENT_START_BLOCK"), 10, 63)
	if err != nil {
		return errors.New("TG_EVENT_START_BLOCK is required and must be a nonnegative integer")
	}
	endpoint := os.Getenv("TG_EVENT_RPC_URL")
	if endpoint == "" {
	...
	worker := projector.Worker{Pool: pool, RPC: rpc, Manifest: manifest, StartBlock: start, EventsOnly: true}
	for {
