...
func run(args []string, out io.Writer) error {
	if len(args) != 1 {
		return errors.New("usage: maintenance-submitter --describe | --once | --run")
	}
	if args[0] == "--describe" {
		return json.NewEncoder(out).Encode(map[string]any{"service": "maintenance-submitter", "implemented": true, "durableScheduling": true, "transactionSubmission": true, "executionComplete": false})
	}
	...
	defer pool.Close()
	worker := maintenance.SubmissionWorker{Store: maintenance.Store{Pool: pool, ChainID: cfg.ChainID}, RPC: rpc, From: os.Getenv("TG_MAINTENANCE_FROM"), Manifest: manifest}
	for {
		...
		if e != nil {
			...
			if err := json.NewEncoder(out).Encode(map[string]any{"action": "unavailable", "executionComplete": false}); err != nil {
				return err
			}
		} else {
			...
			if args[0] == "--once" {
				if result.Action != "idle" && result.Status != "acknowledged" {
					return errors.New("maintenance submission requires attention; inspect persisted outcome before recovery")
				}
				return nil
			}
		}
