...
export interface ReadModelRepository {
  atRevision?(revision: string): ReadModelRepository | undefined;
...
export interface VerifiedReadModelSnapshot {
  readonly executionSpecId: "V1-EXEC-11";
...
function validateSource(source: MarketReadModel["source"], sync: SyncStatus): void {
  if (source.chainId !== sync.chainId) throw new Error("source chain does not match snapshot chain");
...
function validateMarket(market: MarketReadModel, sync: SyncStatus): void {
  ...
  assertHex(market.quoteAssetConfigId, 32, "quoteAssetConfigId"); assertHex(market.tickerGardenBaselineId, 32, "tickerGardenBaselineId");
  ...
  if (market.identity !== undefined) {
    const identity = market.identity;
    if (!identity || typeof identity !== "object") throw new Error("invalid market identity");
    if (typeof identity.blockNumber !== "string" || typeof identity.deployedAt !== "string") throw new Error("invalid identity integer");
    assertUint(identity.blockNumber,"identity.blockNumber"); assertUint(identity.deployedAt,"identity.deployedAt");
    assertHex(identity.blockHash,32,"identity.blockHash"); assertHex(identity.runtimeCodeHash,32,"identity.runtimeCodeHash");
    if (BigInt(identity.blockNumber)>BigInt(market.source.blockNumber) || BigInt(identity.blockNumber)>9223372036854775807n || BigInt(identity.deployedAt)>9223372036854775807n || (identity.blockNumber===market.source.blockNumber && identity.blockHash!==market.source.blockHash)) throw new Error("identity creation source mismatch");
    for(const [value,max] of [[identity.name,4096],[identity.symbol,4096],[identity.metadataURI,16384]] as const){if(typeof value!=="string" || new TextEncoder().encode(value).length>max) throw new Error("invalid identity display field");}
  }
...
export class InMemoryReadModelRepository implements ReadModelRepository {
  ...
  constructor(input: VerifiedReadModelSnapshot) {
    if (input.executionSpecId !== "V1-EXEC-11" || input.reconciliationAlerts.length !== 0) throw new Error("read model snapshot is not V1-reconciled");
    if (![4663, 46630, 421614].includes(input.sync.chainId)) throw new Error("unsupported snapshot chain");
...
export function createEmptyRepository(chainId: 4663 | 46630 | 421614 = 4663): InMemoryReadModelRepository {
  return new InMemoryReadModelRepository({
  executionSpecId: "V1-EXEC-11", reconciliationAlerts: [],
  sync: { chainId, status: "unavailable", blockNumber: null, blockHash: null, finality: "unavailable", headBlockNumber: null, headBlockHash: null, lagBlocks: null, revision: "empty" },
});

}

export const EMPTY_REPOSITORY = createEmptyRepository();
