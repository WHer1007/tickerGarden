...
function applyObservations(state: V1IndexerState, event: DecodedV1Event, at: Provenance): void {
  for (const observation of event.observations ?? []) {
    ...
    if (observation.kind === "asset" || observation.kind === "assetIdentity" || observation.kind === "quote" || observation.kind === "baseline" || observation.kind === "template") {
...
export function applyV1Event(state: V1IndexerState, event: DecodedV1Event): "applied" | "duplicate" {
  ...
  if (event.signature === "StockQuoteConfigBound(bytes32,bytes32,address,bytes32,bytes32,bytes32)") {
    const old = state.configs.get(key("quote", event.args.configId));
    if (!old) throw new Error(`cannot bind unknown quote config ${event.args.configId}`);
    if (
      typeof old.values.quoteAsset !== "string" ||
      lower(old.values.quoteAsset) !== lower(event.args.quoteAsset)
    ) {
      throw new Error(`stock quote binding asset mismatch for ${event.args.configId}`);
    }
  }
  if (
  ...
  switch (event.signature) {
    ...
    case "StockQuoteConfigBound(bytes32,bytes32,address,bytes32,bytes32,bytes32)": {
      const k = key("quote", event.args.configId); const old = state.configs.get(k)!;
      state.configs.set(k, {
        ...old,
        values: { ...old.values, ...event.args },
        provenance: at,
      });
      break;
    }
    ...
    case "TickerGardenBaselineAdded(bytes32,bytes32,bytes32)":
      state.configs.set(key("baseline", event.args.baselineId), { kind: "baseline", id: lower(event.args.baselineId), status: 1n, values: values(event.args), provenance: at });
      break;
    case "TickerGardenBaselineStatusChanged(bytes32,uint8,uint8,bytes32)": {
      const k = key("baseline", event.args.baselineId); const old = state.configs.get(k);
      state.configs.set(k, { kind: "baseline", id: lower(event.args.baselineId), status: event.args.newStatus, values: { ...old?.values, reasonHash: event.args.reasonHash }, provenance: at });
      break;
    }
    ...
    case "RewardConverted(bytes32,address,uint32,uint256,uint256)":
      // creatorEpoch zero identifies the staker conversion path. Creator conversions
      // have no user Gauge position to mutate; their event remains in the provenance log.
      if (event.args.creatorEpoch === 0n) {
        mergePosition(state.gaugePositions, gaugeKey(event.args.user, event.args.marketId), {
          ...event.args,
          conversionType: "staker",
          lastEvent: event.signature,
        }, at);
      }
      break;
    case "RewardBatchConverted(bytes32,uint256,address,address,uint256,uint256)":
      break;
    case "RawRewardExitRequested(bytes32,address,uint256)":
      mergePosition(state.rewardExits, gaugeKey(event.args.user, event.args.marketId), {
        ...event.args,
        rawRewardExitRequested: true,
        rawRewardExitStatus: "requested",
        lastEvent: event.signature,
      }, at);
      break;
    case "RawRewardExitCancelled(bytes32,address)":
      mergePosition(state.rewardExits, gaugeKey(event.args.user, event.args.marketId), {
        ...event.args,
        availableAt: 0n,
        rawRewardExitRequested: false,
        rawRewardExitStatus: "cancelled",
        lastEvent: event.signature,
      }, at);
      break;
    ...
    case "FeeClaimed(uint8,address,bytes32,uint32,address,uint256)": {
      put(state.feeClaims, at.eventKey, event.args, at);
      const a = event.args;
      const claimKey = key(a.marketId, a.feeAsset, a.beneficiaryType, a.beneficiary, a.beneficiaryEpoch);
      const previous = state.feeClaimTotals.get(claimKey)?.values;
      put(state.feeClaimTotals, claimKey, {
        marketId:a.marketId, feeAsset:a.feeAsset, beneficiaryType:a.beneficiaryType,
        beneficiary:a.beneficiary, beneficiaryEpoch:a.beneficiaryEpoch,
        claimedAmount:BigInt(previous?.claimedAmount as bigint ?? 0n)+a.amount,
        claimCount:BigInt(previous?.claimCount as bigint ?? 0n)+1n,
        firstClaimEventKey:previous?.firstClaimEventKey ?? at.eventKey,
