...
const rewardValues = object({claimStatus:{description:"Observed reward and lock conditions only; candidate does not certify solvency, simulation or successful payment.",type:"string",enum:["candidate","no_rewards","rage_quit_pending","position_locked","raw_exit_required","raw_exit_waiting"]},claimCandidateAmount:{...uintString,description:"Original fee-asset unpaid amount only when claimStatus is candidate; otherwise zero."},positionUnlockAt:uintString,positionLockSatisfied:{type:"boolean",description:"Gauge principal lock condition only; true for Creator. Other claim conditions are reported separately."},conversionStatus:{type:"string",enum:["not_applicable","no_rewards","not_graduated","rage_quit_pending","raw_exit_ready","candidate"]},conversionCandidateAmount:uintString,marketId:bytes32,feeAsset:address,assetKind:{type:"string",enum:["quote","meme"]},beneficiaryType:{type:"string",enum:["0","1"]},beneficiary:address,beneficiaryEpoch:uintString,unpaidAmount:uintString,observedClaimedAmount:uintString,observedClaimCount:uintString,firstClaimEventKey:nullable({type:"string"}),rawRewardExitAt:uintString,rawRewardExitReady:{type:"boolean"},observedAtTimestamp:uintString,historyComplete:{type:"boolean",const:false},publicationEligible:{type:"boolean",const:false}});
...
const spec = {
  openapi: "3.1.0",
  info: { title: "TickerGarden V1 Read API", version: "2.27.0", description: "Non-custodial read API with published snapshots and explicitly incomplete finalized reward observations." },
  ...
  paths: {
    ...
    "/v1/stats/holders":{get:{operationId:"getGlobalHolderCounts",description:"Go backend. Full finalized market histories in one snapshot. Global and per-STOCK positive addresses are deduplicated; market-address pairs are separate. Adjusted counts exclude the union of known protocol addresses across all markets. These are addresses, not people or reward eligibility. At most 1000 markets and 1000000 positive market-address pairs; complete-history budgets and 10-second timeout apply. Missing or oversized histories return 503.",responses:{"200":response(ref("GlobalHolderCountsResponse")),"400":response(ref("CandleError")),"405":response(ref("CandleError")),"503":response(ref("CandleError"))}}},
    ...
    "/v1/assets/{assetUid}/statistics":{get:{operationId:"getAssetStatistics",description:"Go backend. Registered STOCK execution flows for [from,to), read in one verified database snapshot. Quote currencies and fee assets remain separate; includes internal conversions explicitly. This is not reserves, holder balances or USD value. Retired assets remain queryable. Maximum 1000 markets, 10000 executions per market and 100000 total; incomplete or oversized observations return 503, never truncated totals.",parameters:[{name:"assetUid",in:"path",required:true,schema:bytes32},{name:"from",in:"query",required:true,schema:uintString},{name:"to",in:"query",required:true,schema:uintString}],responses:{"200":response(ref("AssetStatisticsResponse")),"400":response(ref("CandleError")),"405":response(ref("CandleError")),"503":response(ref("CandleError"))}}},
    ...
    "/v1/markets/{marketId}/detail":{get:{operationId:"getTokenDetail",description:"Display-only approved token detail analytics. Prefers cached scheduled Dune results and falls back per section to finalized indexer coverage. Null means unavailable, never zero. Amounts are decimal strings; price and cap are Quote denominated. Protocol-balance exclusions define circulating supply. No execution or settlement authority.",parameters:[{name:"marketId",in:"path",required:true,schema:bytes32},{name:"period",in:"query",required:false,schema:{type:"string",enum:["1H","12H","1D"]}}],responses:{"200":response(ref("TokenDetailResponse")),"400":response(ref("CandleError")),"405":response(ref("CandleError")),"503":response(ref("CandleError"))}}},
    ...
    "/v1/prices/references":{get:{operationId:"listDisplayPriceReferences",description:"Go backend. Cached provider-reported display estimates only, never a chain snapshot, execution price or creation authorization. No query parameters. Stale/unavailable prices are null.",responses:{"200":response(ref("DisplayPriceResponse")),"400":response(ref("DisplayPriceError"),"Query parameters unsupported"),"405":response(ref("DisplayPriceError"),"Read only")}}},
    "/v1/users/{address}/rewards": {get:{operationId:"listUserRewards",description:"Go backend: current finalized observations, not a certified payout or conversion plan. Incomplete claim history is explicitly marked; unavailable data returns 503. Pagination must restart if the projection revision changes.",parameters:[{name:"address",in:"path",required:true,schema:address},...queryParameters],responses:{"200":response(ref("RewardPage")),"400":response(ref("RewardError"),"Invalid query or changed revision"),"405":response(ref("RewardError"),"Read only"),"503":response(ref("RewardError"),"Observations unavailable")}}},
    "/health": { get: { operationId: "getHealth", responses: { "200": response(ref("HealthResponse")), ...errors } } },
    "/v1/markets": { get: { operationId: "listMarkets", parameters: [{ name: "assetUid", in: "query", required: false, schema: bytes32 },
      ...
      { name: "sort", in: "query", required: false, schema: { type: "string", enum: ["marketId_asc", "marketId_desc", "createdAt_asc", "createdAt_desc", "name_asc", "launchPhase_asc", "volume24hUsd_desc", "marketCapUsd_desc"] }, description: "ID, deployment-time, name, lifecycle-phase, 24-hour USD volume or USD market-cap order, with ascending marketId as tie-breaker. USD sorts use one revision-cached complete directory and place unavailable values last. Filters apply to the full revision-pinned catalog before pagination. Cursors are bound to normalized filters and sort." }, ...queryParameters], responses: { "200": response(ref("MarketPage")), "503": response(ref("ApiErrorResponse"), "Market identity or ranking metrics unavailable"), ...errors } } },
    "/v1/markets/{marketId}": { get: { operationId: "getMarket", parameters: [{ name: "marketId", in: "path", required: true, schema: bytes32 }, revisionParameter], responses: { "200": response(ref("MarketDetailResponse")), "404": response(ref("ApiErrorResponse"), "Market not found"), ...errors } } },
  ...
  components: { schemas: {
    ...
    SnapshotUpdatesResponse:object({mode:{type:"string",enum:["unchanged","changed","reset"]},sync,invalidated:{type:"array",maxItems:4,uniqueItems:true,items:{type:"string",enum:["markets","configs","positions","accounts"]}},pollAfterMs:{type:"integer",const:5000}}),
    ...
    MarketReadModel: market, MarketIdentityReadModel: marketIdentity, MarketMetricsReadModel: marketMetrics, ConfigReadModel: config, QuoteClaimable: claimQuote, MemeClaimable: claimMeme,
