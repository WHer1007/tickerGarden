...
var (
	receiptJob       = "0x" + "11" + strings.Repeat("11", 31)
...
func validReceiptObservation() ReceiptObservation {
	...
	return ReceiptObservation{JobKey: receiptJob, TransactionHash: receiptTx, GenesisHash: receiptGen, Head: head, Finalized: finalized}
}
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				if !errors.Is(err, ErrUnavailable) {
					t.Fatalf("receiptStatus() error = %v, want ErrUnavailable", err)
				}
				return
			}
