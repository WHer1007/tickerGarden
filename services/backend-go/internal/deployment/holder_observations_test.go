...
func holderSetup(t *testing.T) (*feeFixture, chainrpc.Header, map[string]MarketDiscovery, string) {
	...
	f.calls[distributor+Hash([]byte("TWAB_SCHEMA()"))[:10]] = bytesWord(Hash([]byte("TRANSFER_LOG_TWAB_7D_V1")))
	...
	for field, value := range map[string]string{"finalityDelaySeconds": "1", "finalityDelayBlocks": "1", "rootPublicationWindow": "a", "rootReviewDelay": "a", "claimWindow": "64"} {
		f.calls[distributor+Hash([]byte(field + "()"))[:10]] = bytesWord(value)
	}
	f.calls[distributor+Hash([]byte("rootServiceTreasury()"))[:10]] = addrWord("0x" + strings.Repeat("7", 40))
...
func TestHolderInvalidInputsReturnNoPartialBatch(t *testing.T) {
	for _, name := range []string{"registry", "timing zero", "timing block bound", "treasury missing", "treasury zero", "treasury self", "token binding", "token code", "treasury code", "vault", "market", "status", "epoch budget", "late balance", "native malformed", "reorg"} {
		t.Run(name, func(t *testing.T) {
			...
			switch name {
			case "timing zero":
				f.calls[d+Hash([]byte("rootReviewDelay()"))[:10]] = bytesWord("0")
			case "timing block bound":
				f.calls[d+Hash([]byte("finalityDelayBlocks()"))[:10]] = bytesWord("100")
			case "treasury missing":
				delete(f.calls, d+Hash([]byte("rootServiceTreasury()"))[:10])
			case "treasury zero":
				f.calls[d+Hash([]byte("rootServiceTreasury()"))[:10]] = addrWord(zero20)
			case "treasury self":
				f.calls[d+Hash([]byte("rootServiceTreasury()"))[:10]] = addrWord(d)
			case "registry":
...
func TestHolderHistoricalServiceAsset(t *testing.T) {
	for _, mode := range []string{"valid", "missing balance", "insolvent", "bad target", "budget"} {
		t.Run(mode, func(t *testing.T) {
			f, b, markets, d := holderSetup(t)
			asset := "0x" + strings.Repeat("6", 40)
			targets := []ServiceAssetTarget{{Distributor: d, Asset: asset}}
			f.calls[d+Hash([]byte("totalQuoteLiability(address)"))[:10]+addressArgument(asset)] = bytesWord("0")
			f.calls[d+Hash([]byte("totalServiceLiability(address)"))[:10]+addressArgument(asset)] = bytesWord("3")
			key := asset + Hash([]byte("balanceOf(address)"))[:10] + addressArgument(d)
			f.calls[key] = bytesWord("3")
			if mode == "missing balance" {
				delete(f.calls, key)
			}
			if mode == "insolvent" {
				f.calls[key] = bytesWord("2")
			}
			if mode == "bad target" {
				targets[0].Asset = "bad"
			}
			if mode == "budget" {
				targets = make([]ServiceAssetTarget, MaxHistoricalServiceAssets+1)
			}
			batch, err := ObserveHolderBlock(context.Background(), f, f.manifest, b, markets, targets...)
			if mode == "missing balance" || mode == "bad target" || mode == "budget" {
				if err == nil || len(batch.Observations) != 0 {
					t.Fatal("invalid history target accepted", err)
				}
				return
			}
			if err != nil {
			...
			found := false
			for _, row := range batch.Observations {
				if row.Kind == "treasurySolvency" && row.Key == d+":"+asset {
					found = true
					checks := row.Value["checks"].(map[string]bool)
					if row.Value["totalServiceLiability"] != "3" || row.Value["knownHolderMarketOutstanding"] != "0" || checks["balanceCoversLiabilities"] != (mode == "valid") {
						t.Fatal(row)
					}
				}
			}
			if !found || batch.Expected != len(batch.Observations) {
				t.Fatal("old asset omitted", batch)
			}
		})
	}
}
