...
async function prepare(input:PreparationInput){
 ...
 for(const row of rows){
  try{
   ...
   const updated=await pool.query(`WITH locked_head AS MATERIALIZED (SELECT block_number FROM ${s}.confirmed_display_cursor WHERE environment=$1 AND chain_id=$2 AND deployment_digest=$3 AND block_number=$6 AND block_hash=$7 FOR SHARE NOWAIT) UPDATE ${s}.confirmed_display_markets m SET payload=$5::jsonb WHERE m.environment=$1 AND m.chain_id=$2 AND m.deployment_digest=$3 AND m.market_id=$4 AND m.storage_revision=$8 AND EXISTS(SELECT 1 FROM locked_head)`,[...id,row.market_id,JSON.stringify(next),row.head_number,row.head_hash,row.storage_revision]);
