...
import {recentDisplayTrades} from './state.ts';
...
export async function hydrateDisplayHistory(client:Pick<Pool,'query'>,d:DeploymentIdentity,state:DisplayState,schemaName?:string){
 const schema=displaySchema(schemaName),id=displayIdentity(d);
  // One-time worker backfill for existing markets, with no time-window cutoff.
  // Keep newer receipt-derived records ahead of finalized history, deduplicated.
  if(!state.recentTradesHydrated){
   const rows=await client.query<{payload:TradeActivity}>(`SELECT t.payload FROM ${schema}.market_trades t JOIN ${schema}.chain_blocks b ON b.environment=t.environment AND b.chain_id=t.chain_id AND b.deployment_digest=t.deployment_digest AND b.hash=t.block_hash WHERE t.environment=$1 AND t.chain_id=$2 AND t.deployment_digest=$3 AND t.market_id=$4 AND b.canonical AND b.finalized AND b.number<=$5 ORDER BY b.number DESC,(t.payload->'source'->>'transactionIndex')::bigint DESC,t.log_index DESC LIMIT 30`,[...id,state.market.marketId,state.blockNumber]);
   state={...state,recentTrades:recentDisplayTrades(state.recentTrades??[],state.trades,rows.rows.map(({payload})=>displayTrade(payload))),recentTradesHydrated:true};
  }
  ...
  if(state.latestTrade===undefined){
   const latest=state.recentTrades?.[0]??state.trades[0]??null;
