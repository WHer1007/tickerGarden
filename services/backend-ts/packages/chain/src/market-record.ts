...
export function decodeMarketRecord(raw: Hex) {
  if (raw.length === 2 + 21 * 64) return decodeFunctionResult({ abi: burnMarketRecordAbi, functionName: 'market', data: raw });
  if (raw.length === 2 + 20 * 64) {
    const value=decodeFunctionResult({abi:legacyMarketRecordAbi,functionName:'market',data:raw});
    return {...value,config:{...value.config,burnMemeFees:false}};
  }
