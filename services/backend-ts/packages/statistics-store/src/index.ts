...
export async function readProtocolStatistics(input:{readonly pool:Pool;readonly deployment:DeploymentIdentity;readonly schemaName?:string}){
  ...
  ]);const decimals:Record<string,number>={};const feeAssetSet=new Set(rows.rows.map(row=>row.asset));for(const market of markets){decimals[market.quoteAsset]=quoteDecimals(market.quoteAssetConfigId);if(feeAssetSet.has(market.memeToken))decimals[market.memeToken]=18}
