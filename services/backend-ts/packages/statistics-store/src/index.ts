...
export async function readMarketStatistics(input: { readonly pool: Pool; readonly deployment: DeploymentIdentity; readonly marketIds?: readonly Hex32[]; readonly now?: Date; readonly schemaName?: string }) {
  ...
  const [lastTrades,lastBuys,volumes,supplies]=await Promise.all([
    ...
    input.pool.query<{market_id:Hex32;total_supply_raw:string}>(`SELECT market_id,total_supply_raw::text FROM ${schema}.holder_snapshots WHERE environment=$1 AND chain_id=$2 AND deployment_digest=$3 AND market_id=ANY($4::text[]) AND block_number=$5 AND block_hash=$6`,[...identity(input.deployment),marketIds,point.number,point.hash]),
  ]);
  ...
  for(const market of markets){
    ...
    if(quoteUsd&&quoteReference){
      const observed=market.display?.blockNumber===point.number&&market.display.blockHash===point.hash?market.display:undefined;
...
export async function readProtocolStatistics(input:{readonly pool:Pool;readonly deployment:DeploymentIdentity;readonly schemaName?:string}){
  const schema=identifier(input.schemaName??'tickergarden_serverless'),[projection,point]=await Promise.all([checkpoint(input.pool,schema,input.deployment),ingestionPoint(input.pool,schema,input.deployment)]),from=Math.max(0,point.asOf-86400);
  await coverageFor(input.pool,schema,input.deployment,point,from,point.asOf);
  const [rows,markets,positions]=await Promise.all([
    ...
    currentMarkets(input.pool,schema,input.deployment,projection.revision),currentPositions(input.pool,schema,input.deployment),
  ]);const decimals:Record<string,number>={};for(const market of markets){decimals[market.quoteAsset]=quoteDecimals(market.quoteAssetConfigId);decimals[market.memeToken]=18}
  ...
  const stockAmounts:Record<string,string>={};const stakingUsers=new Set<Address>();
...
export async function readGlobalSeries(input:{readonly pool:Pool;readonly deployment:DeploymentIdentity;readonly from:number;readonly to:number;readonly interval:60|300|900|3600|14400|86400;readonly schemaName?:string}){
  ...
  const data=await flowData(input);const identities=aggregateFlowGroups(data.markets,[],true).map(g=>({assetUid:g.assetUid,quoteAsset:g.quoteAsset,quoteDecimals:g.quoteDecimals,binding:g.binding}));
  const buckets=new Map<number,Array<{payload:TradeActivity}>>();for(const row of data.trades){const timestamp=Number(row.payload.timestamp),at=Math.floor(timestamp/input.interval)*input.interval;if(at>=input.from&&at<input.to){const bucket=buckets.get(at)??[];bucket.push(row);buckets.set(at,bucket)}}
  const points=[];for(let at=input.from;at<input.to;at+=input.interval){const found=new Map(aggregateFlowGroups(data.markets,buckets.get(at)??[],true).map(g=>[`${g.assetUid}:${g.quoteAsset}`,g]));points.push({timestamp:at,groups:identities.map(id=>found.get(`${id.assetUid}:${id.quoteAsset}`)??{...id,tradeCount:0,internalTradeCount:0,unclassifiedTradeCount:0,quoteVolumeRaw:'0',internalQuoteVolumeRaw:'0',fees:[],unknownFeeTradeCount:0})})}
...
async function globalFlow(input:{readonly pool:Pool;readonly deployment:DeploymentIdentity;readonly from:number;readonly to:number;readonly schemaName?:string},series:boolean){
  const data=await flowData(input),output=aggregateFlowGroups(data.markets,data.trades,series);
  ...
  return{chainId:input.deployment.chainId,displayOnly:true as const,coverage:data.coverage,marketCount:data.markets.length,registeredStockCount:stocks.length,boundMarketCount:data.markets.length,unboundMarketCount:0,stocks,groups:output};
}
async function flowData(input:{readonly pool:Pool;readonly deployment:DeploymentIdentity;readonly from:number;readonly to:number;readonly schemaName?:string}){
  ...
  const markets=await currentMarkets(input.pool,schema,input.deployment,point.revision);
  ...
  return{coverage,markets,trades};
}
function aggregateFlowGroups(markets:readonly Market[],trades:readonly {payload:TradeActivity}[],series:boolean){
  const byMarket=new Map(markets.map(m=>[m.marketId,m]));
