...
export async function fetchPriceReferences(targets: readonly PriceTarget[], options: { readonly fetcher?: typeof fetch; readonly now?: Date; readonly maxAgeSeconds?: number } = {}): Promise<PriceReference[]> {
  ...
  let assets: unknown[]; let quotes: unknown[];
  try { [assets, quotes] = await Promise.all([get('/assets').then((v) => array(v.assets)), get('/prices').then((v) => array(v.quotes))]); }
  ...
  return targets.map((target) => {
    try {
      ...
      if (asset.id !== target.assetUid || asset.tokenSymbol !== target.symbol || asset.status !== 'ASSET_STATUS_ACTIVE' || typeof asset.currentMultiplier !== 'string'
        || !matchesDeployment(asset.deployments, target)) throw new Error('asset_unavailable');
      // An action announcement (including a future dividend) is not a trading halt.
      // Keep using the current multiplier until an explicitly scheduled transition is due.
      let multiplierEffective: number | undefined;
      if (asset.pendingMultiplier !== '') {
        const effective = typeof asset.pendingMultiplierEffectiveTime === 'string' ? Date.parse(asset.pendingMultiplierEffectiveTime) : NaN;
        if (typeof asset.pendingMultiplier !== 'string' || !positive(asset.pendingMultiplier)
          || !Number.isFinite(effective) || effective <= retrieved.getTime()) throw new Error('multiplier_transition_pending');
      ...
      if (compareDecimal(bid, ask) > 0 || !PRODUCT_DECIMAL.test(bid) || !/[1-9]/.test(bid) || !PRODUCT_DECIMAL.test(ask) || !/[1-9]/.test(ask) || !positive(asset.currentMultiplier)) throw new Error('invalid_price');
      const asOf = new Date(quote.generatedAt); if (!Number.isFinite(asOf.getTime()) || asOf > retrieved) throw new Error('invalid_timestamp');
      const expires = new Date(Math.min(asOf.getTime() + maxAge * 1_000, multiplierEffective ?? Infinity)); if (expires <= retrieved) return { ...unavailable(target, retrieved, 'price_expired'), status: 'stale' as const, multiplier: asset.currentMultiplier, asOf: asOf.toISOString(), expiresAt: expires.toISOString() };
...
export async function fetchTestnetPriceReferences(
  ...
  if (targets.length > 64 || targets.some((target) => target.chainId !== 46630)) throw new Error('invalid testnet display price targets');
  ...
  const nativeTarget: PriceTarget = { chainId: CURRENT_CHAIN_ID, token: NATIVE_TOKEN, assetUid: NATIVE_ASSET_UID, symbol: 'ETH' };
  let ethUsd: string;
...
async function fetchNativeUsd(fetcher: typeof fetch, currency:'ETH'='ETH'): Promise<string> {
  const response = await fetcher(`https://api.coinbase.com/v2/prices/${currency}-USD/spot`, { headers: { accept: 'application/json' }, redirect: 'error', signal: AbortSignal.timeout(10_000) });
  ...
  if (body.data?.base !== currency || body.data.currency !== 'USD' || typeof amount !== 'string' || !positive(amount)) throw new Error('native_response_invalid');
  return amount;
}
...
export async function fetchRuntimePriceReferences(targets:readonly PriceTarget[],options:Parameters<typeof fetchTestnetPriceReferences>[1]):Promise<PriceReference[]> {
 ...
 if(usdg)currencies.push({chainId:4663,token:String(usdg.values.quoteAsset) as Address,assetUid:usdg.id,symbol:'USDG'});
 for(const target of currencies){
  if(target.symbol==='USDG'){
   result.push({...target,source:'fixed_usd',unit:'USD_PER_WHOLE_TOKEN',status:'available',bidUsd:'1',askUsd:'1',multiplier:'1',asOf:now.toISOString(),expiresAt:new Date(now.getTime()+300_000).toISOString(),retrievedAt:now.toISOString()});
   continue;
  }
  try{const price=await fetchNativeUsd(options.fetcher??fetch,'ETH');result.push({...target,source:'coinbase_spot',unit:'USD_PER_WHOLE_TOKEN',status:'available',bidUsd:price,askUsd:price,multiplier:'1',asOf:now.toISOString(),expiresAt:new Date(now.getTime()+300_000).toISOString(),retrievedAt:now.toISOString()});}
  catch{result.push(unavailableFrom(target,now,'coinbase_spot','spot_price_unavailable'));}
 }
 return result;
}
