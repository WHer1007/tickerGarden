...
export async function latestPrices(pool:Pick<Pool,'query'>,deployment:DeploymentIdentity,now:Date,schemaName?:string){
 ...
 // One effective current quote per asset/source; history belongs in logs.
 return (await pool.query<{asset:Address;payload:PriceReference}>(`SELECT asset,payload FROM ${schema}.price_references WHERE environment=$1 AND chain_id=$2 AND deployment_digest=$3 ORDER BY asset,source`,identity(deployment))).rows;
}
