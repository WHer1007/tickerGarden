...
`npm run alchemy:webhook:plan` prints the redacted Custom Webhook plan. After the pipeline has a public HTTPS callback, run the create command through the root `tooling` environment. It creates the subscription inactive, rejects any response that does not explicitly contain `is_active=false`, and atomically stores the returned webhook ID/signing key in the current mode-0600 `.env.<profile>.local`; stdout never contains the Auth Token or signing key. Copy those two runtime values into the protected pipeline Preview environment, test a real signed delivery, then activate the webhook. The block notification only wakes the RPC ingestion path; it is never accepted as complete log data.
