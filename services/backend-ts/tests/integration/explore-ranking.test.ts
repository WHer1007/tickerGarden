...
test('Explore ranks actual display prices and finalized buys before filtering and paging',async t=>{
 ...
 try{
  ...
  for(let n=1;n<=6;n++){
   ...
   const payload={marketId:h(n),memeToken:a(n),quoteAsset:a(99),assetUid:h(n%2),launchPhase:n>4?1:0,identity:{name:`Token ${n}`,symbol:`T${n}`,deployedAt:String(n)},display:{priceQuote:n===6?null:String(n),totalSupplyRaw:'100000000000000000000',blockNumber:'10',blockHash:h(10)}};
  ...
  for(const [market,tx,log,block,side] of [[1,2,1,10,'buy'],[2,2,2,10,'buy'],[3,1,9,10,'buy'],[4,99,1,11,'buy'],[5,9,9,10,'sell']] as const){
   await pool.query(`INSERT INTO ${s}.market_trades(environment,chain_id,deployment_digest,market_id,block_hash,transaction_hash,log_index,occurred_at,classification,base_raw,quote_raw,payload) VALUES($1,$2,$3,$4,$5,$6,$7,now(),'unclassified',1,1,$8)`,[...id,h(market),h(block),h(100+market),log,{side,timestamp:'10',source:{transactionIndex:String(tx)}}]);
  }
  const buys=await query({sort:'recentBuy_desc'});
  assert.deepEqual(buys.items.map((x:any)=>x.marketId),[h(2),h(1)]);
  assert.deepEqual((await query({sort:'recentBuy_desc'},buys.nextCursor!)).items.map((x:any)=>x.marketId),[h(3),h(4)]);
  // A price refresh invalidates the old ranking cursor instead of silently
  // mixing rankings across pages; the frontend can then recover to page one.
