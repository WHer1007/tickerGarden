...
test('TS-10 history projection serves complete directories and exact deduplicated payments', { timeout: 30_000 }, async (context) => {
  ...
  try {
    ...
    await handle.pool.query(`INSERT INTO ${schema}.projection_records(environment,chain_id,deployment_digest,scope,revision,identity,sort_key,payload_digest,payload) VALUES ('test',46630,$1,'markets',$2,$3,$3,$4,$5)`,
      [deployment.deploymentDigest, revision, marketId, hash('e'), { marketId, memeToken: token, quoteAsset: quote, creator, creatorFeesToHolders: true,
    ...
    await saveEvent(handle.pool, schema, deployment.deploymentDigest, 'HolderRewardsDistributorV1', 'HolderStreamMarketRegistered',
    ...
    assert.deepEqual(await projectF72History({ pool: handle.pool, deployment, blockNumber: 1n, blockHash: hash('b'), generation: 0n, schemaName }),
      { rewards: 3, activities: 4, aggregates: 4 });
    ...
    assert.equal(holderHistory.complete, true); assert.deepEqual(holderHistory.claimed, { [quote]: '7', [token]: '3' });
    ...
    assert.equal(secondActivity.status, 200); assert.equal((await secondActivity.json() as { items: unknown[] }).items.length, 2);
