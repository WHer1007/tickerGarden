...
test('20,000 scoped markets reuse durable scan coverage across batches/restart and discard it on reorg',{timeout:120000},async t=>{
 ...
 try{
  ...
  await pool.query(`INSERT INTO ${s}.confirmed_display_markets(environment,chain_id,deployment_digest,market_id,block_number,block_hash,payload)
  SELECT $1,$2,$3,'0x'||lpad(to_hex(n),64,'0'),1,$4,jsonb_build_object('creation',jsonb_build_object('marketId','0x'||lpad(to_hex(n),64,'0'),'memeToken','0x'||lpad(to_hex(n),40,'0'),'curve','0x'||lpad(to_hex(n+30000),40,'0'),'gauge','0x'||repeat('0',40),'source',jsonb_build_object('blockNumber','1')),'market',jsonb_build_object('marketId','0x'||lpad(to_hex(n),64,'0'),'launchPhase',0,'identity',jsonb_build_object('deployedAt','1'))) FROM generate_series(1,20000)n`,[...id,h(1)]);
  const client=await pool.connect();
  try{
   const scan=await readDisplayScan(client,d,rpc,2n,201n,2001n,name);assert.equal(scan.creations.size,20000);const first=logCalls;assert.ok(first>=158);assert.equal(factoryCalls,1);
