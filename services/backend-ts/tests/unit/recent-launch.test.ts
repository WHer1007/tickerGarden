...
test('creation analytics includes initial mint and buy without any finalized journal',()=>{
 ...
 assert.equal(result.confirmation,'confirmed');assert.equal(result.holders.count,1);assert.equal(result.holders.totalSupplyRaw,supply);assert.equal(result.holders.circulatingSupplyRaw,supply);assert.equal(result.holders.basis,'CHAIN_TOTAL_SUPPLY_V1');assert.equal(result.trades.length,1);assert.equal(result.trades[0]?.price,'0.01');assert.equal(result.statistics.price,'0.01');assert.equal(result.statistics.volume24h,'0.01');assert.equal(result.statistics.volumeTo,1000);
