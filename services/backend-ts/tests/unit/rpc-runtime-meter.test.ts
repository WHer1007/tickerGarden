...
import {rpcRuntimeOptions,rpcControlPoolRole} from '../../packages/rpc-control/src/runtime.ts';
import {summarizeRpcUsage} from '../../packages/rpc-control/src/meter.ts';
test('RPC control pools have separate per-service connection allocations',()=>{
 for(const service of ['read-api','pipeline','resident-worker','confirmed-display-worker'])assert.equal(rpcControlPoolRole(service),`${service}-rpc-control`);
 for(const service of ['holder-publication','creator-sweep','stake-cleanup-plan'])assert.equal(rpcControlPoolRole(service),'tooling-rpc-control');
});
