...
test('generic Transfer queries only project tokens and splits oversized ranges without loss',async()=>{
 ...
 const rpc={call:async(_method:string,[filter]:any[])=>{calls.push(filter);if(!filter.topics[0].includes(topic)){assert.equal(filter.address,undefined);return [{kind:'protocol'}];}assert.deepEqual(filter.address,[token]);if(filter.fromBlock!==filter.toBlock)throw Error('RPC response exceeds size limit');return [{block:filter.fromBlock}];}} as unknown as RpcTransport;
 const logs=await displayLogs(rpc,new Map([[token,'TickerMemeTokenV1'],['factory','TickerGardenFactoryV1']]),1n,2n);
 assert.deepEqual(logs,[{kind:'protocol'},{block:'0x1'},{block:'0x2'}]);assert.equal(calls.length,4);
});
