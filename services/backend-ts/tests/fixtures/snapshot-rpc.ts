...
export class FakeRpc extends RpcTransport {
 lastRound=0n;publisher=a(99);badBalance=false;underfunded=false;root=h(0);dataHash=h(0);badChain=false;orphan=false;simulations=0;balanceReads=0;
 ...
 override async callAt(_target:Address,data:Hex,block:bigint):Promise<Hex>{
  ...
  else if(f==='balanceOf'){this.balanceReads++;result=this.badBalance?0n:BigInt(String(call.args![0]))-10n;}
