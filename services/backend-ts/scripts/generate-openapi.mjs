...
const spec = {
  openapi: "3.1.0",
  info: { title: "TickerGarden V1 Read API", version: "4.0.0", description: "Non-custodial TypeScript Serverless read API for the routes consumed by the current frontend." },
  ...
  paths: {
    ...
    "/v1/market-statistics": {get:{operationId:"getMarketStatistics",description:"Display-only cached market cap and last observed buy, independent of financial snapshot synchronization. Queues at most 100 registry-bound market IDs; never performs RPC in the request. Unknown historical buys remain null.",parameters:[{name:"markets",in:"query",required:false,schema:{type:"string",maxLength:6699}}],responses:{"200":response(ref("MarketStatisticsResponse")),"400":{description:"Invalid market list"},"405":{description:"Read only"}}}},
    "/v1/markets": { get: { operationId: "listMarkets", parameters: [{ name: "assetUid", in: "query", required: false, schema: bytes32 },
      ...
      { name: "sort", in: "query", required: false, schema: { type: "string", enum: ["marketId_asc", "marketId_desc", "createdAt_asc", "createdAt_desc", "name_asc", "launchPhase_asc", "volume24hUsd_desc", "marketCapUsd_desc", "recentBuy_desc"] }, description: "ID, deployment-time, name, lifecycle-phase, 24-hour USD volume or USD market-cap order, with ascending marketId as tie-breaker. USD sorts use one revision-cached complete directory and place unavailable values last. Filters apply to the full revision-pinned catalog before pagination. Cursors are bound to normalized filters and sort." }, ...queryParameters], responses: { "200": response(ref("MarketPage")), "503": response(ref("ApiErrorResponse"), "Market identity or ranking metrics unavailable"), ...errors } } },
