...
const spec = {
  openapi: "3.1.0",
  info: { title: "TickerGarden V1 Read API", version: "5.12.0", description: "Non-custodial TypeScript Serverless read API for the routes consumed by the current frontend." },
  ...
  paths: {
    ...
    "/v1/markets/{marketId}/detail":{get:{operationId:"getTokenDetail",description:"Display-only approved token detail analytics. Prefers cached scheduled Dune results and falls back per section to finalized indexer coverage. Null means unavailable, never zero. Amounts are decimal strings; price and cap are Quote denominated. Circulating supply follows token totalSupply; only burns reduce it. No execution or settlement authority.",parameters:[{name:"marketId",in:"path",required:true,schema:bytes32},{name:"period",in:"query",required:false,schema:{type:"string",enum:["1H","12H","1D"]}},{name:"section",in:"query",required:false,schema:{type:"string",pattern:"^(activity|statistics|chart|trades|holders|fees)(,(statistics|chart|trades|holders|fees))*$"},description:"Comma-separated changed display sections. Omit for initial full detail; activity selects trades and fees."}],responses:{"200":response(ref("TokenDetailResponse")),"400":response(ref("CandleError")),"405":response(ref("CandleError")),"503":response(ref("CandleError"))}}},
    ...
    "/v1/markets": { get: { operationId: "listMarkets", parameters: [recentParameter,{ name: "assetUid", in: "query", required: false, schema: bytes32 },
      ...
      {name:"stakingEnabled",in:"query",required:false,schema:{type:"boolean"}},
  ...
  components: { schemas: {
    ...
    TokenDetailHolders:object({totalSupplyRaw:uintString,circulatingSupplyRaw:uintString,count:{type:"integer",minimum:0},basis:{type:"string",enum:["CHAIN_TOTAL_SUPPLY_V1","TOTAL_MINUS_KNOWN_PROTOCOL_BALANCES_V1"],description:"CHAIN_TOTAL_SUPPLY_V1 equals current onchain totalSupply including locked and protocol-held balances; legacy basis accepted during rollout."},items:{type:"array",maxItems:100,items:ref("TokenDetailHolder")}}),
