...
链上只使用 Stock Token 的实际 raw balance 与 decimals。`decimals` 用于执行“单个非零市场仓位严格大于 `0.5 STOCK`”的门槛，并把10个完整 STOCK转换为每市场不可变的 raw-unit 饱和值；手续费权重使用同一 Gauge 内各用户已激活 STOCK 的相对比例。Quote 侧手续费按 Quote 分，Meme 侧手续费按 Meme 分，不转换成 STOCK，也不进行美元净额结算。

此前生成的 Chainlink 目录和 backing-target 工具仅保留为 `V2-EXEC-1` 历史研究证据，不是 `V2-EXEC-3` 的协议输入、准入条件或部署门禁；边界见 [`spec/RETIRED_STOCK_PRICE_RESEARCH.md`](./spec/RETIRED_STOCK_PRICE_RESEARCH.md)。
...
`S=0` 时总费近似40/0/40/20；`0<S<B` 时质押者份额从0线性释放，Creator 与 Platform 对称下降；`S>=B` 时达到20/40/20/20。质押者 Bucket 按费用发生时各用户的 `userActiveStock/S` 分配；超过饱和点的所有 active STOCK 仍参与池内比例。新分配在30秒激活前不参与；增加仓位不会获得历史手续费；24小时锁定从本次 allocation 交易开始计算。每种收费资产使用独立 accumulator 和 remainder，最终整数余数确定性归 Platform。
...
5. 通过48小时延迟的 `PROTOCOL_ADMIN_ROLE` 逐资产调用 `registerAsset(bytes32,address,uint8,address)`；
...
- 暂停或退役不得改变既有市场的 STOCK Base，不得阻塞历史 Creator/Staker/Platform claim、成熟 pending、到期释放或用户 STOCK 本金退出；
...
- 单个非零用户市场仓位严格大于 `0.5 STOCK`，不要求0.5的整数倍。
...
- 手续费按实际收费资产原币种守恒；有效 STOCK 总量通过 `min(S,B)/B` 决定质押者总 Bucket，用户 active STOCK 比例决定该 Bucket 的内部份额。
- 每市场 `B` 必须等于创建时冻结的 `10 × 10^stockDecimals`，且进入 MarketConfig、expectedEconomics 与事件；它不依赖价格。
